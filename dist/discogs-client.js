...
function _nextWaiter(lane) {
    ...
    for (const p of _PRIORITY_ORDER) {
        ...
        if (q.length)
            return { resolve: q.shift(), priority: p };
    }
    return undefined;
}
    ...
    const tick = () => {
        ...
        setTimeout(() => {
            ...
            if (picked) {
                ...
                if (picked.priority === "realtime")
                    lane.lastRealtime = lane.lastDispatch;
                picked.resolve();
            }
            tick();
        }, wait);
...
function _nextWaiterEligible(lane) {
    const paused = _sweepsPaused(lane);
