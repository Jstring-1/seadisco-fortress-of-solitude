...
app.get("/search", async (req, res) => {
    ...
    try {
        const results = await dc.search(q, {
            type: req.query.type,
            artist,
            releaseTitle: req.query.release_title,
        ...
        if (userId && isFirstPage) {
            ...
            if (rawQ && rawQ.toLowerCase() !== artistParam.toLowerCase())
                p.q = rawQ;
            if (artistParam)
                p.artist = artistParam;
            if (req.query.release_title)
                p.release_title = String(req.query.release_title);
            if (req.query.label)
                p.label = String(req.query.label);
            if (req.query.year)
                p.year = String(req.query.year);
            if (req.query.genre)
                p.genre = String(req.query.genre);
            if (req.query.style)
                p.style = String(req.query.style);
