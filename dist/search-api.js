...
app.get("/api/user/collection/prices", async (req, res) => {
    ...
    const sort = req.query.sort ?? "value_desc";
    const page = Math.max(1, parseInt(req.query.page) || 1);
    const perPage = Math.min(200, parseInt(req.query.per_page) || 96);
    try {
...
app.post("/api/user/alerts", express.json(), async (req, res) => {
    ...
    if (!releaseId || !threshold) {
        res.status(400).json({ error: "releaseId and threshold required" });
        return;
    }
    if (type !== "below" && type !== "above") {
        res.status(400).json({ error: "type must be 'below' or 'above'" });
        return;
    }
    ...
    if (threshold >= 100000) {
        res.status(400).json({ error: "threshold must be less than 100000" });
        return;
    }
    try {
