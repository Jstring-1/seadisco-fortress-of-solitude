...
// GET /api/blues-archive/timeline — year-bucketed release feed.
// Defaults to 1925–1930 / limit 500 (the project's intended early-
// blues window). Includes cover_thumb when release_cache has the
// release; null otherwise. Admin-only.
app.get("/api/blues-archive/timeline", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const fromYear = Math.max(1900, Math.min(2100, parseInt(String(req.query.from ?? "1925"), 10) || 1925));
        const toYear = Math.max(fromYear, Math.min(2100, parseInt(String(req.query.to ?? "1930"), 10) || 1930));
        const limit = Math.max(1, Math.min(2000, parseInt(String(req.query.limit ?? "500"), 10) || 500));
        res.json(await listBluesTimelineReleases({ fromYear, toYear, limit }));
    }
    catch (err) {
        console.error("[blues-archive timeline]", err);
        res.status(500).json({ error: String(err) });
    }
});
