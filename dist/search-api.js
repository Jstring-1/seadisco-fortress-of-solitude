...
import { scoreAiChannels, AI_FLAG_THRESHOLD } from "./yt-ai-detect.js";
...
function _extractDiscogsProfile(profile) {
    return {
...
// ── Account cap ─────────────────────────────────────────────────────────
// Total SeaDisco accounts (Clerk users, connected or not). Matches the
// Clerk development instance's 100-user limit: new sign-ups are closed in
// the UI once it's reached, and accounts that never connect Discogs are
// deleted after UNCONNECTED_DELETE_DAYS idle to free their spot.
const MAX_USERS = 100;
const UNCONNECTED_DELETE_DAYS = 42;
// (the global API kill switch lives in api-guard.ts — shared with the
// Discogs client and the workers)
// Clerk account count, memoized briefly — the public sign-up status check
// hits this on page loads. null = Clerk unreachable / not configured.
let _clerkCountMemo = { n: null, at: 0 };
async function getClerkAccountCount(maxAgeMs = 60_000) {
    if (_clerkCountMemo.n != null && Date.now() - _clerkCountMemo.at < maxAgeMs)
        return _clerkCountMemo.n;
    ...
    try {
        ...
        if (!cr.ok)
            return _clerkCountMemo.n;
        const n = Number((await cr.json())?.total_count);
        if (!Number.isFinite(n))
            return _clerkCountMemo.n;
        _clerkCountMemo = { n, at: Date.now() };
        return n;
    }
    catch {
        return _clerkCountMemo.n;
...
const _locStats = {
    ...
    lastFailureAt: null,
    lastFailureMsg: "",
    recentTs: [], // timestamps of every proxy hit in last 24h
};
const LOC_STATS_WINDOW_MS = 24 * 60 * 60 * 1000;
function _locStatsPrune() {
    const cutoff = Date.now() - LOC_STATS_WINDOW_MS;
    while (_locStats.recentTs.length && _locStats.recentTs[0] < cutoff)
        _locStats.recentTs.shift();
}
function _locStatsRecord(kind, errorMsg) {
    const now = Date.now();
...
async function loggedFetch(service, url, init) {
    ...
    const discogsPriority = init?.discogsPriority;
    // Strip our extra fields from init before passing to real fetch
    const { context: _ctx, discogsPriority: _pri, ...fetchInit } = (init ?? {});
    // Strip query params with tokens for safety
    const cleanUrl = url.replace(/token=[^&]+/g, "token=***").replace(/key=[^&]+/g, "key=***").replace(/apikey=[^&]+/g, "apikey=***");
    // Raw Discogs fetches (sync, marketplace, profile) bypass
    // DiscogsClient.get(), so gate them here too — same per-credential
    // lane keyed off the Authorization header — so ALL Discogs traffic
    // shares one budget per api key. Default realtime; scheduled/background
    // callers pass discogsPriority.
    if (service === "discogs") {
        const auth = fetchInit.headers?.Authorization ?? fetchInit.headers?.authorization;
        await discogsGate(discogsKeyFromAuthHeader(auth), discogsPriority ?? "realtime");
    }
    try {
...
let _clerkUsernameCache = null;
let _clerkUsernameCacheAt = 0;
let _clerkUsernameInflight = null;
...
let _clerkCreatedAtCache = new Map();
async function _refreshClerkUsernameCache(force = false) {
    const now = Date.now();
    ...
    if (_clerkUsernameInflight)
        return _clerkUsernameInflight;
    _clerkUsernameInflight = (async () => {
        const out = new Map();
        const createdOut = new Map();
        const clerkSecret = process.env.CLERK_SECRET_KEY ?? "";
        if (!clerkSecret)
            return out;
        try {
            ...
            console.warn("[clerk username cache]", e?.message ?? e);
        }
        return out;
    })();
    try {
        return await _clerkUsernameInflight;
    }
    finally {
        _clerkUsernameInflight = null;
    }
}
...
// Year-Label home-strip tab: admin-only unless YEAR_LABEL_OPEN=1, which
// opens it to every signed-in user.
const _yearLabelOpen = (process.env.YEAR_LABEL_OPEN === "1" || process.env.YEAR_LABEL_OPEN === "true");
...
// The Clerk frontend host comes from the publishable key (a custom
// clerk.<domain> in production, *.clerk.accounts.dev in dev).
const _clerkCspHost = (() => {
    try {
        const h = Buffer.from(authPk.replace(/^pk_(test|live)_/, ""), "base64").toString("utf8").replace(/\$+$/, "");
...
const _cspDirectivesFor = (nonce) => [
    ...
    "font-src 'self' data:",
    `connect-src 'self' https://*.clerk.accounts.dev ${_clerkCspHost} https://www.googletagmanager.com https://www.google-analytics.com https://*.google-analytics.com https://*.analytics.google.com`,
    `frame-src https://www.youtube.com https://www.youtube-nocookie.com https://*.clerk.accounts.dev ${_clerkCspHost} https://challenges.cloudflare.com`,
...
function _jsonLd(obj) {
    // Safe inside <script>: no "</script>" / "<!--" breakouts.
    return JSON.stringify(obj).replace(/</g, "\\u003c").replace(/\u2028/g, "\\u2028").replace(/\u2029/g, "\\u2029");
}
function _entityHead(type, id, data, canonical) {
    const title = String(data?.title || "Untitled");
    const artist = _entityArtistLine(data);
    const year = Number(data?.year) > 0 ? String(data.year) : "";
    const genres = [...(data?.genres || []), ...(data?.styles || [])].map(String).filter(Boolean);
    const tracks = (Array.isArray(data?.tracklist) ? data.tracklist : []).filter((t) => t?.type_ !== "heading" && t?.title);
    const labels = Array.isArray(data?.labels) ? data.labels : [];
    const format = Array.isArray(data?.formats) && data.formats[0]?.name ? String(data.formats[0].name) : "";
    const img = String((Array.isArray(data?.images) && (data.images.find((i) => i?.type === "primary") || data.images[0])?.uri) || "");
    ...
    const ld = {
        ...
        name: title,
        url,
        ...(artist ? { byArtist: (Array.isArray(data?.artists) ? data.artists : []).map((a) => ({ "@type": "MusicGroup", name: _plainArtist(a?.name) })) } : {}),
...
function _entityBody(type, data) {
    const e = _escAttr;
    const title = String(data?.title || "Untitled");
    const artist = _entityArtistLine(data);
    const year = Number(data?.year) > 0 ? String(data.year) : "";
    ...
    const img = (Array.isArray(data?.images) && (data.images.find((i) => i?.type === "primary") || data.images[0])) || null;
    ...
    const rows = tracks.map((t) => t?.type_ === "heading"
...
async function _sendEntityPage(req, res, type, id) {
    ...
    if (Number.isFinite(id) && id > 0) {
        try {
            data = await getCachedRelease(id, type, 10 * 365 * 24 * 3600);
        }
        catch {
            data = null;
        }
...
let _sitemapCounts = null;
async function _sitemapTypeCounts() {
    if (_sitemapCounts && Date.now() - _sitemapCounts.at < 6 * 3600_000)
        return _sitemapCounts;
    const r = await getPool().query(`SELECT type, COUNT(*)::int AS n FROM release_cache
      WHERE type IN ('master', 'release') AND COALESCE(data->>'title', '') <> ''
      GROUP BY type`);
    const out = { at: Date.now(), master: 0, release: 0 };
    for (const row of r.rows)
        out[row.type] = Number(row.n) || 0;
    _sitemapCounts = out;
    return out;
}
function _xmlEsc(s) {
...
app.get("/sitemap.xml", async (_req, res) => {
    try {
        ...
        for (const type of ["master", "release"]) {
            ...
            for (let i = 0; i < n; i++)
                files.push(`/sitemap-${type}-${i + 1}.xml`);
        }
...
app.get(/^\/sitemap-(master|release)-(\d{1,4})\.xml$/, async (req, res) => {
    ...
    try {
        const r = await getPool().query(`SELECT discogs_id, data->>'title' AS title, data->'artists' AS artists,
              to_char(cached_at AT TIME ZONE 'UTC', 'YYYY-MM-DD') AS lastmod
         FROM release_cache
        WHERE type = $1 AND COALESCE(data->>'title', '') <> ''
        ORDER BY discogs_id
        LIMIT $2 OFFSET $3`, [type, _SITEMAP_PAGE_SIZE, (page - 1) * _SITEMAP_PAGE_SIZE]);
        if (!r.rows.length) {
            res.status(404).end();
            return;
        }
...
// The route above matches the raw path, but express.static decodes it —
// so /%61dmin-panel.html would slip past the gate. Refuse any other
// spelling of the admin fragment before static sees it.
...
app.get("/api/me", async (req, res) => {
    ...
    const userId = await getClerkUserId(req);
    if (!userId) {
        res.json({ signedIn: false, isAdmin: false, isDemo: false, ytOpen: _ytOpenToUsers, yearLabelOpen: _yearLabelOpen });
        return;
    }
    ...
    res.json({
        ...
        yearLabelOpen: _yearLabelOpen,
    });
});
// GET /api/user-count — admin-only
app.get("/api/user-count", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const count = await getClerkAccountCount();
    res.json({ count: count ?? 0, limit: MAX_USERS });
});
// GET /api/signup-status — public. Whether new accounts can be created.
// Fails open when Clerk can't be reached (Clerk's own limit still applies).
app.get("/api/signup-status", async (_req, res) => {
    const count = await getClerkAccountCount();
    res.set("Cache-Control", "no-store");
    res.json({ open: count == null || count < MAX_USERS, limit: MAX_USERS });
});
...
app.get("/api/user/token", async (req, res) => {
    ...
    // Hibernation is retired: any hibernated account
...
// POST /api/admin/track-yt/set — body: { releaseId, releaseType,
//   trackPosition, trackTitle?, videoId?, videoTitle?, mode,
//   oldVideoId?, albumIds? }
// Admin only. mode='replace' pins a video that wins over the Discogs
// videos[] match (also used to overwrite an existing crowd-sourced
// override); mode='block' hides a wrong Discogs match (videoId is
// ignored / stored as ''). Upserts unconditionally — admin writes are
// not subject to first-submission-wins.
//
// oldVideoId (the video the row showed before this write) triggers a
// cascade into saved playlists + play queues: items pointing at the
// displaced video are swapped to the new one (replace) or removed
// (block). albumIds scopes the sweep to items added from this album.
app.post("/api/admin/track-yt/set", express.json({ limit: "8kb" }), async (req, res) => {
    ...
    const videoId = typeof b.videoId === "string" ? b.videoId.trim() : "";
    const videoTitle = typeof b.videoTitle === "string" ? b.videoTitle.slice(0, 256) : null;
    const mode = b.mode === "block" ? "block" : "replace";
    const oldVideoId = typeof b.oldVideoId === "string" && /^[A-Za-z0-9_-]{11}$/.test(b.oldVideoId.trim())
        ? b.oldVideoId.trim() : "";
    const albumIds = (Array.isArray(b.albumIds) ? b.albumIds : [])
        .slice(0, 8).map((x) => String(x ?? "").trim()).filter((s) => /^\d{1,12}$/.test(s));
    if (!releaseId || !trackPosition) {
        res.status(400).json({ error: "Bad request" });
        return;
    }
    if (mode === "replace" && !/^[A-Za-z0-9_-]{11}$/.test(videoId)) {
        res.status(400).json({ error: "Bad videoId" });
        return;
    }
    try {
        await setTrackYtOverrideAdmin({
            releaseId, releaseType, trackPosition, trackTitle,
            videoId: mode === "block" ? "" : videoId,
            videoTitle, submittedBy: String(adminId), mode,
        });
        // Cascade only when a video is actually being displaced.
        let cascade = null;
        if (oldVideoId && (mode === "block" || oldVideoId !== videoId)) {
            try {
                cascade = await cascadeTrackYtVideoSwap({
                    oldVideoId,
                    newVideoId: mode === "block" ? null : videoId,
                    albumIds,
                });
            }
            catch (ce) {
                // Cascade failure shouldn't fail the override write — report it.
                console.error("[/api/admin/track-yt/set] cascade error:", ce?.message ?? ce);
            }
        }
        res.json({ ok: true, cascade });
    }
    catch (e) {
        console.error("[/api/admin/track-yt/set] error:", e?.message ?? e);
...
app.post("/api/youtube/report-unavailable", express.json({ limit: "1kb" }), async (req, res) => {
    ...
    if (!ytReportUserLimiter.check(userId)) {
...
app.delete("/api/user/account", async (req, res) => {
    ...
    try {
        ...
        _clerkCountMemo.at = 0; // a spot may have opened under the account cap
        res.json({ ok: true });
    }
    catch (err) {
...
app.get("/api/auth/discogs/callback", async (req, res) => {
    ...
    try {
        ...
        if (!existingToken) {
            // The account cap is enforced at sign-up, not here. Create a
...
app.get("/api/user/profile", async (req, res) => {
    ...
    if (stale && profile.username) {
        try {
            const client = await getDiscogsClientForUser(userId); // own creds only — never the demo admin fallback
            if (client) {
...
app.post("/api/user/profile/refresh", express.json(), async (req, res) => {
    ...
    try {
        ...
        const profileClient = await getDiscogsClientForUser(userId);
        if (!profileClient) {
...
// Daily cleanup of unbounded scratch caches — the YouTube-search cache
// (>7d), archive.org-search cache (>90d), and price history (>1yr).
// These are side tables, NOT the release/master cache; cheap indexed
// cached_at deletes that keep them from growing forever. Fires 2 min
// after boot (so a fresh deploy trims promptly) and then every 24h.
function _pruneScratchCaches() {
    pruneYoutubeSearchCache().then(n => { if (n)
        console.log(`[prune] youtube_search_cache: ${n} rows`); }).catch(() => { });
    pruneDiscogsSearchCache().then(n => { if (n)
        console.log(`[prune] discogs_search_cache: ${n} rows`); }).catch(() => { });
    pruneArchiveSearchCache().then(n => { if (n)
        console.log(`[prune] archive_search_cache: ${n} rows`); }).catch(() => { });
    prunePriceHistory().catch(() => { });
}
setTimeout(_pruneScratchCaches, 2 * 60 * 1000);
setInterval(_pruneScratchCaches, 24 * 60 * 60 * 1000);
        ...
        if (syncCollection) {
            ...
            for (let page = 1;; page++) {
                ...
                try {
                    ...
                    const r = await fetchWithRetry(`https://api.discogs.com/users/${encodeURIComponent(username)}/collection/folders/0/releases?per_page=500&page=${page}`);
                    data = await r.json();
                    consecFail = 0;
                }
                catch (err) {
                    ...
                    consecFail++;
                    gapPages++;
                    if (consecFail >= 3 || gapPages >= MAX_SYNC_GAP_PAGES) {
                        console.error(`Sync ${username}: ending collection phase — ${consecFail} consecutive / ${gapPages} total page failures`);
                        break;
                    }
                    continue;
                }
                ...
                for (const i of items)
                    allInstanceIds.push(i.instanceId ?? -i.id);
            ...
            try {
                ...
                for (const order of ["desc", "asc"]) {
                    ...
                    for (let page = 1; page <= MAX_WANT_PAGE; page++) {
                        ...
                        let data;
                        try {
                            const r = await fetchWithRetry(`https://api.discogs.com/users/${encodeURIComponent(username)}/wants?per_page=500&page=${page}&sort=added&sort_order=${order}`);
                            data = await r.json();
                            consecFail = 0;
                        }
                        catch (err) {
                            ...
                            gapPages++;
                            if (consecFail >= 3 || gapPages >= MAX_SYNC_GAP_PAGES) {
                                console.error(`Sync ${username}: ending wantlist phase — ${consecFail} consecutive / ${gapPages} total page failures`);
                                aborted = true;
                                break;
                            }
                            continue;
                    ...
                    if (!wantlistHadGaps && seenWantIds.size >= wantlistEstimate && wantlistEstimate > 0)
                        break;
                }
...
app.post("/api/user/sync", express.json(), async (req, res) => {
    ...
    const discogsClient = await getDiscogsClientForUser(userId); // own creds only: a demo account must not sync the admin library
    if (!discogsClient) {
...
async function refreshInventoryListing(userId, client, listingId) {
...
function buildListingBody(body) {
    ...
    if (has(body.comments))
        payload.comments = String(body.comments);
    if (body.allowOffers != null)
        payload.allow_offers = !!body.allowOffers;
    if (has(body.externalId))
        payload.external_id = String(body.externalId);
    ...
    if (has(body.formatQuantity)) {
        ...
        if (Number.isFinite(n) && n > 0)
            payload.format_quantity = n;
    }
    if (!Number.isFinite(payload.release_id) || payload.release_id < 1)
...
app.post("/api/user/inventory/create", express.json(), async (req, res) => {
    ...
    try {
        ...
        if (!listingId) {
            res.status(502).json({ error: "Discogs create did not return a listing_id" });
            return;
        }
...
async function refreshOrder(userId, client, orderId) {
...
async function syncOrdersOnly(userId, username, client) {
    let total = 0;
    for (let page = 1;; page++) {
        if (page > 1)
...
async function syncInventoryOnly(userId, username, client) {
    let total = 0;
    for (let page = 1;; page++) {
        if (page > 1)
            await sleep(DISCOGS_CALL_DELAY_MS);
...
app.post("/api/user/orders/:orderId/status", express.json(), async (req, res) => {
    ...
    if (body.shipping != null)
        payload.shipping = Number(body.shipping);
    if (!payload.status && payload.shipping == null) {
        res.status(400).json({ error: "status or shipping required" });
        return;
    }
    try {
        const url = `https://api.discogs.com/marketplace/orders/${encodeURIComponent(orderId)}`;
...
app.post("/api/user/orders/:orderId/messages", express.json(), async (req, res) => {
    ...
    const payload = { message: String(body.message) };
    if (body.subject)
        payload.subject = String(body.subject);
    if (body.status)
        payload.status = String(body.status);
    try {
        const url = `https://api.discogs.com/marketplace/orders/${encodeURIComponent(orderId)}/messages`;
...
// getWantedItems reads every wantlist row, so compute it at most once per
// 10 minutes for everyone rather than once per request.
let _wantedMemo = null;
app.get("/api/wanted", async (req, res) => {
    ...
    try {
        ...
        if (!_wantedMemo || Date.now() - _wantedMemo.at > 10 * 60_000) {
            _wantedMemo = { at: Date.now(), items: await getWantedItems() };
        }
        const items = _wantedMemo.items;
        res.json({ items });
    }
    catch (e) {
        res.status(500).json({ error: String(e) });
...
app.get("/api/user/collection/export", async (req, res) => {
    ...
    try {
        ...
        await _streamLibraryCsv(res, "collection", userId, new Map(folders.map(f => [f.folderId, f.name])));
    }
    catch (e) {
        if (!res.headersSent)
...
app.get("/api/user/wantlist/export", async (req, res) => {
    ...
    try {
        await _streamLibraryCsv(res, "wantlist", userId, null);
    }
    catch (e) {
        if (!res.headersSent)
            res.status(500).json({ error: "Export failed" });
...
// Writes the CSV page by page (see getLibraryExportPage).
async function _streamLibraryCsv(res, kind, userId, folderMap) {
    const PAGE = 2000;
    let rows = await getLibraryExportPage(kind, userId, 0, PAGE);
    res.setHeader("Content-Type", "text/csv; charset=utf-8");
    res.setHeader("Content-Disposition", `attachment; filename=seadisco-${kind}.csv`);
    res.write(_csvLines([], folderMap, true));
    let offset = 0;
    while (rows.length) {
        res.write("\n" + _csvLines(rows, folderMap, false));
        if (rows.length < PAGE)
            break;
        offset += PAGE;
        rows = await getLibraryExportPage(kind, userId, offset, PAGE);
    }
    res.end();
}
function _csvLines(rows, folderMap, headerOnly) {
    ...
    try {
        ...
        try {
            if (folderId === 1)
                ...
                folderName = folders.find((f) => Number(f.folderId) === folderId)?.name ?? null;
            }
        }
        catch { }
...
app.put("/api/user/settings/default-folder", express.json(), async (req, res) => {
    ...
    if (!Number.isFinite(folderId) || folderId < 1) {
        res.status(400).json({ error: "folderId required (>=1)" });
        return;
    }
    try {
...
app.post("/api/user/folders/rename", express.json(), async (req, res) => {
    ...
    if (fid === 0 || fid === 1) {
        res.status(400).json({ error: "Cannot rename the built-in 'All' or 'Uncategorized' folders" });
        return;
    }
    try {
...
app.post("/api/user/folders/delete", express.json(), async (req, res) => {
    ...
    const fid = Number(folderId);
    if (fid === 0 || fid === 1) {
        res.status(400).json({ error: "Cannot delete the built-in 'All' or 'Uncategorized' folders" });
        return;
    }
    try {
        ...
        if (force) {
            ...
            for (let i = 0; i < contents.length; i++) {
                ...
                if (!item.instanceId)
                    continue; // skip synthetic legacy rows (shouldn't happen post-migration)
                if (i > 0)
                    await _sleep(DISCOGS_CALL_DELAY_MS);
                ...
                if (!mr.ok && mr.status !== 204) {
                    ...
                    res.status(mr.status).json({ error: `Failed to move item during folder delete: ${text}` });
                    return;
                }
            }
            await moveAllCollectionItemsBetweenFolders(ctx.userId, fid, 1);
...
app.post("/api/user/wantlist/notes", express.json(), async (req, res) => {
    ...
    try {
        ...
        if (!r.ok && r.status !== 204) {
            ...
            res.status(r.status).json({ error: `Discogs error: ${txt}` });
            return;
        }
...
function _normalizeLocResult(r) {
    ...
    const _pickImage = () => {
        const isUseful = (u) => !!u && !/\/static\/images\/original-format\//i.test(u);
        const clean = (u) => String(u).split("#")[0];
        // 1. Top-level image_url array (most common for Jukebox)
        if (Array.isArray(r.image_url)) {
            for (const u of r.image_url)
                if (typeof u === "string" && isUseful(u))
                    return clean(u);
        }
...
app.get("/api/loc/search", async (req, res) => {
    ...
    try {
        ...
        const results = playableOnly
            ? allNormalized.filter((r) => r && typeof r.streamUrl === "string" && r.streamUrl.length > 0)
            : allNormalized;
...
const _GUTENBERG_FETCH_HEADERS = { "User-Agent": "SeaDisco/1.0 (+https://seadisco.com)", "Accept": "application/json" };
const _GUTENBERG_HTML_HEADERS = { "User-Agent": "SeaDisco/1.0 (+https://seadisco.com)", "Accept": "text/html" };
...
// ── Playlists ──────────────────────────────────────────────────────────
// Saved snapshots of the play queue, shareable via /?pl=<id>. Owners can
// rename/delete; everyone can read by id (the share link is the
// permission token). No anon writes — playlists are sign-in only.
//
//   POST   /api/user/playlists         — create from current queue
//   GET    /api/user/playlists         — list mine
//   GET    /api/playlists/:id          — public read (for share links)
//   PATCH  /api/user/playlists/:id     — rename (owner only)
//   DELETE /api/user/playlists/:id     — delete (owner only)
app.post("/api/user/playlists", express.json({ limit: "256kb" }), async (req, res) => {
    const userId = await getClerkUserId(req);
    if (!userId) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    // Coerce + validate each item to the QueueItem shape. Anything
    // missing source/externalId is dropped silently — the server should
    // never persist garbage just because the client sent it.
    const cleaned = items
        .map((it) => ({
        ...
        .filter((it) => (it.source === "yt" || it.source === "loc") && it.externalId);
    if (!cleaned.length) {
        res.status(400).json({ error: "no valid items" });
        return;
    }
    try {
        const id = await createPlaylist(userId, String(name ?? ""), cleaned);
        res.json({ id });
    }
    catch (e) {
        // createPlaylist throws "Playlist limit reached" when the per-user
        // soft cap (100) is hit — surface that to the client so the toast
        // can explain instead of just showing "Save failed".
        const msg = String(e?.message ?? e);
        const code = msg.startsWith("Playlist limit") ? 409 : 500;
...
// Public — anyone with the id (share link) can read. We deliberately
// don't require auth here so the share-URL pattern works without
// forcing recipients to sign in just to peek. The owner field is
// returned but client renders a generic "shared playlist" label —
// no need to leak the clerk id beyond the JSON.
// Owner-only read by numeric id (ids are sequential, so they must not be
// a public handle).
app.get("/api/playlists/:id", async (req, res) => {
    ...
    try {
        const pl = await getPlaylist({ id, ownerId: userId });
        if (!pl) {
            res.status(404).json({ error: "Not found" });
            return;
        }
        const { clerk_user_id: _owner, ...playlist } = pl;
        res.json({ playlist });
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
...
// Public read of a shared playlist by its unguessable token (the ?pl=
// share link). The owner's id is not exposed.
app.get("/api/playlists/shared/:token", async (req, res) => {
    ...
    try {
        const pl = await getPlaylist({ shareToken: token });
        if (!pl) {
            res.status(404).json({ error: "Not found" });
            return;
        }
        const { clerk_user_id: _owner, id: _id, ...playlist } = pl;
        res.json({ playlist });
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
...
// Owner-only: get (minting on first use) the share token for a playlist.
app.post("/api/user/playlists/:id/share", async (req, res) => {
    const userId = await getClerkUserId(req);
    if (!userId) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    try {
        const token = await ensurePlaylistShareToken(id, userId, crypto.randomBytes(18).toString("base64url"));
        if (!token) {
...
app.patch("/api/user/playlists/:id", express.json({ limit: "8kb" }), async (req, res) => {
    const userId = await getClerkUserId(req);
    if (!userId) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    try {
        const ok = await renamePlaylist(id, userId, name);
        if (!ok) {
            res.status(404).json({ error: "Not found or not yours" });
            return;
        }
...
app.delete("/api/user/playlists/:id", async (req, res) => {
    ...
    try {
        const ok = await deletePlaylist(id, userId);
        if (!ok) {
            res.status(404).json({ error: "Not found or not yours" });
            return;
        }
...
// PUT /api/user/playlists/:id/replace — owner-only "overwrite" used
// by the save flow when the user picks an existing playlist's name.
// Keeps the same id (and share-URL), wipes its items, writes the new
// ones. Optionally renames in the same transaction.
app.put("/api/user/playlists/:id/replace", express.json({ limit: "256kb" }), async (req, res) => {
    const userId = await getClerkUserId(req);
    if (!userId) {
        res.status(401).json({ error: "Unauthorized" });
        return;
    ...
    try {
        const ok = await replacePlaylistItems(id, userId, cleaned, typeof name === "string" ? name : undefined);
        if (!ok) {
            res.status(404).json({ error: "Not found or not yours" });
            return;
        }
...
app.get("/api/user/favorites", async (req, res) => {
    ...
    try {
        const limit = Math.min(parseInt(req.query.limit) || 96, 200);
...
app.use("/api/admin", (req, res, next) => {
    ...
    if (!entry || now > entry.resetAt) {
        ...
        if (adminRateCounts.size > 5000) {
            for (const [k, v] of adminRateCounts) {
                if (now > v.resetAt)
                    adminRateCounts.delete(k);
            }
        }
...
function _ytValidateQueryConfig(raw) {
    const errs = [];
    for (const k of ["albumTemplate", "trackTemplate"]) {
        const v = raw?.[k];
        ...
        if (v.length > 300)
            errs.push(`${k} is longer than 300 characters`);
        if (!/\{(artist|album|track)\}/.test(v))
            errs.push(`${k} must include {artist}, {album} or {track}`);
        const unknown = (v.match(/\{[^}]*\}/g) || []).filter((t) => !/^\{(artist|album|track|noise)\}$/.test(t));
        if (unknown.length)
            errs.push(`${k} has unknown placeholder(s): ${unknown.join(", ")}`);
    }
    if (raw?.noise != null && typeof raw.noise !== "string")
        errs.push("noise must be text");
    if (typeof raw?.noise === "string" && raw.noise.length > 400)
...
async function _ytGetQueryConfig() {
    if (_ytQueryCfgMemo && Date.now() - _ytQueryCfgMemo.at < 30_000)
        return _ytQueryCfgMemo.cfg;
    let parsed = null;
    try {
...
function _ytClampQuery(q) {
    if (q.length <= _YT_MAX_Q_LEN)
        return q;
...
function _ytBuildQuery(cfg, mode, parts) {
...
function _ytSearchCacheKey(q, pageToken = "", embeddableOnly = true) {
...
// readable by both.
function _ytSimplifySearchItems(raw) {
...
function _ytWorkerItem(it) {
    if (it?.id?.videoId)
        return { videoId: String(it.id.videoId), sn: it.snippet || {} };
    if (it?.videoId) {
        return {
...
// spend. Errors are swallowed — the in-memory count is authoritative
// for gating; the DB row is a recovery cache.
function _bumpYtQuotaPersisted(workerSearches, projectUnits) {
    _ytQuotaUnitsToday += projectUnits;
    _ytReviewSearchesToday += workerSearches;
    if (workerSearches || projectUnits) {
        bumpReviewQuota(workerSearches, projectUnits).catch((e) => console.warn("[yt-quota] persist failed:", e?.message ?? e));
    }
}
...
// Discogs search cache TTL. Catalogue metadata is near-static, so a few
// hours costs nothing in freshness and removes the ~1s rate-gate wait
// from every repeated query, page-back and "load more" re-fetch. Rows
// are pruned at 3 days (see pruneDiscogsSearchCache).
const _DISCOGS_SEARCH_TTL_MS = 6 * 60 * 60 * 1000;
...
// A banned channel is filtered out of ALL YouTube results — live
// search below and the yt-review worker. The set is cached in-memory
// for a short TTL so we don't hit the DB on every search response;
// _bumpBannedChannelCache() clears it immediately after an admin edit.
let _bannedChannelSet = new Set();
let _bannedChannelAt = 0;
const _BANNED_CACHE_TTL_MS = 60_000;
async function _getBannedChannelIds() {
    if (Date.now() - _bannedChannelAt < _BANNED_CACHE_TTL_MS)
        return _bannedChannelSet;
    try {
        _bannedChannelSet = await getBannedYoutubeChannelIds();
        _bannedChannelAt = Date.now();
    }
    catch { /* keep the last-known set on error */ }
    return _bannedChannelSet;
}
function _invalidateBannedChannelCache() { _bannedChannelAt = 0; }
// Strip banned-channel items from a YouTube search body. Returns a
// NEW body (doesn't mutate the cached original) only when something is
// removed; otherwise returns the input untouched.
function _ytFilterBannedBody(body, banned) {
    if (!banned.size || !body || !Array.isArray(body.items) || !body.items.length)
        return body;
    const kept = body.items.filter((it) => !banned.has(String(it?.channelId || "")));
    if (kept.length === body.items.length)
        return body;
    return { ...body, items: kept };
}
...
app.get("/api/youtube/search", async (req, res) => {
    ...
    // Banned-channel filter, applied to EVERY response path below
    // (cached + fresh) so a ban takes effect retroactively over already-
    // cached search bodies without needing to purge the cache.
    const _banned = await _getBannedChannelIds();
    ...
    if (!noCache) {
        ...
        if (memCached) {
            ...
            res.json(_ytFilterBannedBody(enriched, _banned));
            return;
        }
    ...
    if (_ytQuotaUnitsToday + 100 > _YT_DAILY_SOFT_CAP_UNITS) {
        ...
        // Surface the last cached_at for this query (regardless of
        // staleness) so the client can show "last searched X ago" on
        // the external-YouTube fallback link.
        const lastTs = await getYoutubeSearchCacheTimestamp(cacheKey).catch(() => null);
        res.status(429).json({
            ...
            usedToday: _ytQuotaUnitsToday,
            cap: _YT_DAILY_SOFT_CAP_UNITS,
            lastSearchedAt: lastTs ? lastTs.toISOString() : null,
        });
        return;
    }
    ...
    if (inflight) {
        try {
            ...
            res.json(_ytFilterBannedBody(body, _banned));
            return;
        }
        catch (e) {
...
app.get("/api/youtube/video-info", async (req, res) => {
    ...
    try {
        ...
        if (_ytVideoInfoCache.size >= _YT_VIDEO_INFO_CACHE_MAX) {
            ...
            if (firstKey !== undefined)
                _ytVideoInfoCache.delete(firstKey);
        }
    ...
    try {
        ...
        const items = rows.map((row) => {
            ...
            const matrix = (Array.isArray(d.identifiers) ? d.identifiers : [])
                .filter((it) => /matrix|runout/i.test(String(it?.type ?? "")))
                .map((it) => ({
...
app.get("/api/feed/random", async (req, res) => {
    ...
    try {
        ...
        const excludeIds = excludeStr
            ...
            : [];
        // mode=rare uses a dedicated sampler that hunts for early-era
        // releases with high want / near-zero have. No taste bias — the
        // whole point is rarity, not affinity. Default mode keeps the
        // weighted-random behaviour with optional taste bias for
        // signed-in users.
        const mode = String(req.query.mode ?? "").trim();
        let rows;
        // Catalog modes (feed/rare/dig/active/played) read from the
        // pre-computed pool table for sub-second response. The
        // background refresher (every ~2h) reruns the heavy SQL and
        // replaces the pool. If the pool is empty (cold boot, first
        // deploy), we fall back to the direct query and trigger a
        // refresh in the background so subsequent requests land on a
        // warm pool.
        if (mode === "rare") {
            const genreParam = String(req.query.genre ?? "").trim();
            if (genreParam) {
                // Strict-genre Rare bypasses the pool — the pool only holds
                // the cross-genre default; per-genre would explode the
                // table. The direct query is already fast (single genre
                // window).
                rows = await getFeedRareAlbums(limit, excludeIds, genreParam);
            }
            else {
                rows = await getFeedPoolItems("rare", limit, excludeIds);
                if (!rows.length) {
                    rows = await getFeedRareAlbums(limit, excludeIds, null);
                    refreshFeedPool("rare").catch(() => { });
                }
            }
        }
        else if (mode === "dig") {
            rows = await getFeedPoolItems("dig", limit, excludeIds);
            if (!rows.length) {
                rows = await getFeedDigAlbums(limit, excludeIds);
                refreshFeedPool("dig").catch(() => { });
            }
        }
        else if (mode === "active") {
            rows = await getFeedPoolItems("active", limit, excludeIds);
            if (!rows.length) {
                rows = await getFeedActiveAlbums(limit, excludeIds);
                refreshFeedPool("active").catch(() => { });
            }
        }
        else if (mode === "played") {
            rows = await getFeedPoolItems("played", limit, excludeIds);
            if (!rows.length) {
                rows = await getFeedPlayedAlbums(limit, excludeIds);
                refreshFeedPool("played").catch(() => { });
            }
        }
        else {
            // Feed: try the pool first. Signed-in users still get the
            // taste bias on the cold path (when pool is empty); when the
            // pool is warm we trade the per-user nudge for sub-second
            // response. The pool itself was picked with the scarcity
            // weighting so the bar is already high.
            rows = await getFeedPoolItems("feed", limit, excludeIds);
            if (!rows.length) {
                let tasteProfile = null;
                try {
                    const uid = await getClerkUserId(req);
                    if (uid)
                        tasteProfile = await getOrComputeUserTasteProfile(uid);
                }
                catch { /* fall through with no bias */ }
                rows = await getFeedRandomAlbums(limit, type, excludeIds, tasteProfile);
                refreshFeedPool("feed").catch(() => { });
            }
        }
        const items = rows.map((row) => {
            const d = row.data ?? {};
...
app.get("/api/user/personal-suggestions", async (req, res) => {
    ...
    try {
        ...
        const items = allItems.filter((it) => Array.isArray(it.format) && it.format.includes("Vinyl"));
...
app.get("/api/admin/behavior-stats", async (req, res) => {
    ...
    try {
        const [items, clerkUsernames] = await Promise.all([
            ...
            _refreshClerkUsernameCache().catch(() => new Map()),
        ]);
...
app.get("/api/admin/users-unified", async (req, res) => {
    ...
    try {
        ...
        const behaviorLastActive = new Map(); // clerkUserId → ms
        for (const b of behavior) {
            if (b.last_active)
                behaviorLastActive.set(b.clerk_user_id, new Date(b.last_active).getTime());
        }
        ...
        if (clerkSecret) {
            for (const u of sync) {
                if (!u.clerkUserId)
                    continue;
                if (behaviorLastActive.has(u.clerkUserId))
                    continue; // have real activity — skip the Clerk call
                try {
                    let latest = 0;
        ...
        for (const b of behavior) {
            ...
            if (b.username && !r.discogsUsername)
                r.discogsUsername = b.username;
        ...
        for (const cs of (collStats?.users ?? [])) {
...
app.get("/api/admin/cache-rate", async (req, res) => {
    ...
    try {
        ...
        for (const r of hourly.rows)
            counts.set(Number(r.t), Number(r.n));
...
// Admin: preview how many cached pressings are redundant (a specific
// release whose master is also cached and which no user data pins).
app.get("/api/admin/redundant-releases/preview", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const p = await previewRedundantReleases();
        res.json(p);
    }
    catch (e) {
        console.error("[/api/admin/redundant-releases/preview] error:", e?.message ?? e);
        res.status(500).json({ error: String(e?.message ?? e) });
    }
});
// Admin: prune redundant pressings. body { mode: 'warm_only'|'all' }.
// Deletes from release_cache + the mirrored split-cache rows in one
// transaction. Protected pressings (in any user library/list/queue/
// override) are never touched.
app.post("/api/admin/redundant-releases/prune", express.json({ limit: "1kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const mode = req.body?.mode === "all" ? "all" : "warm_only";
    try {
        const r = await pruneRedundantReleases(mode);
        res.json({ ok: true, mode, ...r });
    }
    catch (e) {
        console.error("[/api/admin/redundant-releases/prune] error:", e?.message ?? e);
        res.status(500).json({ error: String(e?.message ?? e) });
    }
});
...
app.get("/api/admin/sync-status", async (req, res) => {
    ...
    const clerkUsernames = await _refreshClerkUsernameCache().catch(() => new Map());
    const oneDayAgoMs = Date.now() - 24 * 60 * 60 * 1000;
...
function _buildReleaseCacheWhere(q) {
    const where = [];
    const args = [];
...
// represented in release_cache — those stubs are redundant once
// Discogs has the actual release. Pass no body to purge globally.
app.post("/api/admin/external-discography/purge-covered", express.json({ limit: "32kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const body = req.body || {};
    const label = body.label ? String(body.label).trim().slice(0, 200) : undefined;
    try {
        const result = await purgeExternalDiscographyCovered({ label });
        _invalidateLabelsCarouselCache(label);
        res.json({ ok: true, deleted: result.deleted, label: label ?? null });
    }
    catch (err) {
        console.error("[external-discography/purge]", err);
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
// One-shot cleanup for label_names that already shipped with HTML
// markup or runs of whitespace from a dirty seed file. Sanitizes in
// place, merges any rows whose post-clean (label_name, catno, side,
// source) collides with another.
app.post("/api/admin/external-discography/clean-label-names", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const result = await cleanDirtyExternalDiscographyLabelNames();
        _invalidateLabelsCarouselCache();
        res.json({ ok: true, ...result });
    }
    catch (err) {
        console.error("[external-discography/clean-label-names]", err);
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
// ── Label directory ────────────────────────────────────────────────
// Unified label list for the admin Labels tab: every label name from
// external_discography + release_cache, with counts on both sides
// and the Discogs label_id if known. Curator uses the Search button
// to assign IDs to the labels we scraped from outside Discogs.
app.get("/api/admin/label-directory", async (req, res) => {
    ...
    const forceReader = rawReader === "v1" ? "v1" :
...
app.post("/api/admin/label-directory/merge", express.json({ limit: "4kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const fromLabel = String((req.body || {}).fromLabel ?? "").trim();
    const toLabel = String((req.body || {}).toLabel ?? "").trim();
    if (!fromLabel || !toLabel) {
        res.status(400).json({ error: "fromLabel and toLabel required" });
        return;
    }
    try {
        const result = await mergeExternalLabel(fromLabel, toLabel);
        _invalidateLabelsCarouselCache();
        res.json({ ok: true, ...result });
    }
    catch (err) {
        console.error("[label-directory merge]", err);
        res.status(400).json({ error: err?.message ?? String(err) });
    }
});
// Alias one Discogs label_id under another so the Label directory
// collapses them into a single row. Both must be positive ints.
app.post("/api/admin/label-directory/set-alias", express.json({ limit: "4kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const b = req.body || {};
    const aliasId = Number(b.aliasLabelId);
    const canonicalId = Number(b.canonicalLabelId);
    const reason = typeof b.reason === "string" ? b.reason.trim().slice(0, 200) : null;
    if (!Number.isFinite(aliasId) || aliasId <= 0) {
        res.status(400).json({ error: "aliasLabelId required" });
        return;
    }
    if (!Number.isFinite(canonicalId) || canonicalId <= 0) {
        res.status(400).json({ error: "canonicalLabelId required" });
        return;
    }
    try {
        await addLabelAlias(aliasId, canonicalId, reason);
        _invalidateLabelsCarouselCache();
        res.json({ ok: true });
    }
    catch (err) {
        console.error("[label-directory set-alias]", err);
        res.status(400).json({ error: err?.message ?? String(err) });
    }
});
app.post("/api/admin/label-directory/remove-alias", express.json({ limit: "4kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const aliasId = Number((req.body || {}).aliasLabelId);
    if (!Number.isFinite(aliasId) || aliasId <= 0) {
        res.status(400).json({ error: "aliasLabelId required" });
        return;
    }
    try {
        const result = await removeLabelAlias(aliasId);
        _invalidateLabelsCarouselCache();
        res.json({ ok: true, ...result });
    }
    catch (err) {
        console.error("[label-directory remove-alias]", err);
        res.status(400).json({ error: err?.message ?? String(err) });
    }
});
app.post("/api/admin/label-directory/set-id", express.json({ limit: "4kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const labelName = String((req.body || {}).labelName ?? "").trim();
    const labelIdRaw = (req.body || {}).labelId;
    const labelId = labelIdRaw === null || labelIdRaw === ""
        ? null
        : Number(labelIdRaw);
    if (!labelName) {
        res.status(400).json({ error: "labelName required" });
    ...
    if (labelId !== null && !Number.isFinite(labelId)) {
        res.status(400).json({ error: "labelId must be a number or null" });
        return;
    }
    try {
        const result = await setLabelDirectoryId(labelName, labelId);
        _invalidateLabelsCarouselCache(labelName);
        res.json({ ok: true, ...result });
    }
    catch (err) {
        console.error("[label-directory set-id]", err);
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
// Discogs label search piggybacking on the admin's OAuth credentials.
// Used by the Label directory grid to surface candidate label IDs.
app.get("/api/admin/label-directory/discogs-search", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const q = String(req.query.q ?? "").trim().slice(0, 200);
    if (!q) {
    ...
    if (!ADMIN_CLERK_ID) {
        res.status(503).json({ error: "Admin Clerk id not configured" });
        return;
    }
    try {
        const oauth = await getOAuthCredentials(ADMIN_CLERK_ID);
        if (!oauth) {
            res.status(503).json({ error: "Admin Discogs OAuth not connected" });
            return;
        }
        if (!discogsConsumerKey || !discogsConsumerSecret) {
            res.status(503).json({ error: "DISCOGS_CONSUMER_KEY/SECRET not configured" });
            return;
        }
        ...
        const sr = await client.search(q, { type: "label", perPage: 10 });
        ...
        console.error("[label-directory discogs-search]", err);
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
// ── Cache analytics ─────────────────────────────────────────────
// Faceted drilldown over release_cache. Any combination of filters;
// returns total match count + top-20 per facet dimension + a small
// sample. Powers the "Explore cache" panel in admin.
app.post("/api/admin/cache-analytics", express.json({ limit: "8kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const b = req.body || {};
    const trim = (v) => {
    ...
    try {
        for (let offset = 0; offset < limit; offset += CHUNK) {
           ...
           LIMIT $${argsWithPaging.length - 1} OFFSET $${argsWithPaging.length}`;
            ...
            if (offset === 0)
                console.log(`[release-cache export] first-chunk rows=${r.rows.length} args=${JSON.stringify(argsWithPaging)}`);
            if (!r.rows.length)
                break;
            for (const row of r.rows) {
                if (format === "csv") {
...
// Credentials never leave the server in a dump, encrypted or not — a
// restored backup just means those users reconnect Discogs.
const _DUMP_REDACTED_COLUMNS = {
    user_tokens: new Set(["discogs_token", "oauth_access_token", "oauth_access_secret"]),
    oauth_request_tokens: new Set(["token", "token_secret"]),
};
app.get("/api/admin/db/dump-all", async (req, res) => {
    ...
    const dumpTable = async (table) => {
        ...
        while (true) {
            ...
            if (!r.rows.length)
                return;
            const redact = _DUMP_REDACTED_COLUMNS[table];
            for (const row of r.rows) {
                const clean = { __table: table };
                for (const [k, v] of Object.entries(row)) {
                    if (k === "ctid")
                        continue;
                    if (redact?.has(k) && v != null && v !== "" && v !== "__oauth__") {
                        clean[k] = "[redacted]";
                        continue;
                    }
                    clean[k] = v instanceof Date ? v.toISOString() : v;
            ...
            lastCtid = r.rows[r.rows.length - 1].ctid;
            if (r.rows.length < CHUNK)
                return;
        }
    };
    try {
        const tablesQ = await getPool().query(`SELECT table_name FROM information_schema.tables
...
function _buildBluesArtistsWhere(q) {
    const where = [];
    const args = [];
    ...
    if (String(q.has_discogs || "") === "1")
        where.push(`discogs_id IS NOT NULL`);
    if (String(q.has_wiki || "") === "1")
        where.push(`wikipedia_suffix IS NOT NULL AND wikipedia_suffix <> ''`);
    if (String(q.has_youtube || "") === "1")
        where.push(`youtube_urls IS NOT NULL AND jsonb_array_length(youtube_urls) > 0`);
...
app.get("/api/blues-archive/tunings", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const num = (v, def) => {
        ...
        const position = String(req.query.position ?? "").trim().slice(0, 80);
        const pitch = String(req.query.pitch ?? "").trim().slice(0, 40);
        ...
        const result = await listBluesTunings({
            q: q || undefined,
...
app.get("/api/blues-archive/tunings/facets", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const facets = await getBluesTuningsFacets();
        res.json(facets);
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.post("/api/blues-archive/tunings", express.json({ limit: "8kb" }), async (req, res) => {
    ...
    try {
        ...
        const r = await getPool().query(`INSERT INTO blues_tunings_grid (artist, track, title, position, pitch, notes)
       VALUES ($1, $2, $3, $4, $5, $6) RETURNING id`, [artist, track, title, position, pitch, notes]);
...
// GET /api/blues-archive/tunings/matching-ids — ids matching the
// current listing filter, capped at 10k. Mirrors the lyrics bulk
// editor's "Select all matching" link.
app.get("/api/blues-archive/tunings/matching-ids", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const q = String(req.query.q ?? "").trim().slice(0, 200);
        ...
        const position = String(req.query.position ?? "").trim().slice(0, 80);
        const pitch = String(req.query.pitch ?? "").trim().slice(0, 40);
        const out = await listBluesTuningIdsMatching({
            q: q || undefined,
            artist: artist || undefined,
            position: position || undefined,
            pitch: pitch || undefined,
...
// POST /api/blues-archive/tunings/bulk-update-position
//   body: { ids: number[], position: string | null }
app.post("/api/blues-archive/tunings/bulk-update-position", express.json({ limit: "256kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const body = req.body ?? {};
        ...
        if (idsRaw.length > 10000) {
            res.status(413).json({ error: "too many ids (max 10000 per call)" });
            return;
        }
        const positionRaw = body.position;
        const position = positionRaw == null || positionRaw === ""
            ? null
            : String(positionRaw).trim().slice(0, 80) || null;
        const updated = await bulkUpdateTuningPosition(idsRaw, position);
        res.json({ updated });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
// POST /api/blues-archive/tunings/bulk-delete  body: { ids: number[] }
app.post("/api/blues-archive/tunings/bulk-delete", express.json({ limit: "256kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const body = req.body ?? {};
        ...
        const deleted = await bulkDeleteTunings(idsRaw);
        res.json({ deleted });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.delete("/api/blues-archive/tunings/:id(\\d+)", async (req, res) => {
    ...
    try {
        const r = await getPool().query(`DELETE FROM blues_tunings_grid WHERE id = $1`, [id]);
...
app.post("/api/admin/lyrics/bans", express.json({ limit: "8kb" }), async (req, res) => {
    ...
    try {
        ...
        if (kind !== "title" && kind !== "artist" && kind !== "body_hash") {
            res.status(400).json({ error: "kind must be 'title', 'artist', or 'body_hash'" });
            return;
        }
        let value;
        if (kind === "body_hash") {
            // Body-hash bans: client sends a lyricId; server reads the row's
            // plaintext, hashes with the same normalisation the worker uses
            // (CRLF→LF, trim trailing whitespace; case + punctuation
            // preserved), and stores the digest. Keeping the hash logic
            // server-side guarantees the popup and the rescrape worker
            // produce the same fingerprint.
            const lyricId = parseInt(String(req.body?.lyricId ?? ""), 10);
            if (!Number.isFinite(lyricId) || lyricId <= 0) {
                res.status(400).json({ error: "lyricId required for body_hash ban" });
                return;
            }
            const row = await getLyricById(lyricId).catch(() => null);
            if (!row) {
                res.status(404).json({ error: "lyric not found" });
                return;
            }
            const plaintext = String(row.plaintext ?? "").trim();
            if (!plaintext) {
                res.status(400).json({ error: "lyric has no plaintext to fingerprint" });
                return;
            }
            value = _lyricsBodyHash(plaintext);
        }
        else {
        ...
        const n = await addBluesLyricsBans([{
                kind, value, reason,
                bannedBy: req.clerkUserId ?? null,
            }]);
        res.json({ ok: true, inserted: n, value });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    }
...
// Symmetric junction stored as (lo_id, hi_id, kind). Either side
...
async function _computeCwStats() {
    ...
    const totalR = await getPool().query(`SELECT COUNT(*)::int AS n FROM release_cache WHERE type IN ('release', 'master')`);
...
app.get("/api/admin/cache-warm-runs/status", async (req, res) => {
    ...
    try {
        ...
        res.json({
            ...
            queue: await getCacheWarmQueue().catch(() => []),
        });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.post("/api/admin/cache-warm-runs/start", express.json({ limit: "4kb" }), async (req, res) => {
    ...
    const genreKey = String(body.genreKey ?? "").trim();
    const styleKey = String(body.styleKey ?? "").trim();
    if (!genreKey) {
        res.status(400).json({ error: "genreKey required" });
        return;
    }
    try {
        const out = await startCacheWarmRun({
            genreKey,
            styleKey,
...
// Queue several combos to run back-to-back. Body: { combos: [{genreKey,
// styleKey?, fromYear?, toYear?, alsoNoYear?, resetCursor?}, ...] }.
// The first starts immediately (if idle); the rest drain as each run
// finishes. If a run is already going, all combos are queued behind it.
app.post("/api/admin/cache-warm-runs/start-batch", express.json({ limit: "16kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const raw = Array.isArray(req.body?.combos) ? req.body.combos : [];
    const num = (v) => { const n = parseInt(String(v ?? ""), 10); return Number.isFinite(n) ? n : undefined; };
    const combos = raw
        .map((c) => ({
        genreKey: String(c?.genreKey ?? "").trim(),
        styleKey: String(c?.styleKey ?? "").trim(),
        fromYear: num(c?.fromYear),
        toYear: num(c?.toYear),
        alsoNoYear: !!c?.alsoNoYear,
        resetCursor: !!c?.resetCursor,
    }))
        .filter((c) => c.genreKey);
    if (!combos.length) {
        res.status(400).json({ error: "no combos" });
        return;
    }
    try {
        const out = await enqueueCacheWarmRuns(combos);
        if (!out.ok) {
            res.status(409).json({ error: out.error || "could not start" });
            return;
        }
        _invalidateCwStats();
        res.json({ ok: true, started: out.started, queued: out.queued });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
// Read or clear the pending batch queue (does not touch the active run).
app.get("/api/admin/cache-warm-runs/queue", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        res.json({ queue: await getCacheWarmQueue() });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
app.post("/api/admin/cache-warm-runs/queue/clear", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        await clearCacheWarmQueue();
        _invalidateCwStats();
        res.json({ ok: true });
    }
    catch (err) {
...
// Rescue endpoint for when the in-memory _runningKey is stuck (e.g.
// crashed worker that didn't reach its finally block, or hot-reload
// during dev). Lets Start work again without a server restart.
app.post("/api/admin/cache-warm-runs/force-clear", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
...
// ── Catalog-number cache-warm endpoints ────────────────────────────
// Sibling to /api/admin/cache-warm-runs/* but keyed by the curated
// CATNO_SERIES seed list defined in src/cache-warm-catno.ts. Each
// series is a (label, catno range, year_max) tuple — the worker
// walks that range one catno at a time, caches matching releases.
app.get("/api/admin/cache-warm-catno/status", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const rows = await listCacheWarmCatnoRuns();
        const rowsByKey = new Map();
        for (const r of rows)
            rowsByKey.set(r.series_key, r);
        // Hydrate the response with all configured series — even ones
        // that have never been started yet (no row in cache_warm_catno_runs).
        const series = CATNO_SERIES.map(s => {
            ...
            return {
                ...
                yearMax: s.yearMax,
                notes: s.notes || "",
                phase: row?.phase ?? "catno",
                current_catno: row?.current_catno ?? null,
                label_sweep_page: row?.label_sweep_page ?? null,
                total_searched: row?.total_searched ?? 0,
                total_cached: row?.total_cached ?? 0,
                total_skipped: row?.total_skipped ?? 0,
                total_errors: row?.total_errors ?? 0,
                last_run_at: row?.last_run_at ?? null,
                last_cached_at: row?.last_cached_at ?? null,
                recent_cached: row?.recent_cached ?? [],
                recent_errors: row?.recent_errors ?? [],
            };
        });
        res.json({
            series,
            running: isCacheWarmCatnoRunning(),
            active: getActiveCacheWarmCatnoKey(),
        });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.post("/api/admin/cache-warm-catno/start", express.json({ limit: "1kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const seriesKey = String(req.body?.seriesKey ?? "").trim();
    if (!seriesKey) {
        res.status(400).json({ error: "seriesKey required" });
        return;
    }
    try {
...
app.post("/api/admin/cache-warm-catno/stop", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
...
app.post("/api/admin/cache-warm-catno/force-clear", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
...
app.post("/api/admin/cache-warm-catno/reset", express.json({ limit: "1kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const seriesKey = String(req.body?.seriesKey ?? "").trim();
    if (!seriesKey) {
    ...
    try {
        await resetCacheWarmCatnoRun(seriesKey);
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
// ── Legacy stubs — the old per-genre cron endpoints. Returns 410
// (Gone) so any stale frontend cached scripts get a clear signal
// instead of triggering an empty no-op.
app.all(/^\/api\/admin\/genre-cache-warm\/.*/, async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
...
// Fingerprint a lyric body so a re-upload of the same plaintext can be
// short-circuited on rescrape. Normalisation is intentionally light:
// CRLF→LF and trailing-whitespace trim only. Case + punctuation are
// preserved so a real edit (typo fix, missing line, different stanza
// order) produces a different hash and DOES come through.
function _lyricsBodyHash(plaintext) {
    const norm = String(plaintext ?? "").replace(/\r\n?/g, "\n").trimEnd();
...
async function _lyricsRefreshRecentRun(opts) {
    ...
    try {
        ...
        const bannedTitles = await getBannedLyricTitleSet().catch(() => new Set());
        const bannedArtists = await getBannedLyricArtistSet().catch(() => new Set());
        const bannedBodyHashes = await getBannedLyricBodyHashSet().catch(() => new Set());
        ...
        for (let i = 0; i < titles.length; i++) {
            ...
            try {
                ...
                if (!fetched) {
                    ...
                    // Body-hash ban: skip if this page's normalized plaintext
                    // matches a previously deleted lyric's fingerprint. Doesn't
                    // block re-upload of a different version of the same song.
                    if (bannedBodyHashes.size && bannedBodyHashes.has(_lyricsBodyHash(plaintext))) {
                        _lyricsScrapeState.pagesSkipped++;
                        await new Promise(r => setTimeout(r, _LYRICS_THROTTLE_MS));
                        continue;
                    }
        ...
        if (opts.stampLastAt) {
            try {
                await setAppSetting("lyrics_scrape_last_at", new Date().toISOString());
            }
            catch (e) {
                console.warn("[lyrics] stamp lyrics_scrape_last_at failed:", e);
            }
        }
    }
    catch (e) {
...
async function _lyricsPreScrapeRun() {
    if (_lyricsScrapeState.running)
        return;
    _lyricsScrapeState = {
        running: true,
        ...
        for (let i = 0; i < pages.length; i++) {
            ...
            try {
                ...
                if (!fetched) {
                    ...
                    // Body-hash ban: skip when this page's normalized plaintext
                    // matches a previously deleted lyric's fingerprint. A real
                    // edit on the wiki produces a different hash and gets
                    // through — exactly the behaviour the popup's "Delete +
                    // block this exact text" button is meant to enable.
                    if (bannedBodyHashes.size && bannedBodyHashes.has(_lyricsBodyHash(plaintext))) {
                        _lyricsScrapeState.pagesSkipped++;
                        await new Promise(r => setTimeout(r, _LYRICS_THROTTLE_MS));
                        continue;
...
app.post("/api/admin/lyrics/scrape/since-last", async (req, res) => {
    ...
    try {
        ...
        console.warn("[lyrics] read lyrics_scrape_last_at failed:", e);
    }
...
// longest match (more-specific). Returns null when nothing matches.
// Sharing this single algorithm between the popup and the worker
// means the worker's queue is consistent with what a curator would
// stage by hand.
function _ytAlbumAutoMatch(videoTitle, tracks) {
    const v = _ytNormTitleTS(videoTitle);
    if (!v || !Array.isArray(tracks) || !tracks.length)
        return null;
    let best = null;
    let bestScore = 0;
...
function _ytReviewScoreTitle(candTitle, artist, track) {
...
function _ytParseDiscogsDuration(dur) {
...
function _ytSplitTitleDecorations(videoTitle, artist) {
...
function _ytAutoApproveVerdict(args) {
    ...
    if (!wantArtist)
        return { auto: false, reason: "no_track_artist" };
    ...
    if (isTopic) {
        ...
        if (chanArtist !== wantArtist)
            return { auto: false, reason: `artist_mismatch: "${chanArtist}" vs "${wantArtist}"` };
        path = "topic";
    }
    else {
        if (!artistInTitle)
    ...
    if (marker)
        return { auto: false, reason: `version_marker: ${marker[0]}` };
    if (args.embeddable === false)
        return { auto: false, reason: "not_embeddable" };
    if (args.regionBlocked)
        return { auto: false, reason: "region_blocked" };
...
async function _ytReviewFetchVideoDetails(ids) {
    const out = new Map();
    const unique = Array.from(new Set(ids.filter(Boolean)));
    if (!unique.length || !_youtubeApiKey)
        return out;
...
async function _ytReviewAutoApproveEnabled() {
    try {
        return (await getAppSetting("yt_review_auto_approve")) !== "0";
    }
    catch {
        return true;
    }
...
async function _ytReviewSearch(q, opts = {}) {
    ...
    try {
        ...
        if (cached)
            return { ok: true, body: cached, cached: true };
    }
    catch { }
    if (!_youtubeApiKey)
    ...
    if (worker && _ytReviewSearchesToday >= _YT_REVIEW_DAILY_BUDGET)
        return { ok: false, reason: "worker_cap" };
    const params = [
        "part=snippet",
        "type=video",
        "maxResults=50",
    ...
    try {
        ...
        if (!resp.ok) {
            ...
            try {
                ...
                errSummary = j?.error?.errors?.[0]?.reason || j?.error?.message || txt.slice(0, 200);
            }
            catch { }
        ...
        if (body.items.length) {
            try {
                await setYoutubeSearchCache(cacheKey, body);
            }
            catch { }
        }
        ...
        console.warn(`[yt-review] search threw for q=${q}:`, msg);
        return { ok: false, reason: `throw: ${msg}` };
    }
}
async function _ytReviewExistingOverrideMap(masterId) {
    try {
...
async function _runYtReviewWorker() {
    if (_ytReviewRunning)
        return;
    _ytReviewRunning = true;
    _ytReviewStopRequested = false;
    await updateReviewState({ running: true, last_error: null, message: `Walking pre-${YT_REVIEW_YEAR_CUTOFF} Blues masters by year…` });
    ...
    try {
        while (!_ytReviewStopRequested) {
            ...
            const [existingOverrides, searchedPositions] = await Promise.all([
                _ytReviewExistingOverrideMap(masterId),
                getSearchedTrackPositions(masterId).catch(() => new Set()),
            ]);
            // Filter the tracklist to the tracks we still need to consider —
            ...
            for (const tr of tracklist) {
                ...
                if (!pos || !tTitle)
                    continue;
                if (existingOverrides.has(pos))
                    continue;
                if (searchedPositions.has(pos))
                    continue;
            ...
            if (!result.ok) {
                // Caps are re-checked at top of loop, so these should be rare
                // here — but surface the actual reason in last_error so the
                // admin can see WHY it stopped without digging into logs.
                if (result.reason === "project_cap" || result.reason === "worker_cap") {
                    await updateReviewState({ message: `Stopping: ${result.reason === "worker_cap" ? "worker" : "project"} daily cap reached.`, running: false });
                    break;
                }
            ...
            // matches by track position. Each (track, video) pair becomes
            ...
            for (const it of items) {
                ...
                if (bannedChannels.has(String(sn.channelId || "")))
                    continue;
            ...
            for (const tr of eligibleTracks) {
                ...
                for (const h of hits) {
                    ...
                    if (ins && insStatus !== "superseded") {
                        queuedThisMaster++;
                        await bumpReviewCounter("total_queued", 1);
                    }
                    if (takeAuto && ins) {
                ...
                // zero hits for it. clearEmptySearchedRows lets the admin
                // selectively trigger a retry later.
                await logTrackSearched(masterId, tr.position, hits.length, "album");
            }
            if (queuedThisMaster === 0)
...
function _ytReviewDailyTick() {
    (async () => {
        if (!(await _ytReviewDailyEnabled())) {
            console.log("[yt-review] daily auto-run disabled — skipping");
            return;
        }
        if (_ytReviewRunning) {
            console.log("[yt-review] daily auto-run: already running — skipping");
            return;
        }
...
app.post("/api/admin/yt-review/query-config", express.json({ limit: "4kb" }), async (req, res) => {
    ...
    try {
        ...
        if (errors.length) {
            res.status(400).json({ error: "invalid_config", details: errors });
            return;
        }
...
app.post("/api/admin/yt-review/query-test", express.json({ limit: "4kb" }), async (req, res) => {
    ...
    try {
        ...
        const mode = b.mode === "track" ? "track" : "album";
...
app.post("/api/admin/yt-review/channel-trust", express.json({ limit: "2kb" }), async (req, res) => {
    ...
    try {
        await setChannelTrust(channelId, channelTitle, state);
...
// GET /api/admin/yt-review/bans — the hard-ban list.
app.get("/api/admin/yt-review/bans", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        res.json({ rows: await listYoutubeChannelBans() });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    }
...
function _parseYtChannelId(input) {
...
// is unrecognisable.
async function _ytResolveChannelId(input) {
    const s = String(input || "").trim();
    if (!s)
        return null;
    ...
    let username = null; // legacy /user/NAME → forUsername
    let handle = null; // @handle / /c/NAME → forHandle
    ...
    if (mUser)
        ...
        handle = mBareHandle[1];
    else if (/^[A-Za-z0-9._-]+$/.test(s))
        handle = s; // bare name, no @ / URL
    if (!username && !handle)
        return null;
    if (!_youtubeApiKey)
        return null;
...
// Bans a channel: filtered from all results, pending queue rows from it
// superseded, auto-approved overrides from it removed.
app.post("/api/admin/yt-review/bans", express.json({ limit: "2kb" }), async (req, res) => {
    ...
    try {
        const out = await banYoutubeChannel(channelId, req.body?.channelTitle ?? null, req.body?.reason ?? null);
...
// DELETE /api/admin/yt-review/bans — body: { channelId }
app.delete("/api/admin/yt-review/bans", express.json({ limit: "1kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const channelId = String(req.body?.channelId ?? "").trim();
    if (!channelId) {
    ...
    try {
        const removed = await unbanYoutubeChannel(channelId);
        _invalidateBannedChannelCache();
        res.json({ ok: removed });
    }
    catch (err) {
...
// Resync the app's daily quota counters to zero. Use when Google Console
// shows the "Search Queries per day" quota has headroom but the app's
// counter drifted high (e.g. it carried searches across a Pacific-
// midnight reset). Zeroes both in-memory and persisted counters so the
// worker can run again. Refuses while the worker is running so it can't
// race the live counter.
app.post("/api/admin/yt-review/quota/reset", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    if (_ytReviewRunning) {
        res.status(409).json({ error: "stop the worker first" });
        return;
    }
    try {
        _ytQuotaUnitsToday = 0;
        _ytReviewSearchesToday = 0;
        _ytQuotaDay = _ytQuotaDayLA();
        await resetReviewQuota();
        res.json({ ok: true });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.post("/api/admin/yt-review/reset-cursor", express.json({ limit: "1kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    if (_ytReviewRunning) {
        res.status(409).json({ error: "cannot reset while worker is running — stop it first" });
        return;
    }
...
// POST /api/admin/yt-review/dismiss-pending — throw out the whole pending
// queue in favor of a fresh search: deletes pending candidates, clears the
// search log for those tracks, and rewinds the cursor so the next Start
// re-searches them with the current query. Refuses while running.
app.post("/api/admin/yt-review/dismiss-pending", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    if (_ytReviewRunning) {
        res.status(409).json({ error: "cannot dismiss while worker is running — stop it first" });
        return;
    }
    try {
        const out = await reviewQueueDismissPendingForResearch();
        await updateReviewState({
            cursor_year: null,
            cursor_master_id: null,
            cursor_track_pos: null,
        ...
        console.error("[yt-review dismiss-pending]", err);
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
...
// have no YouTube source (no Discogs video[] AND no pin), overall and for
// strict-Blues masters. Heavy (expands every master's tracklist); called
// on demand from the admin YT-review tab.
app.get("/api/admin/yt-review/coverage", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const s = await getYtCoverageStats();
        res.json(s);
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.post("/api/admin/yt-review/rescan-empties", express.json({ limit: "2kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const masterId = req.body?.masterId != null ? Number(req.body.masterId) : null;
    try {
        const n = await clearEmptySearchedRows(Number.isFinite(masterId) ? masterId : null);
...
async function _ytReviewDecideAndPin(id, action, adminUserId) {
    ...
    if (action === "approve" && out.videoId && out.masterId && out.trackPosition) {
        try {
          ...
          WHERE id = $1 AND status = 'approved'`, [id]).catch(() => { });
            throw pinErr;
        }
    }
    return true;
...
app.post("/api/admin/yt-review/decide", express.json({ limit: "4kb" }), async (req, res) => {
    ...
    try {
        if (!await _ytReviewDecideAndPin(id, action, adminUserId)) {
...
app.post("/api/admin/yt-review/approve-top", async (req, res) => {
    ...
    if (!adminUserId)
        return;
    if (_ytApproveTopBusy) {
        res.status(409).json({ error: "a batch is already running" });
        return;
    }
    const limit = Math.max(1, Math.min(500, parseInt(String(req.query.limit ?? "200"), 10) || 200));
    _ytApproveTopBusy = true;
    try {
        const r = await getPool().query(`SELECT id, master_id, track_position, candidate_channel_title, candidate_title, candidate_description
        ...
        for (const row of r.rows) {
            ...
            if (!cur || (pref && !cur.pref))
                top.set(key, { id: Number(row.id), pref });
        }
        const picks = [...top.values()].slice(0, limit);
        let approved = 0, failed = 0;
        for (const { id } of picks) {
            try {
                if (await _ytReviewDecideAndPin(id, "approve", adminUserId))
                    approved++;
                else
                    failed++;
            }
                ...
                failed++;
                console.warn(`[yt-review approve-top] id=${id}: ${e?.message ?? e}`);
            }
        }
...
// for one (master, track). One atomic statement instead of N per-row
// decides, so the "none of these are right" case is a single click and a
// single round trip.
// POST /api/admin/yt-review/unreject { id } — move a rejected candidate
// back to pending so it can be approved after all.
app.post("/api/admin/yt-review/unreject", express.json({ limit: "1kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const id = parseInt(String(req.body?.id ?? ""), 10);
    if (!Number.isFinite(id)) {
    ...
    try {
        if (!await reviewQueueUnreject(id)) {
...
app.post("/api/admin/yt-review/reject-track", express.json({ limit: "1kb" }), async (req, res) => {
    const adminUserId = await requireAdmin(req, res);
    if (!adminUserId)
        return;
    const masterId = parseInt(String(req.body?.masterId ?? ""), 10);
    // A track position can legitimately be empty; only the master id is required.
    const trackPosition = String(req.body?.trackPosition ?? "");
    if (!Number.isFinite(masterId)) {
        res.status(400).json({ error: "bad_request" });
        return;
    }
    try {
        const rejected = await reviewQueueRejectTrack(masterId, trackPosition, adminUserId);
...
app.post("/api/admin/yt-review/apply-trust", express.json({ limit: "1kb" }), async (req, res) => {
    ...
    try {
        ...
        for (const r of pending) {
            if (!banned.has(String(r.candidate_channel_id || "")))
                continue;
        ...
        const decidedTracks = new Set();
        let approved = 0;
        for (const r of eligible) {
            const key = `${r.master_id}:${r.track_position}`;
            if (decidedTracks.has(key))
                continue;
...
app.post("/api/admin/yt-review/custom-approve", express.json({ limit: "2kb" }), async (req, res) => {
    ...
    if (!videoId) {
        res.status(400).json({ error: "could_not_parse_video_id", detail: `Not a recognized YouTube URL or 11-char ID: ${url}` });
        return;
    }
    try {
        const pool = getPool();
        ...
        // decision.
        const rowR = await pool.query(`SELECT master_id, track_position, track_title, status
         FROM track_yt_review_queue WHERE id = $1`, [id]);
        ...
        await pool.query(`UPDATE track_yt_review_queue
          SET status                  = 'approved',
              reviewed_at             = NOW(),
              reviewed_by             = $2,
              ...
              title_score             = NULL
        WHERE id = $1`, [id, adminUserId, videoId, `https://i.ytimg.com/vi/${videoId}/mqdefault.jpg`]);
...
app.get("/api/admin/yt-review/errors", async (req, res) => {
    ...
    try {
        ...
        const rows = await listReviewErrors(limit);
        res.json({ rows });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
app.post("/api/admin/yt-review/delete-approval", express.json({ limit: "4kb" }), async (req, res) => {
    const adminUserId = await requireAdmin(req, res);
    if (!adminUserId)
        return;
    const id = parseInt(String(req.body?.id ?? ""), 10);
    ...
    try {
        const out = await reviewQueueDeleteApproval(id, adminUserId);
        if (!out.ok) {
            res.status(404).json({ error: "not_found_or_not_approved" });
            return;
        }
        res.json({ ok: true, masterId: out.masterId, trackPosition: out.trackPosition });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    }
});
// GET /api/admin/yt-review/channel-profiles?ids=UC..,UC..
// Profile (avatar, handle, subs, video count, country, description) per
...
async function _ytFetchChannelProfiles(ids) {
    ...
    for (let i = 0; i < missing.length; i += 50) {
        ...
        try {
            ...
            const resp = await fetch(url);
            if (!resp.ok) {
                console.warn(`[yt-review] channels.list failed: HTTP ${resp.status}`);
                continue;
            }
            const body = await resp.json();
            _bumpYtQuotaPersisted(0, 1);
            ...
            console.warn("[yt-review] channels.list threw:", e?.message ?? e);
        }
    }
    if (fresh.length)
        await upsertChannelProfiles(fresh).catch(e => console.warn("[yt-review] profile cache write failed:", e?.message ?? e));
    return cached;
}
...
async function _ytAiHuntConfig() {
    try {
        const raw = await getAppSetting("yt_ai_hunt_config");
        const p = raw ? JSON.parse(raw) : {};
        const queries = Array.isArray(p.queries)
            ? p.queries.map((q) => String(q).trim().slice(0, 120)).filter(Boolean).slice(0, 50)
        ...
        return { ..._YT_AI_HUNT_DEFAULTS };
    }
}
async function _ytAiHuntState() {
    try {
        return JSON.parse((await getAppSetting("yt_ai_hunt_state")) || "{}") || {};
    }
    catch {
        return {};
    }
...
async function _ytAiHuntSearch(q, opts = {}) {
    if (!_youtubeApiKey)
        return { ok: false, reason: "no_api_key" };
    _ytQuotaMaybeReset();
    if (_ytQuotaUnitsToday + 100 > _YT_DAILY_SOFT_CAP_UNITS)
...
async function _ytAiAnalyze(videos, source, withDetails) {
    ...
    const profiles = await _ytFetchChannelProfiles(first.map(f => f.channelId)).catch(() => new Map());
...
async function _runYtAiHunt(trigger) {
    if (_ytAiHuntRunning)
        return;
    _ytAiHuntRunning = true;
    const summary = { at: new Date().toISOString(), trigger, searches: 0, flagged: 0, added: 0, error: null };
    try {
        ...
        summary.flagged += q.flagged;
        summary.added += q.added;
...
function initYtAiHuntSchedule() {
    const arm = () => {
        const wait = _ytAiHuntMsUntilRun();
        setTimeout(() => {
            (async () => {
                if (!(await _ytAiHuntConfig()).daily) {
                    console.log("[yt-ai-hunt] daily run disabled — skipping");
                    return;
                }
...
app.get("/api/admin/yt-ai/candidates", async (req, res) => {
    ...
    try {
        res.json({ rows: await listAiChannelCandidates(status, 300) });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    }
...
app.post("/api/admin/yt-ai/config", express.json({ limit: "8kb" }), async (req, res) => {
    ...
    const b = req.body || {};
    const next = {
        queries: Array.isArray(b.queries) ? b.queries.map((q) => String(q).trim().slice(0, 120)).filter(Boolean).slice(0, 50) : cur.queries,
    ...
    if (!next.queries.length)
        next.queries = _YT_AI_HUNT_DEFAULTS.queries;
    try {
...
app.post("/api/admin/yt-ai/search", express.json({ limit: "1kb" }), async (req, res) => {
    ...
    try {
        ...
        if (!r.ok) {
            res.status(502).json({ error: r.reason });
            return;
        }
...
// POST /api/admin/yt-review/confirm-auto — body: { ids: number[] }
// Keep auto-pins: they move from the Auto-pinned view to Approved.
app.post("/api/admin/yt-review/confirm-auto", express.json({ limit: "8kb" }), async (req, res) => {
    const adminUserId = await requireAdmin(req, res);
    if (!adminUserId)
        return;
    ...
    try {
        const confirmed = await reviewQueueConfirmAuto(ids, adminUserId);
...
app.get("/api/admin/lyrics", async (req, res) => {
    ...
    try {
        ...
        const tuningLike = String(req.query.tuningLike ?? "").trim().slice(0, 80);
        const artist = String(req.query.artist ?? "").trim().slice(0, 200);
        ...
        const result = await listLyrics({
            ...
            tuningLike: tuningLike || undefined,
            artist: artist || undefined,
...
let _bluesLyricYearJob = { status: "idle" };
...
// POST /api/admin/lyrics/resolve-years-cheap
// Sweep blues_lyrics and set first_release_year from each linked
// artist's discogs_releases JSONB where the release title matches the
// lyric title. Zero Discogs API calls — runs in a few seconds against
// the entire table. Returns { updated, stillMissing } so the UI can
// report exactly how many rows landed a year and how many still need
...
// ── Bulk tuning editor ────────────────────────────────────────────
// GET /api/admin/lyrics/matching-ids returns ONLY the ids that match
// the current listing filter — used by the bulk editor's "select all
// matching" link so the curator can apply one change to every hit
// without paginating through them. Same filter shape as the listing
// endpoint; capped at 10k ids per call.
app.get("/api/admin/lyrics/matching-ids", async (req, res) => {
    const adminUserId = await requireAdmin(req, res);
    if (!adminUserId)
        return;
    try {
        ...
        const favorites = req.query.favorites === "1" || req.query.favorites === "true";
        const titlePunct = req.query.titlePunct === "1" || req.query.titlePunct === "true";
        const out = await listLyricIdsMatching({
            search: q || undefined,
            tuning: tuning || undefined,
            tuningLike: tuningLike || undefined,
            artist: artist || undefined,
...
// POST /api/admin/lyrics/bulk-delete  body: { ids: number[] }
// Hard-delete every named lyric in one round-trip. Capped at 10k ids
// per call. Used by the bulk action bar's "Delete selected" button —
// confirmation lives on the client; the server only verifies admin.
app.post("/api/admin/lyrics/bulk-delete", express.json({ limit: "256kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const body = req.body ?? {};
        ...
        const deleted = await bulkDeleteLyrics(idsRaw);
        res.json({ deleted });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
...
// POST /api/admin/lyrics/bulk-update-tuning
//   body: { ids: number[], tuning: string | null }
// Sets `tuning` on every named lyric in one round-trip. Empty / null
// tuning clears the column ("(unspecified)" in the filter dropdown).
// Capped at 10k ids per call to mirror matching-ids.
app.post("/api/admin/lyrics/bulk-update-tuning", express.json({ limit: "256kb" }), async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const body = req.body ?? {};
        ...
        const tuningRaw = body.tuning;
        const tuning = tuningRaw == null || tuningRaw === ""
            ? null
            : String(tuningRaw).trim().slice(0, 80) || null;
        const updated = await bulkUpdateLyricTuning(idsRaw, tuning);
        res.json({ updated });
    }
    catch (err) {
        res.status(500).json({ error: err?.message ?? String(err) });
    ...
    try {
        ...
        if (trackTitles.length) {
            ...
            WHERE ($1::bigint IS NOT NULL AND discogs_release_id = $1::bigint)
               OR ($2::bigint IS NOT NULL AND discogs_master_id  = $2::bigint)`, [Number.isFinite(releaseId) && releaseId > 0 ? releaseId : null,
                ...
                const normTitle = (s) => String(s || "")
                    ...
                    .replace(/\s+/g, " ")
                    .trim();
                const submittedNormToLc = trackTitles.map(t => ({
            ...
            WHERE LOWER(TRIM(page_title)) = ANY($1::text[])
              AND artist_id = ANY($2::int[])`, [titlesLc, archiveArtistIds]);
                for (const row of r.rows) {
                    ...
                    if (!out.tracks[k])
                        out.tracks[k] = { id: row.id, page_title: row.page_title, artist: row.artist, artist_id: row.artist_id, first_release_year: row.first_release_year };
                }
            }
            ...
            if (isAdmin && artistNames.length) {
                ...
                const r = await getPool().query(`SELECT id, artist, title, position, pitch, notes
             FROM blues_tunings_grid
            WHERE LOWER(TRIM(title))  = ANY($1::text[])
              AND LOWER(TRIM(artist)) = ANY($2::text[])`, [titlesLc, namesLc]);
                for (const row of r.rows) {
...
app.get("/api/lyrics/:id/public", async (req, res) => {
    try {
        ...
        const r = await getPool().query(`SELECT id, page_title, artist, tuning, plaintext, first_release_year
         FROM blues_lyrics WHERE id = $1`, [id]);
        const row = r.rows[0];
        if (!row) {
            res.status(404).json({ error: "not_found" });
            return;
...
app.get("/api/wikipedia/search", async (req, res) => {
    ...
    try {
        ...
        if (!r.ok) {
            console.error("[wikipedia/search] HTTP", r.status);
            res.status(502).json({ error: "Wikipedia search failed", status: r.status });
            return;
        }
        const data = await r.json();
...
app.get("/api/wikipedia/lookup", async (req, res) => {
    ...
    try {
        ...
        if (!sr.ok) {
            console.error("[wikipedia] opensearch HTTP", sr.status);
            res.status(502).json({ error: "Wikipedia search failed", status: sr.status });
            return;
        }
...
app.get("/search", async (req, res) => {
    ...
    const q = rawQ;
    const searchArtist = artistSuffix ? (artistBase || undefined) : (artist || undefined);
    ...
    try {
        ...
        const requestedPerPage = req.query.per_page ? parseInt(req.query.per_page) : 12;
        const upstreamPerPage = artistSuffix ? Math.max(requestedPerPage, 100) : requestedPerPage;
        // Cache key = the full upstream param set. Deliberately excludes the
        // user: a /search response depends only on the query, so sharing it
        // is safe and means one person's search warms it for everyone.
        const cacheKey = JSON.stringify([
            q, req.query.type ?? "", searchArtist ?? "", searchRelease ?? "", searchLabel ?? "",
            req.query.year ?? "", req.query.genre ?? "", req.query.style ?? "", req.query.format ?? "",
            req.query.country ?? "", searchBarcode ?? "", req.query.sort ?? "", req.query.sort_order ?? "",
            req.query.page ?? 1, upstreamPerPage,
        ]);
        const cached = await getDiscogsSearchCache(cacheKey, _DISCOGS_SEARCH_TTL_MS / 1000);
        if (cached) {
            res.setHeader("X-SeaDisco-Cache", "hit");
            // Cached album JSON changes rarely; let the browser reuse it briefly
            // instead of refetching on every popup open.
        ...
        // include the field.
        if (searchBarcode && results?.results?.length) {
            const r = results;
        ...
        // Cache the POST-filter payload — the key already carries the
        // artist disambiguator and barcode, so the filtered result is
        // deterministic for this key.
        setDiscogsSearchCache(cacheKey, results).catch(() => { });
        res.setHeader("X-SeaDisco-Cache", "miss");
        res.json(results);
    }
    catch (err) {
...
app.get("/release/:id", async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (_wantsHtml(req)) {
        await _sendEntityPage(req, res, "release", id);
        return;
    }
    res.setHeader("Vary", "Accept");
...
app.get("/master/:id", async (req, res) => {
    const id = parseInt(req.params.id, 10);
    if (_wantsHtml(req)) {
        await _sendEntityPage(req, res, "master", id);
        return;
    }
    res.setHeader("Vary", "Accept");
    const noCache = req.query.nocache === "1";
...
app.get("/master-versions/:id", async (req, res) => {
    ...
    try {
        ...
        const fetchPage = async (p) => {
            try {
                return await dc.getMasterVersions(id, { page: p, perPage: PER_PAGE, sort: "released", sortOrder: "asc" });
            }
            catch (e) {
        ...
        let firstPage = null;
        try {
            firstPage = await dc.getMasterVersions(id, { page: 1, perPage: PER_PAGE, sort: "released", sortOrder: "asc" });
        }
        catch (e) {
            const msg = String(e?.message ?? e);
...
app.get("/series-releases/:id", async (req, res) => {
    ...
    try {
        ...
        const [labelData, relData] = await Promise.all([
            dc.getLabel(id),
            dc.getLabelReleases(id, { perPage: 100 }),
        ]);
        const seriesName = labelData?.name ?? `Series ${id}`;
        const releases = (relData?.releases ?? []).map((r) => ({
            id: r.id,
            title: r.title ?? "",
            artist: r.artist ?? "",
            catno: r.catno ?? "",
...
app.get("/api/admin/overview", async (req, res) => {
    ...
    try {
        ...
        // Account cap + retention so the UI doesn't hardcode them.
        o.maxUsers = MAX_USERS;
        o.unconnectedDeleteDays = UNCONNECTED_DELETE_DAYS;
        ...
        // broader signup figure and what the cap counts. Null if Clerk is
        // unreachable.
        o.clerkUsers = await getClerkAccountCount(0);
        res.json(o);
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
...
// GET /api/year-label/years  ?genre=&strict=0|1[&label=NAME]
// GET /api/year-label/labels ?genre=&strict=0|1[&year=YYYY]
// GET /api/year-label/albums ?year=YYYY&label=NAME + filters
// Browsable either way round (year → label or label → year). Years and
// labels are both sliced from one memoized (year, label, count) scan per
// filter setting; albums are queried per year+label. Admin-only unless
// YEAR_LABEL_OPEN=1 (then any signed-in user, rate limited).
const YEAR_LABEL_GENRES = [
...
const _yearLabelUserLimiter = new PerIpRateLimiter(300, 10 * 60_000, "year-label-user");
async function _ylGate(req, res) {
    const userId = await getClerkUserId(req);
    const isAdmin = !!userId && !!ADMIN_CLERK_ID && userId === ADMIN_CLERK_ID;
    if (isAdmin)
        return true;
    if (!_yearLabelOpen || !userId) {
        res.status(userId ? 403 : 401).json({ error: "Not available" });
        return false;
    }
    if (!_yearLabelUserLimiter.check(userId)) {
        res.status(429).json({ error: "Too many requests; try again in a few minutes" });
        return false;
    }
    return true;
}
function _ylFilter(q) {
    const g = String(q.genre ?? "");
    return {
...
// (year, label, n) pairs per filter setting: 30 min TTL, at most 4 settings
// held (each can be tens of thousands of rows), concurrent requests for the
// same setting share one scan.
const _ylPairsMemo = new Map();
const _ylPairsInflight = new Map();
async function _ylPairs(f, refresh) {
    const key = _ylKey(f);
    const hit = _ylPairsMemo.get(key);
    if (hit && !refresh && Date.now() - hit.at < 30 * 60_000)
        return hit.pairs;
    let p = _ylPairsInflight.get(key);
    if (!p) {
        p = getYearLabelPairs(f).finally(() => _ylPairsInflight.delete(key));
        _ylPairsInflight.set(key, p);
    }
    const pairs = await p;
    _ylPairsMemo.delete(key);
    _ylPairsMemo.set(key, { at: Date.now(), pairs });
    while (_ylPairsMemo.size > 4)
        _ylPairsMemo.delete(_ylPairsMemo.keys().next().value);
    return pairs;
}
const _ylAlbumsMemo = new Map();
app.get("/api/year-label/years", async (req, res) => {
    if (!await _ylGate(req, res))
        return;
    const f = _ylFilter(req.query);
    const hasLabel = typeof req.query.label === "string";
    const label = hasLabel ? String(req.query.label).slice(0, 300) : "";
    try {
        const counts = new Map();
        for (const [y, l, n] of await _ylPairs(f, req.query.refresh === "1")) {
            if (hasLabel && l !== label)
                continue;
            counts.set(y, (counts.get(y) ?? 0) + n);
        }
        const years = [...counts].sort((a, b) => a[0] - b[0]).map(([year, n]) => ({ year, n }));
        res.set("Cache-Control", "no-store");
        res.json({ genres: YEAR_LABEL_GENRES, years });
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
    }
});
app.get("/api/year-label/labels", async (req, res) => {
    if (!await _ylGate(req, res))
        return;
    const f = _ylFilter(req.query);
    const year = req.query.year == null ? null : _ylYear(req.query.year);
    if (req.query.year != null && year == null) {
    ...
    try {
        ...
        const labels = [...counts].map(([label, n]) => ({ label, n }));
        res.set("Cache-Control", "no-store");
        res.json({ genres: YEAR_LABEL_GENRES, year, labels });
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
    }
});
app.get("/api/year-label/albums", async (req, res) => {
    if (!await _ylGate(req, res))
        return;
    const f = _ylFilter(req.query);
    const year = _ylYear(req.query.year);
    const label = String(req.query.label ?? "").slice(0, 300);
    if (year == null) {
        res.status(400).json({ error: "year required" });
        return;
    }
    try {
        const key = `${_ylKey(f)}|${year}|${label}`;
        const hit = _ylAlbumsMemo.get(key);
        let v = hit && req.query.refresh !== "1" && Date.now() - hit.at < 5 * 60_000 ? hit.v : null;
        if (!v) {
            v = await getYearLabelAlbums(f, year, label);
            _ylAlbumsMemo.delete(key);
            _ylAlbumsMemo.set(key, { at: Date.now(), v });
            while (_ylAlbumsMemo.size > 200)
                _ylAlbumsMemo.delete(_ylAlbumsMemo.keys().next().value);
        }
        res.set("Cache-Control", "no-store");
        res.json({ year, label, ...v });
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
    }
...
app.get("/api/admin/db-stats", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    try {
        const [tables, dbBytes] = await Promise.all([getTableRowCounts(), getDatabaseSize().catch(() => 0)]);
        const totalRows = tables.reduce((sum, t) => sum + (t.rows > 0 ? t.rows : 0), 0);
        res.json({ tables, totalRows, dbBytes, approximate: true });
    }
    catch (err) {
        res.status(500).json({ error: String(err) });
    }
...
function startDailySyncSchedule() {
    ...
    async function runMaintenance() {
        try {
            ...
            // Connected accounts are kept; accounts that signed up but NEVER
            // connected Discogs are deleted once idle UNCONNECTED_DELETE_DAYS, which
            // frees their spot under the MAX_USERS account cap.
    ...
    // UNCONNECTED_DELETE_DAYS+ days. Non-connected accounts have no user_tokens row and no synced
    ...
    async function deleteIdleUnconnectedUsers() {
        ...
        const cutoff = Date.now() - UNCONNECTED_DELETE_DAYS * 86400000;
        ...
        if (deleted)
            _clerkCountMemo.at = 0; // spots freed — refresh the cap check
        return deleted;
    }
...
async function syncUserExtras(userId, username, client) {
    ...
    try {
        ...
        for (const list of userLists) {
            try {
                ...
                if (!lr.ok) {
                    console.log(`[extras] ${username}: list ${list.id} items fetch ${lr.status}`);
                    continue;
                }
                const listData = await lr.json();
                const listItems = listData.items ?? [];
                if (listItems.length) {
                    const parsed = listItems.map((item) => ({
                        discogsId: item.id,
    ...
    try {
        for (let page = 1;; page++) {
            await sleep(1200);
            const r = await extrasFetch(`https://api.discogs.com/marketplace/orders?per_page=100&page=${page}&sort=last_activity&sort_order=desc`);
            if (r.status === 401 || r.status === 403 || r.status === 404) {
...
app.listen(PORT, "0.0.0.0", async () => {
    ...
    if (process.env.APP_DB_URL) {
        ...
        // Watchdog: the boot reset only fires on a fresh process start, but a
        // sync can be orphaned WITHOUT a restart — e.g. the container is idled
        // overnight, freezing the run's timers/coroutine so neither the stall
        // guard nor the catch block ever writes a terminal status. This sweep
        // clears any row still 'syncing' whose user has no live run in this
        // process (single-instance assumption; see resetOrphanedSyncingStatuses).
        setInterval(async () => {
            try {
                const freed = await resetOrphanedSyncingStatuses(Array.from(_syncRunningFor));
                if (freed.length)
                    console.log(`Sync watchdog: reset ${freed.length} orphaned syncing row(s): ${freed.join(", ")}`);
            }
            catch (e) {
                console.error("Sync watchdog error:", e);
            }
        }, 5 * 60 * 1000).unref();
        ...
        // Manual cache-warm worker — initialise with admin Clerk id so it
        // can build Discogs OAuth clients on demand. No cron anymore;
        // runs are kicked manually via /api/admin/cache-warm/start.
        try {
            initCacheWarmModule(ADMIN_CLERK_ID);
        }
        catch (e) {
            console.error("[startup] cache-warm init failed:", e);
        }
        try {
            initCacheWarmCatnoModule(ADMIN_CLERK_ID);
        }
        catch (e) {
        ...
        // Rehydrate today's YT quota counters from the DB so a redeploy
        // doesn't make us forget how much we've already spent. UTC date
        // rollover inside getReviewQuotaToday zeroes the row if it's a new
        // day; otherwise we restore the running totals into the in-memory
        // gate variables.
        try {
            const q = await getReviewQuotaToday();
            _ytReviewSearchesToday = q.workerSearches;
            _ytQuotaUnitsToday = q.projectUnits;
            if (q.workerSearches || q.projectUnits) {
                console.log(`[startup] yt-quota rehydrated: worker=${q.workerSearches} project=${q.projectUnits}u`);
            }
        }
        catch (e) {
            console.error("[startup] yt-quota rehydrate failed:", e);
        }
        // If the YT review worker was running when the previous process
        // died (Railway redeploy, crash, etc.), resume it. The persisted
        // `running` flag in track_yt_review_state is our only signal —
        // _ytReviewRunning is per-process and always false at boot.
        setTimeout(async () => {
            try {
                const state = await getReviewState();
                if (state?.running) {
                    console.log("[startup] yt-review worker was running at last shutdown — resuming");
                    _runYtReviewWorker().catch((e) => console.error("[yt-review] resume threw:", e));
                }
            }
            catch (e) {
                console.error("[startup] yt-review resume check failed:", e);
            }
        }, 8000);
        // Catalog-tab pool refresher: rebuilds each mode's pool of ~500
        // candidates every 2h. First tick fires 60s after boot so the
        // initial DB schema is settled. Errors are logged and absorbed
        // so a single mode failure doesn't take down the cycle.
        const REFRESH_INTERVAL_MS = 2 * 60 * 60 * 1000;
        const FEED_POOL_MODES = ["feed", "rare", "dig", "active", "played"];
        const tick = async () => {
            for (const mode of FEED_POOL_MODES) {
                try {
                    await refreshFeedPool(mode);
                }
                catch (e) {
                    console.error(`[feed-pool refresh ${mode}]`, e);
                }
            }
        };
        setTimeout(() => { tick().catch(() => { }); }, 60_000);
        setInterval(() => { tick().catch(() => { }); }, REFRESH_INTERVAL_MS);
    }
});
