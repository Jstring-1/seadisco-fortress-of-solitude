...
// GET /api/admin/blues-picker?strict=1 — admin home-strip "Blues" tab.
// Cached Masters+ Blues albums 1920–1960 with a YouTube video on every
// track (see getFullyCoveredBluesAlbums). Memoized 10 min per mode since
// it scans every early-Blues cache row; ?refresh=1 recomputes.
const _bluesPickerMemo = new Map();
app.get("/api/admin/blues-picker", async (req, res) => {
    if (!await requireAdmin(req, res))
        return;
    const strict = req.query.strict === "1";
    const key = strict ? "strict" : "loose";
    try {
        const hit = _bluesPickerMemo.get(key);
        if (!hit || req.query.refresh === "1" || Date.now() - hit.at > 10 * 60_000) {
            _bluesPickerMemo.set(key, { at: Date.now(), albums: await getFullyCoveredBluesAlbums(strict) });
        }
        const m = _bluesPickerMemo.get(key);
        res.set("Cache-Control", "no-store");
        res.json({ strict, computedAt: new Date(m.at).toISOString(), albums: m.albums });
    }
    catch (e) {
        res.status(500).json({ error: String(e?.message ?? e) });
    }
