...
function _ytAutoApproveVerdict(args) {
    ...
    if (!_ytIsTopicChannel(channelTitle))
        return { auto: false, reason: "not_topic_channel" };
    ...
    if (args.trackDurationSeconds == null)
        return { auto: false, reason: "no_discogs_duration" };
    if (args.videoDurationSeconds == null)
        return { auto: false, reason: "no_video_duration" };
        ...
        while (!_ytReviewStopRequested) {
            ...
            for (const hits of matchesByTrack.values()) {
                for (const h of hits) {
                    if (!autoEnabled)
                        continue;
                    if (_ytIsTopicChannel(String(h.sn.channelTitle || "")))
