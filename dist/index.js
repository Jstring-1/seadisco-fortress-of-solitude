...
// ── Release ───────────────────────────────────────────────────────────────────
server.tool("get_release", "Get full details for a specific Discogs release by its release ID.", {
    release_id: z
...
// ── Master Release ────────────────────────────────────────────────────────────
server.tool("get_master_release", "Get details for a master release (represents the definitive version of a release). Includes all known versions/pressings.", {
    master_id: z
        .union([z.number().int(), z.string()])
...
// ── Artist ────────────────────────────────────────────────────────────────────
server.tool("get_artist", "Get details for a Discogs artist by their artist ID.", {
    artist_id: z
        .union([z.number().int(), z.string()])
...
// ── Label ─────────────────────────────────────────────────────────────────────
server.tool("get_label", "Get details for a record label by its Discogs label ID.", {
    label_id: z
        .union([z.number().int(), z.string()])
...
server.tool("get_marketplace_stats", "Get current Discogs marketplace statistics for a release: lowest price, median price, number currently for sale, and blocked-from-sale status.", {
    ...
    currency: z
        .string()
        .optional()
        .describe("Currency code for prices, e.g. 'USD', 'EUR', 'GBP'. Defaults to your account currency."),
}, async ({ release_id, currency }) => {
    const result = await discogs.getMarketplaceStats(release_id, currency);
    return { content: [{ type: "text", text: JSON.stringify(result, null, 2) }] };
});
server.tool("get_price_suggestions", "Get Discogs suggested price ranges for each vinyl/media condition (Mint, Near Mint, Very Good Plus, etc.) for a release, based on recent sales history.", {
    release_id: z
        .union([z.number().int(), z.string()])
        .describe("Discogs release ID"),
}, async ({ release_id }) => {
