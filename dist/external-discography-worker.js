...
let _totalSeeds = 0;
let _startedAt = null;
const _recentInserted = [];
const _recentErrors = [];
function _sleep(ms) {
    return new Promise(r => setTimeout(r, ms));
}
function _seedPath(source) {
    ...
    return path.join(process.cwd(), "scripts", source === "wirz" ? "wirz-labels.json" : "abrams-labels.json");
}
function _loadSeeds(source) {
...
function _dashToNull(v) {
    if (!v || v === "-" || v === "?" || v === "n/a")
        return null;
    return v;
}
function _parseYearFromAbramsDate(dateStr) {
    if (!dateStr)
        return null;
...
function parseWirzLabel(html, seed) {
...
function parseAbramsLabel(html, _seed) {
    const trRe = /<tr>([\s\S]*?)<\/tr>/gi;
    const sideCounter = new Map();
    const nextSide = (catno) => {
    ...
    while ((trMatch = trRe.exec(html)) !== null) {
        ...
        if (cells.length !== 8)
            continue;
        if (/^(label|cat#?|catalog)\b:?$/i.test(cells[0]))
            continue;
        const catno = cells[0];
        if (!/^\d/.test(catno))
            continue;
...
// ── Public API ────────────────────────────────────────────────
export function initExternalDiscographyWorkerModule() {
    ...
    setTimeout(async () => {
        ...
        try {
            ...
            startExternalDiscographyRun(state.source, { resumeFromIndex: state.cursor ?? 0 }).catch(err => console.error("[external-disc-worker] boot-resume failed:", err));
        }
        catch (err) {
...
export function isExternalDiscographyRunning() {
    return _running;
}
export function requestExternalDiscographyStop() {
    if (_running)
        _stopRequested = true;
}
export function getExternalDiscographyStatus() {
    return {
        running: _running,
...
export async function startExternalDiscographyRun(source, opts = {}) {
    if (_running)
        return { ok: false, error: `Already running: ${_currentSource}` };
...
function _recordInserted(source, label, rows) {
    _recentInserted.unshift({ source, label, rows, at: new Date() });
    if (_recentInserted.length > 30)
        _recentInserted.length = 30;
}
function _recordError(source, slug, msg) {
    _recentErrors.unshift({ source, slug, msg: msg.slice(0, 200), at: new Date() });
    if (_recentErrors.length > 30)
        _recentErrors.length = 30;
...
export function parseExcelloXlsxBuffer(buf) {
    ...
    if (!shXml)
        throw new Error("xl/worksheets/sheet1.xml missing");
    const strings = [];
    ...
    while ((rm = rowRe.exec(shXml)) !== null) {
        ...
        while ((cm = cellRe.exec(rowXml)) !== null) {
            ...
            if (inlineVMatch)
                ...
                val = strings[Number(vMatch[1])];
            else if (vMatch)
                val = vMatch[1];
            cells[col] = val;
        }
        const a = cells.A;
        if (!a || !/^\d+/.test(String(a)))
