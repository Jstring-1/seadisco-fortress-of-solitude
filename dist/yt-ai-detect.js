...
export function scoreAiChannels(videos, profiles = new Map()) {
    ...
    for (const [channelId, vids] of byChannel) {
        ...
        const best = new Map();
        const bump = (label, pts) => { if ((best.get(label) ?? 0) < pts)
            best.set(label, pts); };
        for (const l of aiStrongLabels(channelTitle))
            bump(l, 4);
        for (const v of vids) {
            for (const l of aiStrongLabels(v.title))
                bump(l, 3);
            for (const l of aiStrongLabels(v.description))
                bump(l, 2);
        }
        for (const l of aiStrongLabels(prof?.description ?? ""))
            bump(l, 2);
        if (vids.some(v => v.synthetic === true))
            bump("YouTube “altered or synthetic” label", 5);
        // The bare word only counts where no stronger AI wording already matched.
        const weak = (t) => WEAK_AI.test(t) && aiStrongLabels(t).length === 0;
        if (weak(channelTitle))
            bump("“AI” in the channel name", 2);
        else if (vids.some(v => weak(v.title)))
            bump("“AI” in a title", 1);
        ...
        const flagged = (v) => v.synthetic === true || aiStrongLabels(`${v.title} ${v.description}`).length > 0 || WEAK_AI.test(v.title);
