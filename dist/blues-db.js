...
//
// Phase 2 (enrich): fill first/last recording year + title, aliases, bio
// data from Wikipedia, Discogs, and YouTube for seeded rows.
//
// Wikidata SPARQL endpoint:   https://query.wikidata.org/sparql
// Required header:            User-Agent (per Wikimedia policy)
import { upsertBluesArtistByQid, upsertBluesArtistByDiscogsId, listBluesArtists, getBluesArtist, updateBluesArtist, getLyricsMissingFirstReleaseYear, getPool } from "./db.js";
...
SELECT DISTINCT
  ...
  ?image ?wikipediaArticle ?discogsId
WHERE {
...
function _joinPlace(city, state, country) {
    const c = (city ?? "").trim();
    const s = (state ?? "").trim();
    ...
    if (!c)
        return region;
    if (c.toLowerCase() === region.toLowerCase())
        return c;
    if (c.toLowerCase().includes(region.toLowerCase()))
        return c;
    return `${c}, ${region}`;
}
...
function _titleFromWikiSuffix(suffix) {
    if (!suffix)
        return null;
...
async function _fetchWikipediaIntro(title) {
    ...
    if (!r.ok)
        throw new Error(`Wikipedia ${r.status}`);
    const data = await r.json();
...
async function _enrichOneFromDiscogsArtist(client, row, opts = {}) {
    ...
    try {
        releasesPayload = await client.get(`/artists/${row.discogs_id}/releases`, { sort: "year", sort_order: "asc", per_page: "100" });
    }
    catch {
...
async function _searchDiscogsArtist(client, name) {
    ...
    const data = await client.get("/database/search", { q: name, type: "artist", per_page: "5" });
...
export async function enrichBluesFromDiscogs(client, opts = {}) {
    const start = Date.now();
    const errors = [];
    let attempted = 0, enriched = 0, skipped = 0;
    const PAGE = 200;
...
// ── Lyric first_release_year resolver: Discogs-search worker ─────────
// For every lyric still missing a year after the cheap pass, ask
// Discogs's /database/search for `q=<artist> <title>&type=release`
// (then again `type=master`) and take the MIN year from results whose
// title fuzzy-matches the lyric title.
// Rate-limited 1.1s/req. Background job state lives in
// _bluesLyricYearJob (in search-api.ts); progress callback fires every
// row so the status endpoint can stream counts.
        ...
        try {
            ...
            WHERE id = $2
              AND (first_release_year IS NULL OR first_release_source IN ('artist_releases','discogs_search'))`, [bestYear, row.id]);
                ...
                // Stamp checked_at so the next worker pass can skip recently-
                // tried rows. We don't have a "tried but no result" status yet
                // so we just leave the year NULL — re-running won't waste API
                // calls because we only fetch rows where first_release_year IS NULL.
                // Future improvement: track last_tried_at separately.
                notFound++;
            }
        }
        catch (err) {
...
export async function enrichBluesArtistFromYouTube(id, apiKey) {
    if (!apiKey)
    ...
    if (targetRow.first_recording_title)
        parts.push(`"${targetRow.first_recording_title}"`);
    const q = parts.join(" ");
...
function _normaliseDiscogsName(s) {
    ...
    for (const year of years) {
        for (const filter of DISCOGS_SEED_GENRES) {
            for (let page = 1; page <= maxPages; page++) {
                ...
                let data;
                try {
                    data = await client.get("/database/search", params);
                }
                catch (err) {
                ...
                if (debug)
                    console.log(`[discogs seed] ${year} ${filter.genre}/${filter.style || "*"} p${page} -> ${results.length} rows`);
                for (const r of results) {
                    ...
                    if (lc.includes("various") || lc === "unknown artist" || lc === "no artist")
                        continue;
                    if (/[,&/]/.test(credit) && credit.split(/[,&/]/).length > 2)
                        continue;
                    const key = _normaliseDiscogsName(credit);
                    if (!byName.has(key))
                        byName.set(key, { releases: [], styles: new Set(), labels: new Set() });
                    const b = byName.get(key);
                    const id = r.id;
                    const type = (r.type ?? "master");
                    const yr = r.year ? parseInt(r.year, 10) : year;
                    const labelStr = Array.isArray(r.label) ? r.label[0] : r.label ?? "";
