...
export function initAllBluesModule(adminClerkId) {
    ...
    setTimeout(() => {
        getAppSetting(ACTIVE_KEY).then(raw => {
            ...
            try {
                ...
                startAllBluesRun({ fromYear: p.fromYear, toYear: p.toYear }).catch(err => console.error("[all-blues] boot-resume failed:", err));
            }
            catch { /* ignore */ }
        }).catch(() => { });
    }, 5000);
...
export async function startAllBluesRun(opts) {
    if (_running)
        return { ok: false, error: "All Blues worker already running" };
    ...
    await getPool().query(`UPDATE all_blues_warm_state SET running=true, phase='collect', from_year=$1, to_year=$2,
       started_at=NOW(), last_tick_at=NOW(), last_error=NULL WHERE id=1`, [fromYear, toYear]);
    if (opts.resetQueue) {
        ...
        await getPool().query(`UPDATE all_blues_artist_queue
        ...
        await getPool().query(`UPDATE all_blues_warm_state SET artists_queued=$1, artists_fetched=0,
         artists_errored=0, links_inserted=$2, last_error=NULL WHERE id=1`, [queueSize.rows[0]?.n ?? 0, linksNow.rows[0]?.n ?? 0]);
    }
    (async () => {
        try {
            ...
            if (!_stopRequested)
                await _runReleaseNotes(fromYear, toYear, nameToId);
            if (!_stopRequested)
                await _runFetch(client, nameToId);
            ...
            try {
                const clearFn = globalThis._sdGraphCacheClear;
            ...
            try {
                await getPool().query(`UPDATE all_blues_warm_state SET last_error=$1 WHERE id=1`, [`crash: ${err?.message ?? String(err)}`]);
            }
            catch { }
        }
        finally {
            ...
            _activeParams = null;
            try {
                await getPool().query(`UPDATE all_blues_warm_state SET running=false, phase='idle' WHERE id=1`);
            }
            catch { }
        }
...
async function _runCollect(fromYear, toYear) {
  ...
  `);
    await getPool().query(`UPDATE all_blues_warm_state SET artists_queued = artists_queued + $1,
       links_inserted = links_inserted + $2,
       phase='fetch', last_tick_at=NOW() WHERE id=1`, [manualSeeds.rowCount ?? 0, manualLinks.rowCount ?? 0]);
...
async function _runFetch(client, nameToId) {
    ...
    while (true) {
        ...
        const next = await getPool().query(`SELECT discogs_id FROM all_blues_artist_queue
        ...
        const discogsId = next.rows[0].discogs_id;
        try {
            // Skip if already cached recently
            const cached = await getPool().query(`SELECT 1 FROM discogs_artist_cache WHERE discogs_id=$1`, [discogsId]);
            let data;
            if (cached.rows.length) {
                const r = await getPool().query(`SELECT data FROM discogs_artist_cache WHERE discogs_id=$1`, [discogsId]);
                data = r.rows[0].data;
            }
            else {
                await _sleep(REQ_INTERVAL_MS);
                data = await _withRetry(`artist ${discogsId}`, () => client.getArtist(discogsId));
                await getPool().query(`INSERT INTO discogs_artist_cache (discogs_id, name, profile, data)
             VALUES ($1, $2, $3, $4)
           ON CONFLICT (discogs_id) DO UPDATE SET name=EXCLUDED.name, profile=EXCLUDED.profile,
             data=EXCLUDED.data, cached_at=NOW()`, [discogsId, data?.name ?? null, data?.profile ?? null, data]);
            }
                ...
                for (const mention of mentions) {
                    for (const kind of mention.kinds) {
                        try {
                            ...
                            // separately.
                            const r = await getPool().query(`INSERT INTO all_blues_links (src_id, dst_id, kind, excerpt)
                 SELECT $1, $2, $3, $4
                  WHERE EXISTS (
                    SELECT 1 FROM all_blues_artist_queue
                     WHERE discogs_id=$2 AND seed_year IS NOT NULL
                  )
                 ON CONFLICT (src_id, dst_id, kind) DO NOTHING`, [discogsId, mention.dst, kind, mention.excerpt]);
                            if (r.rowCount)
                                inserted++;
                        }
                        catch (linkErr) {
            ...
            await getPool().query(`UPDATE all_blues_artist_queue SET status='done', fetched_at=NOW() WHERE discogs_id=$1`, [discogsId]);
            await getPool().query(`UPDATE all_blues_warm_state SET artists_fetched = artists_fetched + 1,
           links_inserted = links_inserted + $1, last_tick_at=NOW() WHERE id=1`, [inserted]);
        }
        catch (err) {
            const msg = err?.message ?? String(err);
            await getPool().query(`UPDATE all_blues_artist_queue SET status='error', error=$2, fetched_at=NOW() WHERE discogs_id=$1`, [discogsId, msg.slice(0, 500)]);
            await getPool().query(`UPDATE all_blues_warm_state SET artists_errored = artists_errored + 1,
           last_error=$1, last_tick_at=NOW() WHERE id=1`, [`artist ${discogsId}: ${msg.slice(0, 200)}`]);
...
function _sentenceWindow(text, idx, len) {
    const CAP = 300;
...
function _classifyKinds(window) {
    const out = [];
...
async function _buildNameToIdMap() {
    const map = new Map();
...
function _scanMentions(text, nameToId) {
    const out = [];
    const seenDst = new Set();
    const recordHit = (dst, idx, matchLen) => {
        if (!Number.isFinite(dst) || dst <= 0 || seenDst.has(dst))
            return;
        seenDst.add(dst);
...
async function _runReleaseNotes(fromYear, toYear, nameToId) {
    console.log(`[all-blues] release-notes phase: scanning blues releases (${fromYear}-${toYear})`);
    await getPool().query(`UPDATE all_blues_warm_state SET phase='release-notes', last_tick_at=NOW() WHERE id=1`);
    ...
    const r = await getPool().query(`SELECT rc.discogs_id, rc.type, rc.data
    ...
    await getPool().query(`UPDATE all_blues_warm_state SET last_tick_at=NOW() WHERE id=1`);
    ...
    const insertEdge = async (src, dst, kind, excerpt, releaseId) => {
        if (src === dst)
            return 0;
        try {
            const ins = await getPool().query(`INSERT INTO all_blues_links (src_id, dst_id, kind, excerpt, release_ids)
    ...
    for (const row of r.rows) {
        if (_stopRequested)
            break;
        const data = row.data || {};
        const primaries = Array.isArray(data.artists) ? data.artists : [];
        const srcIds = [];
        ...
        if (Date.now() - lastTickAt >= TICK_EVERY_MS) {
            await getPool().query(`UPDATE all_blues_warm_state SET last_tick_at=NOW(),
           links_inserted = links_inserted + $1 WHERE id=1`, [edgesSinceLastTick]);
            edgesSinceLastTick = 0;
            lastTickAt = Date.now();
        }
    }
    await getPool().query(`UPDATE all_blues_warm_state SET links_inserted = links_inserted + $1,
       last_tick_at=NOW() WHERE id=1`, [edgesSinceLastTick]);
