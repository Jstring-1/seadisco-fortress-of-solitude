...
export async function listBluesTimelineReleases(opts = { fromYear: 1925, toYear: 1930 }) {
  ...
  `;
    const totalR = await getPool().query(`SELECT COUNT(*)::int AS n ${fromSql} ${whereSql}`, [fromY, toY]);
    const total = totalR.rows[0]?.n ?? 0;
    const r = await getPool().query(`SELECT a.id          AS artist_id,
            a.name        AS artist_name,
