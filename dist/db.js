...
export async function pruneAllStaleData() {
    ...
    const i = await getPool().query(`DELETE FROM interest_signals WHERE recorded_at < ${interval30d}`);
    ...
    const tp = await getPool().query(`DELETE FROM user_taste_profiles WHERE updated_at < ${interval30d}`);
    return {
