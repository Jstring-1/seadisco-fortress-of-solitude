...
export async function reorderPlayQueue(clerkUserId, orderedPositions) {
    ...
    try {
        ...
        const insertRow = async (row, pos) => client.query(`INSERT INTO user_play_queue (clerk_user_id, position, source, external_id, data) VALUES ($1, $2, $3, $4, $5)`, [clerkUserId, pos, row.source, row.external_id, JSON.stringify(row.data ?? {})]);
        const seen = new Set();
        let newPos = 1;
        for (const oldPos of orderedPositions) {
            const row = byPos.get(oldPos);
            if (!row || seen.has(oldPos))
                continue; // unknown or duplicate position
            seen.add(oldPos);
...
export async function setTrackYtOverrideAdmin(args) {
    await getPool().query(`INSERT INTO track_youtube_overrides
...
export async function cascadeTrackYtVideoSwap(args) {
    ...
    if (args.newVideoId) {
      ...
      CASE WHEN data ? 'ytUrl'
           ...
           ELSE '{}'::jsonb END`;
        const pr = await getPool().query(`UPDATE user_playlist_items
          SET external_id = $3, data = ${dataPatch}
        WHERE ${scopeSql}
        RETURNING playlist_id`, [args.oldVideoId, albumIds, args.newVideoId]);
        playlistRows = pr.rowCount ?? 0;
        const touched = [...new Set(pr.rows.map((r) => r.playlist_id))];
        if (touched.length) {
            await getPool().query(`UPDATE user_playlists SET updated_at = NOW() WHERE id = ANY($1::int[])`, [touched]);
        }
        const qr = await getPool().query(`UPDATE user_play_queue
          SET external_id = $3, data = ${dataPatch}
        WHERE ${scopeSql}`, [args.oldVideoId, albumIds, args.newVideoId]);
        queueRows = qr.rowCount ?? 0;
    }
    else {
        const pr = await getPool().query(`DELETE FROM user_playlist_items WHERE ${scopeSql} RETURNING playlist_id`, [args.oldVideoId, albumIds]);
        playlistRows = pr.rowCount ?? 0;
        const touched = [...new Set(pr.rows.map((r) => r.playlist_id))];
        if (touched.length) {
            await getPool().query(`UPDATE user_playlists SET updated_at = NOW() WHERE id = ANY($1::int[])`, [touched]);
        }
        const qr = await getPool().query(`DELETE FROM user_play_queue WHERE ${scopeSql}`, [args.oldVideoId, albumIds]);
        queueRows = qr.rowCount ?? 0;
    }
...
export async function getSearchedTrackPositions(masterId) {
    const r = await getPool().query(`SELECT track_position FROM track_yt_review_searched WHERE master_id = $1`, [Number(masterId)]);
    return new Set(r.rows.map((row) => String(row.track_position)));
}
...
export async function listReviewQueue(opts = {}) {
    ...
    const totalR = await getPool().query(`SELECT COUNT(*)::int AS n FROM track_yt_review_queue WHERE ${whereSql}`, params);
    const rowParams = params.slice();
    rowParams.push(limit);
    const limIdx = rowParams.length;
    rowParams.push(offset);
    const offIdx = rowParams.length;
    ...
    const r = await getPool().query(`SELECT * FROM track_yt_review_queue
      WHERE ${whereSql}
      ORDER BY ${orderSql}
      LIMIT $${limIdx} OFFSET $${offIdx}`, rowParams);
...
export async function reviewQueueUnreject(id) {
    const r = await getPool().query(`UPDATE track_yt_review_queue
...
export async function reviewQueueRejectTrack(masterId, trackPosition, reviewer) {
    const r = await getPool().query(`UPDATE track_yt_review_queue
        SET status = 'rejected', reviewed_at = NOW(), reviewed_by = $3
      WHERE master_id = $1 AND COALESCE(track_position, '') = $2 AND status = 'pending'`, [masterId, trackPosition, reviewer]);
    return r.rowCount ?? 0;
}
...
export async function reviewQueueDismissPendingForResearch() {
    const r = await getPool().query(`WITH d AS (
...
export async function getChannelProfiles(ids, maxAgeDays) {
    ...
    if (!ids.length)
        return out;
    const r = await getPool().query(`SELECT channel_id, data FROM yt_channel_profiles
      WHERE channel_id = ANY($1::text[]) AND fetched_at > NOW() - ($2::int * INTERVAL '1 day')`, [ids, maxAgeDays]);
    for (const row of r.rows)
        out.set(String(row.channel_id), row.data);
    return out;
}
export async function upsertChannelProfiles(profiles) {
    for (const p of profiles) {
        await getPool().query(`INSERT INTO yt_channel_profiles (channel_id, data, fetched_at) VALUES ($1, $2, NOW())
       ON CONFLICT (channel_id) DO UPDATE SET data = EXCLUDED.data, fetched_at = NOW()`, [p.id, JSON.stringify(p.data)]);
    }
}
...
export async function getAiHuntDecidedChannelIds() {
    ...
    return new Set(r.rows.map(x => String(x.channel_id)));
}
...
export async function upsertAiChannelCandidates(findings, source) {
    if (!findings.length)
        return 0;
    const ids = findings.map(f => f.channelId);
    const cur = await getPool().query(`SELECT channel_id, status, score, signals, samples, sources FROM yt_ai_channel_candidates WHERE channel_id = ANY($1::text[])`, [ids]);
    const existing = new Map(cur.rows.map(r => [String(r.channel_id), r]));
    let added = 0;
    for (const f of findings) {
        ...
        const samples = [...f.samples, ...(ex?.samples ?? [])]
            .filter((s) => s?.videoId && !seen.has(s.videoId) && seen.add(s.videoId))
            .slice(0, 8);
        await getPool().query(`INSERT INTO yt_ai_channel_candidates (channel_id, channel_title, score, signals, samples, sources, status, first_seen, last_seen)
       ...
       ON CONFLICT (channel_id) DO UPDATE SET
         ...
         last_seen = NOW()
       WHERE yt_ai_channel_candidates.status = 'pending'`, [f.channelId, f.channelTitle, f.score, JSON.stringify(signals), JSON.stringify(samples), JSON.stringify(sources)]);
    }
    return added;
}
export async function listAiChannelCandidates(status, limit = 200) {
    const r = await getPool().query(`SELECT channel_id, channel_title, score, signals, samples, sources, status, first_seen, last_seen, decided_at
       FROM yt_ai_channel_candidates
      WHERE status = $1
      ORDER BY ${status === "pending" ? "score DESC, last_seen DESC" : "decided_at DESC NULLS LAST"}
      LIMIT $2`, [status, Math.max(1, Math.min(500, limit))]);
    return r.rows;
}
export async function countAiChannelCandidates() {
    const r = await getPool().query(`SELECT status, COUNT(*)::int AS n FROM yt_ai_channel_candidates GROUP BY status`);
    const out = { pending: 0, banned: 0, ignored: 0 };
    for (const x of r.rows)
        out[String(x.status)] = Number(x.n);
    return out;
}
export async function setAiChannelCandidateStatus(channelId, status) {
    const r = await getPool().query(`UPDATE yt_ai_channel_candidates
        SET status = $2, decided_at = CASE WHEN $2 = 'pending' THEN NULL ELSE NOW() END
      WHERE channel_id = $1`, [channelId, status]);
    return (r.rowCount ?? 0) > 0;
}
...
export async function getReviewQueueVideosForAiScan(perChannel = 5) {
    const r = await getPool().query(`SELECT candidate_video_id, candidate_title, candidate_description, candidate_channel_id,
...
export async function getChannelQueueSamples(ids, perChannel = 4) {
    const out = new Map();
    if (!ids.length)
        return out;
    const r = await getPool().query(`SELECT channel_id, video_id, title, thumb, status FROM (
     ...
     ORDER BY channel_id, rn`, [ids, perChannel]);
    for (const row of r.rows) {
...
export async function listChannelTrust(limit = 200) {
    const r = await getPool().query(`WITH human AS (
...
export async function refreshDerivedChannelTrust(minApprovals = 5, minRatio = 0.9) {
    const qualifying = `
    SELECT candidate_channel_id AS channel_id,
           MAX(candidate_channel_title) AS channel_title,
           COUNT(*) FILTER (WHERE status = 'approved')::int AS approvals,
  ...
  `;
    ...
    const removed = await getPool().query(`DELETE FROM yt_channel_trust
      WHERE source = 'derived'
        AND channel_id NOT IN (SELECT channel_id FROM (${qualifying}) q)`, [minApprovals, minRatio]);
    const ins = await getPool().query(`INSERT INTO yt_channel_trust (channel_id, channel_title, state, source, approvals, rejections, updated_at)
     ...
     ON CONFLICT (channel_id) DO UPDATE
        SET channel_title = EXCLUDED.channel_title,
            ...
            updated_at    = NOW()
      WHERE yt_channel_trust.source = 'derived'`, [minApprovals, minRatio]);
...
export async function getTrustedChannelIds() {
    const r = await getPool().query(`SELECT channel_id FROM yt_channel_trust WHERE state = 'trusted'`);
    return new Set(r.rows.map((x) => String(x.channel_id)));
}
// Admin override. state=null clears the row entirely, letting the next
// refresh re-derive it from history.
export async function setChannelTrust(channelId, channelTitle, state) {
    if (!state) {
        await getPool().query(`DELETE FROM yt_channel_trust WHERE channel_id = $1`, [channelId]);
        return;
    }
    await getPool().query(`INSERT INTO yt_channel_trust (channel_id, channel_title, state, source, updated_at)
     ...
     ON CONFLICT (channel_id) DO UPDATE
        SET state = EXCLUDED.state, source = 'manual',
            ...
            updated_at = NOW()`, [channelId, channelTitle, state]);
}
export async function listYoutubeChannelBans() {
    const r = await getPool().query(`SELECT channel_id, channel_title, reason, created_at
       FROM yt_channel_bans ORDER BY created_at DESC`);
    return r.rows;
}
export async function getBannedYoutubeChannelIds() {
    const r = await getPool().query(`SELECT channel_id FROM yt_channel_bans`);
    return new Set(r.rows.map((x) => String(x.channel_id)));
}
...
export async function banYoutubeChannel(channelId, channelTitle, reason) {
    const id = String(channelId || "").trim();
    if (!id)
        return { ok: false, supersededPending: 0, removedAutoApproved: 0 };
    await getPool().query(`INSERT INTO yt_channel_bans (channel_id, channel_title, reason)
     VALUES ($1, $2, $3)
     ON CONFLICT (channel_id) DO UPDATE
       SET channel_title = COALESCE(EXCLUDED.channel_title, yt_channel_bans.channel_title),
           reason = COALESCE(EXCLUDED.reason, yt_channel_bans.reason)`, [id, channelTitle, reason]);
    const sup = await getPool().query(`UPDATE track_yt_review_queue
        SET status = 'superseded', reviewed_at = NOW(), reviewed_by = 'auto'
      WHERE candidate_channel_id = $1 AND status = 'pending'`, [id]);
    ...
    const removed = await getPool().query(`DELETE FROM track_youtube_overrides
...
export async function unbanYoutubeChannel(channelId) {
...
export async function resetReviewQuota() {
    const today = _quotaDayString();
    await getPool().query(`UPDATE track_yt_review_state
        SET quota_date = $1, quota_worker_searches = 0, quota_project_units = 0
      WHERE id = 1`, [today]);
...
async function _getCacheEnrichmentBatchV1(pairs) {
    ...
    try {
        ...
        const types = capped.map(p => String(p.type));
        const r = await getPool().query(`SELECT discogs_id AS id, type, ${_ENRICH_FIELDS} AS data
         FROM release_cache
...
export async function getDiscogsSearchCache(cacheKey, maxAgeSeconds) {
    try {
        const r = await getPool().query(`SELECT body, cached_at FROM discogs_search_cache WHERE cache_key = $1 LIMIT 1`, [cacheKey]);
        const row = r.rows[0];
        if (!row)
            return null;
        const ageMs = Date.now() - new Date(row.cached_at).getTime();
...
export async function setDiscogsSearchCache(cacheKey, body) {
    try {
        await getPool().query(`INSERT INTO discogs_search_cache (cache_key, body, cached_at)
       VALUES ($1, $2::jsonb, NOW())
       ON CONFLICT (cache_key) DO UPDATE SET body = EXCLUDED.body, cached_at = NOW()`, [cacheKey, JSON.stringify(body)]);
    }
    catch { /* cache is best-effort */ }
}
export async function pruneDiscogsSearchCache() {
    try {
        const r = await getPool().query(`DELETE FROM discogs_search_cache WHERE cached_at < NOW() - INTERVAL '3 days'`);
        return r.rowCount ?? 0;
    }
    catch {
        return 0;
...
export async function getDeadDiscogsIds(ids, type = "master") {
    if (!ids.length)
        return new Set();
    const r = await getPool().query(`SELECT discogs_id FROM discogs_dead_ids WHERE type = $1 AND discogs_id = ANY($2::bigint[])`, [type, ids]);
    return new Set((r.rows ?? []).map((row) => Number(row.discogs_id)));
}
...
export async function recordDeadDiscogsId(id, type = "master") {
    if (!Number.isFinite(id) || id <= 0)
        return;
    await getPool().query(`INSERT INTO discogs_dead_ids (discogs_id, type) VALUES ($1, $2)
       ON CONFLICT (discogs_id, type) DO NOTHING`, [id, type]);
}
