...
async function loadYearBackfill() {
  ...
  if (!el) return;
  try {
    const r = await apiFetch("/api/admin/year-backfill/batches?limit=20");
    if (!r.ok) { el.innerHTML = `<span style="color:#e88">Failed: HTTP ${r.status}</span>`; return; }
    const { batches } = await r.json();

    const previewBlock = _ybfPreview
      ? `<div style="border:1px solid var(--border);border-radius:6px;padding:0.6rem 0.8rem;margin:0.6rem 0;background:rgba(255,255,255,0.02)">
          <div style="font-weight:600;font-size:0.85rem;margin-bottom:0.3rem">Preview</div>
          <div style="font-size:0.8rem">Phase 1 (releases): <strong>${_ybfPreview.phase1Release}</strong> · Phase 2 (masters): <strong>${_ybfPreview.phase2Master}</strong></div>
          <div style="font-size:0.78rem;color:var(--muted);margin-top:0.3rem">By donor source:</div>
    ...
    const batchesBlock = batches.length
      ? `<table style="width:100%;font-size:0.78rem;border-collapse:collapse;margin-top:0.4rem">
          <thead><tr style="text-align:left;color:var(--muted);border-bottom:1px solid var(--border)">
            <th style="padding:0.25rem 0.3rem">Batch</th>
            <th style="padding:0.25rem 0.3rem">When</th>
            <th style="padding:0.25rem 0.3rem;text-align:right">Total</th>
            <th style="padding:0.25rem 0.3rem;text-align:right">Reverted</th>
            <th style="padding:0.25rem 0.3rem">Top sources</th>
            <th></th>
          </tr></thead>
          <tbody>
            ${batches.map(b => {
              const dt = new Date(b.applied_at);
              const when = isNaN(dt) ? b.applied_at : dt.toLocaleString();
              const sources = (b.by_source || []).slice(0, 3).map(s => `${_eHtml(s.donor_source)} (${s.n})`).join(", ");
              const fullyReverted = b.reverted >= b.total;
              return `<tr style="border-bottom:1px dashed rgba(255,255,255,0.05)">
                <td style="padding:0.25rem 0.3rem"><code style="font-size:0.72rem">${b.batch_id.slice(0, 8)}…</code></td>
                <td style="padding:0.25rem 0.3rem;color:var(--muted)">${_eHtml(when)}</td>
                <td style="padding:0.25rem 0.3rem;text-align:right">${b.total}</td>
                <td style="padding:0.25rem 0.3rem;text-align:right;color:${b.reverted > 0 ? "#e88" : "var(--muted)"}">${b.reverted}</td>
                <td style="padding:0.25rem 0.3rem;color:var(--muted)">${sources}</td>
                <td style="padding:0.25rem 0.3rem;text-align:right">
                  ${fullyReverted
                    ? `<span style="color:var(--muted);font-size:0.72rem">rolled back</span>`
                    : `<button class="admin-btn" type="button" onclick="_ybfRollback('${b.batch_id}')" style="font-size:0.72rem;padding:0.15rem 0.4rem">Roll back</button>`}
                </td>
              </tr>`;
            }).join("")}
          </tbody>
        </table>`
      : `<div style="color:var(--muted);font-size:0.78rem;margin-top:0.4rem">No backfill batches yet.</div>`;
    ...
    el.innerHTML = `
      ...
      ${previewBlock}
      <div style="font-weight:600;font-size:0.82rem;margin-top:0.6rem">Recent batches</div>
      ${batchesBlock}
    `;
  } catch (err) {
    el.innerHTML = `<span style="color:#e88">${_eHtml(String(err))}</span>`;
...
// Pull the error message out of a 500 response body so alerts show
// the actual SQL/logic failure instead of just the status code.
async function _errText(r) {
  try {
    const j = await r.json();
    if (j?.error) return `HTTP ${r.status} — ${j.error}`;
  } catch {}
...
async function _ybfRollback(batchId) {
  ...
  try {
    const r = await apiFetch("/api/admin/year-backfill/rollback", {
      method: "POST", headers: { "content-type": "application/json" },
      body: JSON.stringify({ batchId }),
    });
    if (!r.ok) { alert(`Rollback failed: ${await _errText(r)}`); return; }
    const j = await r.json();
    alert(`Rolled back ${j.reverted} rows · drifted (skipped): ${j.drifted} · already reverted: ${j.alreadyReverted}`);
    loadYearBackfill();
  } catch (err) { alert(String(err)); }
}
