...
// ── JS-string-in-attribute escape ────────────────────────────────────────
// For values passed to inline handlers: onclick="fn(${jsAttr(x)})" — NO
// surrounding quotes. JSON.stringify makes a valid JS string literal
// (backslashes, quotes, newlines, U+2028 all escaped) and escHtml then makes
// it safe inside the double-quoted attribute; the browser decodes the
// entities back before the JS runs. The old '${escHtml(x)}' pattern is
// broken: &#39; decodes to ' before the handler is parsed, so an
// apostrophe in Discogs/Wikipedia data ended the string early.
function jsAttr(v) {
...
function renderSharedHeader(opts) {
  ...
  const SITE_VERSION = "build 260918.e64218e4";
  header.innerHTML = `
  ...
  (async () => {
    try {
      ...
      if (window._isAdmin) {
        ...
        // Eagerly preload scanner.js as soon as admin confirms so the
        // click handler can call _sdOpenScanner synchronously. With a
        // lazy .then() chain in the onclick, iOS / mobile Safari
        // treat the user gesture as expired and getUserMedia rejects
        // with NotAllowedError without ever prompting for permission.
        if (typeof _sdLoadModule === "function") {
          _sdLoadModule("/scanner.js").catch(() => {});
        }
      }
    ...
    el.addEventListener("click", e => {
      ...
      try {
        if (b.key === "open") {
          ...
          // searches (footer link) still go to /?v=youtube.
          //
...
async function _baStampCards(items, gridEl) {
  ...
  // The admin flag resolves async after /api/me. The home strip
  // renders during page-load, BEFORE that resolves, so this used to
  // early-return with _isAdmin === undefined and never run — which is
  // why the badges showed up on search results (rendered AFTER the
  // probe lands) but never on the home strip. Block on the flag.
