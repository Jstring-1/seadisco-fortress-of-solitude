  ...
  setTimeout(async () => {
    ...
    try {
      ...
      if (active.startsWith("adhoc:")) {
        ...
        await startAdHocLabelSweep(labelId, labelName).catch(err =>
          console.error("[cache-warm-catno] adhoc boot-resume failed:", err));
        return;
      }
      ...
      if (phase === "label_sweep_masters" || phase === "label_sweep_orphans") {
        ...
        await startCacheWarmCatnoRun(active).catch(err =>
          console.error("[cache-warm-catno] catno boot-resume failed:", err));
      }
    } catch (err) {
...
export async function startAdHocLabelSweep(
  labelId:   number,
  labelName: string,
  opts: { resetCursor?: boolean } = {},
): Promise<{ ok: boolean; error?: string; seriesKey?: string }> {
  if (!Number.isFinite(labelId) || labelId <= 0) return { ok: false, error: "labelId required" };
  const safeName = String(labelName ?? "").trim();
  if (!safeName) return { ok: false, error: "labelName required" };
  if (_runningKey) return { ok: false, error: `Another catno run is in progress: ${_runningKey}` };
  const client = await _adminClient();
  if (!client) return { ok: false, error: "Admin Discogs OAuth not connected (or DISCOGS_CONSUMER_KEY/SECRET missing)" };

  const key = `adhoc:${labelId}`;
  const ephemeral: CatnoSeries = {
    key,
    label:    safeName,
    labelId,
    lo: 0, hi: 0,
  ...
  (async () => {
    try {
      ...
      try { await recordCacheWarmCatnoRunError(key, `crash: ${err?.message ?? String(err)}`); }
      catch (e2) { console.error(`[cache-warm-catno] failed to record crash:`, e2); }
    } finally {
      _runningKey    = null;
      _stopRequested = false;
    }
