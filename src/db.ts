...
export async function initDb() {
  ...
  `);
  await getPool().query(
    `CREATE INDEX IF NOT EXISTS blues_tunings_grid_artist_idx ON blues_tunings_grid (artist)`,
  );
  await getPool().query(
    `CREATE INDEX IF NOT EXISTS blues_tunings_grid_title_idx ON blues_tunings_grid (title)`,
  );
  ...
  try {
    ...
    try {
      const vr = await getPool().query(
        `SELECT value FROM app_settings WHERE key = 'tunings_seed_version'`,
      );
...
function _parseTuningsCsv(raw: string): Array<{
  artist: string; track: string; title: string; position: string; pitch: string; notes: string;
}> {
  ...
  if (iTrack < 0) iTrack = idx("#");
  const rows: Array<{ artist: string; track: string; title: string; position: string; pitch: string; notes: string }> = [];
...
export async function recordCacheWarmRunSkip(genreKey: string, styleKey: string): Promise<void> {
  await getPool().query(
    `UPDATE cache_warm_runs SET total_skipped = total_skipped + 1
      WHERE genre_key = $1 AND style_key = $2`,
    [genreKey, styleKey || ""],
  );
}
...
export async function setSplitCacheReaderEnabled(v: boolean): Promise<void> {
...
export interface YearBackfillPreview {
  ...
  phase2Master:  number;
  bySource:      Array<{ donor_source: string; n: number }>;
  sample:        Array<{
...
export async function previewYearBackfill(): Promise<YearBackfillPreview> {
  const phase1Q = await getPool().query(
    `${_YEAR_BACKFILL_DONOR_CTE}
     SELECT donor_source, COUNT(*)::int AS n FROM matches GROUP BY donor_source ORDER BY n DESC`,
  );
  const totalQ = await getPool().query(
    `${_YEAR_BACKFILL_DONOR_CTE}
...
export interface YearBackfillApplyResult {
  batchId:        string;
  phase1Updated:  number;
  phase2Updated:  number;
  bySource:       Array<{ donor_source: string; n: number }>;
}

export async function applyYearBackfill(): Promise<YearBackfillApplyResult> {
  const client = await getPool().connect();
  try {
    await client.query("BEGIN");
    ...
    const phase1LogQ = await client.query(
      `${_YEAR_BACKFILL_DONOR_CTE}
    ...
    if (phase1Logged > 0) {
      const upd = await client.query(
        `UPDATE release_cache rc
          ...
          WHERE l.batch_id = $1::uuid
            ...
            AND rc.type = l.type
            AND COALESCE(NULLIF(rc.data->>'year','')::int, 0) = 0`,
        [batchId],
      );
      ...
      await client.query(
        `UPDATE discogs_cache_masters_plus mp
          ...
          WHERE l.batch_id = $1::uuid
            ...
            AND mp.type       = l.type
            AND (mp.year IS NULL OR mp.year = 0)`,
        [batchId],
      );
      await client.query(
        `UPDATE discogs_cache_pressings p
            SET year = l.new_year::smallint,
                data = jsonb_set(
          ...
          WHERE l.batch_id = $1::uuid
            ...
            AND (p.year IS NULL OR p.year = 0)`,
        [batchId],
      );
    }
    ...
    const sourceQ = await client.query(
      `SELECT donor_source, COUNT(*)::int AS n
...
export interface YearBackfillBatch {
  batch_id:     string;
  applied_at:   string;
  total:        number;
  reverted:     number;
  by_source:    Array<{ donor_source: string; n: number }>;
}

export async function listYearBackfillBatches(limit: number = 20): Promise<YearBackfillBatch[]> {
  const r = await getPool().query(
  ...
  return r.rows.map(row => {
    ...
    for (const s of (row.sources_raw as any[])) {
...
export interface YearBackfillRollbackResult {
  batchId:        string;
  reverted:       number;
  alreadyReverted: number;
  drifted:        number;   // value changed since we wrote it — left alone
}
...
export async function rollbackYearBackfillBatch(batchId: string): Promise<YearBackfillRollbackResult> {
  if (!/^[0-9a-f-]{36}$/i.test(batchId)) throw new Error("bad batch_id");
  const client = await getPool().connect();
  try {
    await client.query("BEGIN");
    const alreadyPreQ = await client.query(
      `SELECT COUNT(*)::int AS n FROM year_backfill_log
        WHERE batch_id = $1::uuid AND rolled_back_at IS NOT NULL`,
      [batchId],
    );
    ...
    const driftQ = await client.query(
      `SELECT COUNT(*)::int AS n
         FROM year_backfill_log l
         JOIN release_cache rc
        ...
        WHERE l.batch_id = $1::uuid
          ...
          AND NULLIF(rc.data->>'year','')::int IS DISTINCT FROM l.new_year`,
      [batchId],
    );
    ...
    const restoreNullQ = await client.query(
      `UPDATE release_cache rc
        ...
        WHERE l.batch_id = $1::uuid
          ...
          AND rc.discogs_id = l.discogs_id
          AND rc.type = l.type
          AND NULLIF(rc.data->>'year','')::int IS NOT DISTINCT FROM l.new_year`,
      [batchId],
    );
    const restoreValQ = await client.query(
      `UPDATE release_cache rc
    ...
    await client.query(
      `UPDATE discogs_cache_masters_plus mp
        ...
        WHERE l.batch_id = $1::uuid
          ...
          AND mp.discogs_id = l.discogs_id
          AND mp.type = l.type
          AND (mp.year IS NOT DISTINCT FROM l.new_year::smallint)`,
      [batchId],
    );
    await client.query(
      `UPDATE discogs_cache_masters_plus mp
    ...
    await client.query(
      `UPDATE discogs_cache_pressings p
        ...
        WHERE l.batch_id = $1::uuid
          ...
          AND l.type = 'release'
          AND p.discogs_id = l.discogs_id
          AND (p.year IS NOT DISTINCT FROM l.new_year::smallint)`,
      [batchId],
    );
    await client.query(
      `UPDATE discogs_cache_pressings p
