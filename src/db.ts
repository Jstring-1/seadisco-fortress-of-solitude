...
export async function previewYearBackfill(): Promise<YearBackfillPreview> {
  ...
  try {
    ...
    const phase1Q = await client.query(
    ...
    // (label, catno).
    const phase2Q = await client.query(
    ...
    return {
      ...
      bySource:      (payload.bySource ?? []).map((r: any) => ({ donor_source: String(r.donor_source), n: Number(r.n) })),
      sample:        (payload.sample ?? []).map((r: any) => ({
