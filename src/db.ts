...
function _sealSecret(v: string | null | undefined): string | null {
  if (v == null) return null;
  if (!_TOKEN_KEY || v === "" || v === "__oauth__" || v.startsWith(_SECRET_PREFIX)) return v;
...
function _openSecret(v: string | null | undefined): string | null {
  if (v == null || !v.startsWith(_SECRET_PREFIX)) return v ?? null;
  if (!_TOKEN_KEY) {
    if (!_openSecretWarned) { _openSecretWarned = true; console.error("[db] encrypted Discogs credentials found but TOKEN_ENC_KEY is not set — treating them as absent"); }
    return null;
  }
  try {
    ...
    if (!_openSecretWarned) { _openSecretWarned = true; console.error("[db] could not decrypt a stored Discogs credential (wrong TOKEN_ENC_KEY?) — treating it as absent"); }
    return null;
  }
}
...
export async function sealPlaintextTokens(): Promise<number> {
  if (!_TOKEN_KEY) {
    console.warn("[db] TOKEN_ENC_KEY not set — Discogs credentials are stored unencrypted");
    return 0;
  }
  const r = await getPool().query(
    `SELECT clerk_user_id, discogs_token, oauth_access_token, oauth_access_secret
       FROM user_tokens
  ...
  for (const row of r.rows) {
    await getPool().query(
      `UPDATE user_tokens SET discogs_token = $2, oauth_access_token = $3, oauth_access_secret = $4
        WHERE clerk_user_id = $1`,
      [row.clerk_user_id, _sealSecret(row.discogs_token), _sealSecret(row.oauth_access_token), _sealSecret(row.oauth_access_secret)],
    );
  }
  if (r.rows.length) console.log(`[db] encrypted stored Discogs credentials for ${r.rows.length} user(s)`);
...
// initDb() — the schema/migration bootstrap — now lives in db-schema.ts
// (extracted for file size; behaviour unchanged). Re-exported here so the
// public import surface is identical: callers still `import { initDb }
// from "./db.js"`.
export { initDb } from "./db-schema.js";
...
const _TUNINGS_SORT_COLS: Record<string, string> = {
...
export async function listBluesTunings(opts: {
  ...
  if (opts.artist)   { params.push(opts.artist);   where.push(`artist = $${params.length}`); }
  if (opts.position) { params.push(opts.position); where.push(`position = $${params.length}`); }
  if (opts.pitch)    { params.push(opts.pitch);    where.push(`pitch = $${params.length}`); }
  ...
  const rowsR = await getPool().query(
    `SELECT id, artist, track, title, position, pitch, notes
...
export async function storeOAuthRequestToken(token: string, tokenSecret: string, clerkUserId: string, csrfState?: string): Promise<void> {
  await getPool().query(
    ...
    [token, _sealSecret(tokenSecret), clerkUserId, csrfState ?? null]
  );
}
...
export async function setOAuthCredentials(clerkUserId: string, accessToken: string, accessSecret: string): Promise<void> {
  await getPool().query(
    ...
    [clerkUserId, _sealSecret(accessToken), _sealSecret(accessSecret)]
  );
}
...
export async function getOAuthCredentials(clerkUserId: string): Promise<{ accessToken: string; accessSecret: string } | null> {
  ...
  if (!accessToken || accessSecret == null) return null;
...
export async function resetOrphanedSyncingStatuses(activeIds: string[]): Promise<string[]> {
  const r = await getPool().query(
    `UPDATE user_tokens
  ...
  return r.rows.map(row => row.clerk_user_id as string);
}
...
export async function upsertCollectionFolders(
  ...
  const byId = new Map<number, typeof folders[number]>();
    ...
    if (rows.length) {
      await client.query(
        `INSERT INTO user_collection_folders (clerk_user_id, folder_id, folder_name, item_count)
         SELECT $1, x.id, x.nm, x.ct FROM unnest($2::int[], $3::text[], $4::int[]) AS x(id, nm, ct)`,
        [clerkUserId, rows.map(f => Number(f.id)), rows.map(f => f.name), rows.map(f => Number(f.count) || 0)],
      );
    }
    await client.query("COMMIT");
  } catch (e) {
...
export async function getPlaylist(by: { id: number; ownerId: string } | { shareToken: string }): Promise<{
  ...
  const head = "shareToken" in by
    ? await getPool().query(
        `SELECT id, name, clerk_user_id, created_at, updated_at
           FROM user_playlists WHERE share_token = $1`,
        [by.shareToken],
      )
    : await getPool().query(
        `SELECT id, name, clerk_user_id, created_at, updated_at
           FROM user_playlists WHERE id = $1 AND clerk_user_id = $2`,
        [by.id, by.ownerId],
      );
...
export async function ensurePlaylistShareToken(id: number, clerkUserId: string, freshToken: string): Promise<string | null> {
  const r = await getPool().query(
    `UPDATE user_playlists SET share_token = COALESCE(share_token, $3)
      WHERE id = $1 AND clerk_user_id = $2
      RETURNING share_token`,
    [id, clerkUserId, freshToken],
  );
...
export async function getLibraryExportPage(
  kind: "collection" | "wantlist", clerkUserId: string, offset: number, limit: number,
): Promise<any[]> {
  const r = kind === "collection"
    ? await getPool().query(
        `SELECT ${_EXPORT_FIELDS}, folder_id FROM user_collection WHERE clerk_user_id = $1
          ORDER BY LOWER(data->'artists'->0->>'name') ASC, LOWER(data->>'title') ASC, discogs_release_id, instance_id
          LIMIT $2 OFFSET $3`,
        [clerkUserId, limit, offset])
    : await getPool().query(
        `SELECT ${_EXPORT_FIELDS} FROM user_wantlist WHERE clerk_user_id = $1
...
export async function upsertUserLists(
  ...
  const byId = new Map<number, typeof lists[number]>();
  for (const l of lists) byId.set(Number(l.listId), l);
  const rows = [...byId.values()];
  await getPool().query(
    `INSERT INTO user_lists (clerk_user_id, list_id, name, description, item_count, is_public, data, synced_at)
     ...
     DO UPDATE SET name = EXCLUDED.name, description = EXCLUDED.description, item_count = EXCLUDED.item_count,
                   is_public = EXCLUDED.is_public, data = EXCLUDED.data, synced_at = NOW()`,
    [clerkUserId,
...
export async function upsertListItems(
  ...
  for (const it of items) byId.set(Number(it.discogsId), it);
  const rows = [...byId.values()];
  const client = await getPool().connect();
  try {
    await client.query("BEGIN");
    ...
    await client.query(
      `INSERT INTO user_list_items (clerk_user_id, list_id, discogs_id, entity_type, comment, data, synced_at)
       SELECT $1, $2, x.id, x.et, x.cm, x.dt::jsonb, NOW()
         FROM unnest($3::int[], $4::text[], $5::text[], $6::text[]) AS x(id, et, cm, dt)`,
      [clerkUserId, listId,
...
export interface RedundantReleasePreview {
  warmOnly: number;   // redundant AND never opened by a human
  total: number;      // all redundant pressings (incl. viewed)
  releaseRows: number;// total type='release' rows in cache (context)
  masterRows: number; // total type='master' rows in cache (context)
}

export async function previewRedundantReleases(): Promise<RedundantReleasePreview> {
  const r = await getPool().query(
...
export async function pruneRedundantReleases(
  mode: "warm_only" | "all" = "warm_only"
  ...
  try {
    ...
    await client.query(
      `CREATE TEMP TABLE _prune_ids ON COMMIT DROP AS
       WITH ${_PRUNE_PROTECTED_CTE}
       SELECT r.discogs_id
         FROM release_cache r
...
export function projectReleaseData(
  ...
  if (Array.isArray((data as any).artists)) {
    for (const a of (data as any).artists) {
      ...
      if (id != null) artists.push({ id, name: _projCoerceStr(a?.name), role: "main" });
    }
  }
  if (Array.isArray((data as any).extraartists)) {
    for (const a of (data as any).extraartists) {
      const id = _projCoerceInt(a?.id);
      if (id != null) artists.push({ id, name: _projCoerceStr(a?.name), role: "extra" });
    }
  }
...
export async function writeProjectedCache(
  ...
  try {
    ...
    if (proj.artists.length > 0) {
      ...
      if (rows.length > 0) {
        await client.query(
          `INSERT INTO release_artists (discogs_id, bucket, artist_id, role, name) VALUES ${rows.join(",")}`,
          params,
        );
      }
    }
...
// Batched version of writeProjectedCache. Given N rows, executes ~8
// round trips instead of ~8·N. Used by the backfill worker where
// throughput matters more than isolation of a single row. If the
// batched transaction fails, falls back to per-row writes so a
// poison row doesn't sink the batch.
export interface ProjectionBatchInput { discogs_id: number; type: DiscogsCacheType; data: any; }
export interface ProjectionBatchResult { ok: number; err: number; lastError: string | null; }
export async function writeProjectedCacheBatch(
  batch: ProjectionBatchInput[],
): Promise<ProjectionBatchResult> {
  if (batch.length === 0) return { ok: 0, err: 0, lastError: null };
  // Project all rows up front. Non-projectable rows (artist / master-
  // versions in the input) are silently dropped.
  const rows: Array<ProjectionBatchInput & { proj: ProjectedRelease }> = [];
  for (const r of batch) {
    const proj = projectReleaseData(r.type, r.data);
    if (proj) rows.push({ ...r, proj });
  }
  if (rows.length === 0) return { ok: 0, err: 0, lastError: null };
  ...
  const labelRows:  Array<[number, number, number | null, string, string | null]> = [];
  const artistRows: Array<[number, number, number, string, string | null]> = [];
  const tagRows:    Array<[number, number, string, string]> = [];
  for (const r of rows) {
  ...
  try {
    ...
    await chunkedInsert(client,
      ph => `INSERT INTO release_labels  (discogs_id, bucket, label_id, label_name, catno) VALUES ${ph}`,
      labelRows,  5);
    await chunkedInsert(client,
      ph => `INSERT INTO release_artists (discogs_id, bucket, artist_id, role, name)        VALUES ${ph}`,
      artistRows, 5);
    await chunkedInsert(client,
      ph => `INSERT INTO release_tags    (discogs_id, bucket, kind, value)                  VALUES ${ph}`,
      tagRows,    4);
    ...
    // stays isolated.
    let ok = 0, errCount = 0, lastError: string | null = null;
    for (const r of rows) {
      try {
...
export async function getTableRowCounts(): Promise<Array<{ table: string; rows: number; bytes: number }>> {
  const r = await getPool().query(
  ...
  return r.rows.map((row: any) => ({ table: String(row.table), rows: Number(row.rows) || 0, bytes: Number(row.bytes) || 0 }));
}
...
export async function insertReviewCandidate(args: {
  ...
  // The YouTube search that surfaced this candidate, so runs with
  // different query templates can be compared after the fact.
...
export async function supersedeOtherCandidates(
  masterId: number | string, trackPosition: string, keepVideoId: string,
): Promise<number> {
  const r = await getPool().query(
    `UPDATE track_yt_review_queue
        SET status = 'superseded', reviewed_at = NOW(), reviewed_by = 'auto'
      WHERE master_id = $1 AND track_position = $2
        AND candidate_video_id <> $3 AND status = 'pending'`,
    [Number(masterId), trackPosition, keepVideoId],
  );
  return r.rowCount ?? 0;
}
...
export async function listReviewQueue(opts: {
  ...
  if (status === "auto") {
    ...
    if (status === "approved") where.push("COALESCE(reviewed_by, '') <> 'auto'");
  }
  if (q) {
    params.push(`%${q}%`);
...
export async function getReviewQueueCounts(): Promise<{ pending: number; approved: number; auto: number; rejected: number; skipped: number; total: number }> {
  const r = await getPool().query(
    `SELECT CASE WHEN status = 'approved' AND reviewed_by = 'auto' THEN 'auto' ELSE status END AS status,
...
// slot is ignored. Discogs masters have no labels of their own; a cached
// master's label comes from, in order: a label stamped onto the master row
// by a sweep, its cached main release, any cached pressing of it, then its
// cached version list (saved when someone opens the album). Label names
// are grouped with Discogs's " (2)" disambiguator stripped.
...
export async function getYearLabelAlbums(f: YearLabelFilter, year: number, label: string, limit = 1000): Promise<{ albums: YearLabelAlbum[]; truncated: boolean }> {
  ...
  try {
    ...
    const ovMap = new Map<string, Map<string, Ov>>();
    for (const o of ov) {
...
export async function reviewQueueConfirmAuto(ids: number[], reviewer: string): Promise<number> {
  if (!ids.length) return 0;
  const r = await getPool().query(
    `UPDATE track_yt_review_queue
        SET reviewed_by = $2, reviewed_at = NOW()
      WHERE id = ANY($1::int[]) AND status = 'approved' AND reviewed_by = 'auto'`,
    [ids, reviewer],
  );
  return r.rowCount ?? 0;
}
...
export async function logAdminAction(entry: { actor?: string | null; action: string; target?: string | null; status?: number | null; detail?: any }): Promise<void> {
  try {
    await getPool().query(
      `INSERT INTO admin_audit (actor, action, target, status, detail) VALUES ($1, $2, $3, $4, $5)`,
      [entry.actor ?? null, entry.action.slice(0, 200), entry.target ? String(entry.target).slice(0, 300) : null,
...
export async function listAdminActions(limit = 100, before?: number | null): Promise<any[]> {
  const r = await getPool().query(
    `SELECT id, at, actor, action, target, status, detail FROM admin_audit
      WHERE ($2::bigint IS NULL OR id < $2)
      ORDER BY id DESC LIMIT $1`,
    [Math.max(1, Math.min(500, limit)), before ?? null],
  );
  return r.rows;
}
...
export async function ensureBackgroundIndexes(): Promise<void> {
...
export async function bumpReviewCounter(field: "total_searched" | "total_queued" | "total_skipped" | "total_errors" | "total_auto_approved", by = 1): Promise<void> {
...
function _quotaDayString(d: Date = new Date()): string {
      ...
      try {
        const fb = await getPool().query(
          `SELECT DISTINCT ON ((data->>'master_id'))
            ...
            ORDER BY (data->>'master_id'), discogs_id ASC`,
          [missedMasterIds.map(String)]
        );
...
export async function reportYoutubeVideoUnavailable(
  ...
  if (reporterUserId) {
    const first = await getPool().query(
      `INSERT INTO youtube_video_unavailable_reporters (video_id, clerk_user_id)
       VALUES ($1, $2) ON CONFLICT DO NOTHING`,
      [videoId, reporterUserId],
    );
    if ((first.rowCount ?? 0) === 0) {
      const cur = await getPool().query(
        `SELECT status, report_count FROM youtube_video_unavailable WHERE video_id = $1`,
        [videoId],
      );
...
export async function listCacheWarmRuns(): Promise<any[]> {
  const r = await getPool().query(
    `SELECT * FROM cache_warm_runs ORDER BY lower(genre_key) ASC, lower(style_key) ASC`,
  );
  return r.rows;
}
export async function getCacheWarmRun(genreKey: string, styleKey: string): Promise<any> {
  const r = await getPool().query(
    `SELECT * FROM cache_warm_runs WHERE genre_key = $1 AND style_key = $2`,
    [genreKey, styleKey || ""],
  );
  return r.rows[0] || null;
}
export async function upsertCacheWarmRun(
  genreKey: string,
  styleKey: string,
  ...
  if (!cols.length) {
    ...
    await getPool().query(
      `INSERT INTO cache_warm_runs (genre_key, style_key) VALUES ($1, $2)
        ON CONFLICT (genre_key, style_key) DO NOTHING`,
      [genreKey, styleKey || ""],
    );
    return;
  }
  ...
  await getPool().query(
    `INSERT INTO cache_warm_runs (${insertCols})
     VALUES (${insertVals})
     ON CONFLICT (genre_key, style_key) DO UPDATE SET ${setSql}`,
    [genreKey, styleKey || "", ...vals],
  );
}
export async function recordCacheWarmRunHit(
  genreKey: string, styleKey: string,
  title: string, releaseId: number,
): Promise<void> {
  await getPool().query(
    ...
    [genreKey, styleKey || "", title, releaseId],
  );
}
...
export async function recordCacheWarmRunError(genreKey: string, styleKey: string, msg: string): Promise<void> {
  await getPool().query(
    `UPDATE cache_warm_runs
      ...
      WHERE genre_key = $1 AND style_key = $2`,
    [genreKey, styleKey || "", msg.slice(0, 500)],
  );
}
...
export async function getCacheWarmCatnoRun(seriesKey: string): Promise<any> {
  const r = await getPool().query(
    `SELECT * FROM cache_warm_catno_runs WHERE series_key = $1`,
    [seriesKey],
  );
  return r.rows[0] || null;
}
export async function listCacheWarmCatnoRuns(): Promise<any[]> {
  const r = await getPool().query(
    `SELECT * FROM cache_warm_catno_runs ORDER BY label ASC, cat_lo ASC`,
  );
  return r.rows || [];
}
export async function upsertCacheWarmCatnoRun(
  seriesKey: string,
  seed: { label: string; catLo: number; catHi: number; yearMax: number | null },
  patch: Record<string, any> = {},
): Promise<void> {
  const allowed = new Set([
    "current_catno",
  ...
  await getPool().query(
    `INSERT INTO cache_warm_catno_runs (${baseCols.join(", ")})
     VALUES (${placeholders})
     ON CONFLICT (series_key) DO UPDATE SET ${updateSet}`,
    baseVals,
  );
}
export async function recordCacheWarmCatnoRunHit(
  seriesKey: string,
  title: string,
  releaseId: number,
): Promise<void> {
  await getPool().query(
    `UPDATE cache_warm_catno_runs
      ...
      WHERE series_key = $1`,
    [seriesKey, title, releaseId],
  );
}
export async function bumpCacheWarmCatnoRunSkip(seriesKey: string, n: number): Promise<void> {
  if (!Number.isFinite(n) || n <= 0) return;
  await getPool().query(
    `UPDATE cache_warm_catno_runs SET total_skipped = total_skipped + $2
      WHERE series_key = $1`,
    [seriesKey, n],
  );
}
export async function recordCacheWarmCatnoRunSearched(seriesKey: string, n: number): Promise<void> {
  if (!Number.isFinite(n) || n <= 0) return;
  await getPool().query(
    `UPDATE cache_warm_catno_runs SET total_searched = total_searched + $2
      WHERE series_key = $1`,
    [seriesKey, n],
  );
}
export async function recordCacheWarmCatnoRunError(seriesKey: string, msg: string): Promise<void> {
  await getPool().query(
    `UPDATE cache_warm_catno_runs
        SET total_errors  = total_errors + 1,
            recent_errors = (
...
// ── External discography (xlsx / wirz / etc. — non-Discogs sources) ──

export interface ExternalDiscographyRow {
  label_name: string;
  label_id?: number | null;
  catno: string;
  catno_sort?: number | null;
  side?: string | null;       // "A" / "B" / null for LPs
  artist?: string | null;
  title?: string | null;
  year?: number | null;
  matrix?: string | null;
  ...
  notes?: string | null;
  source: string;             // "excello-xlsx-praguefrank" / "wirz.de" / …
  data?: any;                 // raw row JSON for anything unmapped
}
...
export function sanitizeLabelName(s: string | null | undefined): string {
  if (s == null) return "";
  return String(s)
    .replace(/<[^>]+>/g, "")
    .replace(/&nbsp;/g, " ")
...
function _deriveCatnoSort(catno: string | null | undefined): number | null {
  ...
  try {
    ...
    for (let off = 0; off < rows.length; off += _BULK_EXT_DISC_CHUNK) {
      ...
      for (const r of chunk) {
        ...
        args.push(
          sanitizeLabelName(r.label_name),
          r.label_id ?? null,
          r.catno,
          sort,
          r.side ?? null,
...
export async function listExternalDiscographyForLabel(opts: {
  ...
  const args: any[] = [opts.label];
  const where: string[] = [`label_name = $1`];
  ...
  const r = await getPool().query(
    `SELECT id, label_name, label_id, catno, catno_sort, side,
            artist, title, year, matrix, xref, loc, composer, notes,
            source, data
       FROM external_discography
      WHERE ${where.join(" AND ")}
      ORDER BY catno_sort ASC NULLS LAST, catno ASC, side ASC NULLS FIRST`,
    args,
  );
  return r.rows;
}

export async function countExternalDiscographyByLabel(): Promise<Array<{ label_name: string; n: number }>> {
  const r = await getPool().query(
    `SELECT label_name, COUNT(*)::int AS n
       FROM external_discography
      GROUP BY label_name
      ORDER BY n DESC`,
  );
...
export async function purgeExternalDiscographyCovered(opts: { label?: string } = {}): Promise<{ deleted: number }> {
  const args: any[] = [];
  let labelFilter = "";
  if (opts.label) {
...
export async function cleanDirtyExternalDiscographyLabelNames(): Promise<{
  renamed: number; mergedDuplicates: number; renames: Array<{ from: string; to: string; rows: number }>;
}> {
  const dirty = await getPool().query(
    `SELECT DISTINCT label_name
       FROM external_discography
      WHERE label_name ~ '<[^>]+>|\\s\\s|\n|\r|\t'`,
  );
  const renames: Array<{ from: string; to: string; rows: number }> = [];
  let renamed = 0;
  ...
  try {
    ...
    for (const r of dirty.rows) {
      ...
      const dropRes = await client.query(
        `DELETE FROM external_discography ed_new
          ...
          WHERE ed_old.label_name = $1
            ...
            AND ed_new.source = ed_old.source`,
        [oldName, newName],
      );
      mergedDuplicates += dropRes.rowCount ?? 0;
      const updRes = await client.query(
        `UPDATE external_discography SET label_name = $2 WHERE label_name = $1`,
        [oldName, newName],
      );
...
export async function isSplitCacheReaderEnabled(): Promise<boolean> {
  return false;
}
export async function setSplitCacheReaderEnabled(_v: boolean): Promise<void> {
...
// over globally.
async function _listLabelDirectoryV1Raw(
...
async function _listLabelDirectoryV2Raw(
  search: string | undefined,
  limit:  number,
): Promise<LabelDirectoryRow[]> {
  const args: any[] = [];
...
async function _collapseLabelDirectoryAliases(
  rawRows: LabelDirectoryRow[],
): Promise<LabelDirectoryRow[]> {
...
// ── Cache analytics ─────────────────────────────────────────────
//
// Faceted drilldown over release_cache. Given a combination of
// filters (label, artist, genre, style, country, year range, type),
// return the total match count plus top-N counts across every
// dimension the admin might want to slice by — genres, styles,
// labels, artists, countries, decades — and a small sample of
// matching titles for context.
//
// Under the hood: one materialised CTE that captures the filtered
// row set once, then parallel-friendly aggregations off it. Cheaper
// than N separate WHERE scans, and Postgres's JSONB indexes make
// the containment filters fast even at 150k+ rows.

export interface CacheAnalyticsFilters {
  label?:    string;
  artist?:   string;
  genre?:    string;
  style?:    string;
...
export interface CacheAnalyticsResult {
  totalCount: number;
  facets: {
    genres:    CacheAnalyticsFacet[];
    styles:    CacheAnalyticsFacet[];
    labels:    CacheAnalyticsFacet[];
    artists:   CacheAnalyticsFacet[];
    countries: CacheAnalyticsFacet[];
    decades:   Array<{ decade: number; count: number }>;
  };
  sample: Array<{
    id: number; type: string; title: string;
    artist: string; label: string; year: number | null;
  }>;
}

export async function computeCacheAnalytics(
  f: CacheAnalyticsFilters,
  opts: { forceReader?: "v1" | "v2" } = {},
...
async function _computeCacheAnalyticsV1(f: CacheAnalyticsFilters): Promise<CacheAnalyticsResult> {
  const args: any[] = [];
  const where: string[] = [];
  const push = (v: any): string => { args.push(v); return `$${args.length}`; };
...
async function _computeCacheAnalyticsV2(f: CacheAnalyticsFilters): Promise<CacheAnalyticsResult> {
  const args: any[] = [];
  const where: string[] = [];
  const push = (v: any): string => { args.push(v); return `$${args.length}`; };
...
export async function resetCacheWarmRun(genreKey: string, styleKey: string): Promise<void> {
  await getPool().query(
    `UPDATE cache_warm_runs
...
export async function deleteCacheWarmRun(genreKey: string, styleKey: string): Promise<void> {
  await getPool().query(
    `DELETE FROM cache_warm_runs WHERE genre_key = $1 AND style_key = $2`,
    [genreKey, styleKey || ""],
  );
}
...
// carry it as id.
export function searchResultMasterLabels(results: any[]): Map<number, { name: string; catno: string }> {
  const out = new Map<number, { name: string; catno: string }>();
...
export async function stampCachedMasterLabels(items: Array<{ id: number; name: string; catno?: string }>): Promise<number> {
    ...
    `UPDATE release_cache rc
      ...
      WHERE rc.type = 'master' AND rc.discogs_id = x.id
        AND jsonb_array_length(CASE WHEN jsonb_typeof(rc.data->'labels') = 'array'
                                    THEN rc.data->'labels' ELSE '[]'::jsonb END) = 0`,
    [rows.map(i => i.id), rows.map(i => i.name), rows.map(i => i.catno ?? "")],
  );
  return r.rowCount ?? 0;
}
...
export async function countMastersNeedingLabel(): Promise<number> {
  const r = await getPool().query(`SELECT COUNT(*)::int AS n FROM release_cache rc WHERE ${_MLB_NEEDS}`);
  return Number(r.rows[0]?.n ?? 0);
}

export async function listMastersNeedingLabel(afterId: number, limit: number): Promise<Array<{ id: number; mainRelease: number | null }>> {
  const r = await getPool().query(
  ...
  return (r.rows as any[]).map(x => ({ id: Number(x.id), mainRelease: x.main_release != null ? Number(x.main_release) : null }));
}

export async function markMasterLabelTried(id: number): Promise<void> {
  await getPool().query(
    `UPDATE release_cache SET data = jsonb_set(data, '{_labelTried}', to_jsonb(NOW()::text), true)
      WHERE type = 'master' AND discogs_id = $1`,
    [id],
  );
}
...
// to. No API calls.
export async function stampMasterLabelsFromCache(): Promise<number> {
  const client = await getPool().connect();
  try {
    await client.query("SET statement_timeout = 0");
    const r = await client.query(
      `UPDATE release_cache t
...
export async function runReadonlyQuery(
  ...
  try {
    ...
    if (runnerRole) {
      if (!/^[a-z_][a-z0-9_]{0,62}$/.test(runnerRole)) throw new Error("SQL_RUNNER_ROLE is not a valid role name");
