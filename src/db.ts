...
export async function getEbaySearchCache(queryKey: string): Promise<{ results: any[]; total: number; cachedAt: string } | null> {
  const r = await getPool().query(
    `SELECT results_json, total_results, cached_at
     FROM ebay_search_cache
     WHERE query_key = $1 AND cached_at > NOW() - INTERVAL '30 minutes'`,
    [queryKey]
  );
...
export async function setEbaySearchCache(queryKey: string, results: any[], total: number): Promise<void> {
  await getPool().query(
    `INSERT INTO ebay_search_cache (query_key, results_json, total_results, cached_at)
     VALUES ($1, $2, $3, NOW())
     ON CONFLICT (query_key) DO UPDATE SET
       results_json = $2, total_results = $3, cached_at = NOW()`,
    [queryKey, JSON.stringify(results), total]
  );
}

export async function pruneEbaySearchCache(): Promise<number> {
  const r = await getPool().query(
    `DELETE FROM ebay_search_cache WHERE cached_at < NOW() - INTERVAL '30 minutes'`
  );
  return r.rowCount ?? 0;
}
