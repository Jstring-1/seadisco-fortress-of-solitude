  ...
  try {
    ...
    for (const m of matches.rows) {
      ...
      const cur = await client.query(
        `SELECT COALESCE(NULLIF(data->>'year','')::int, NULL) AS year
           FROM release_cache WHERE discogs_id = $1 AND type = $2`,
        [m.discogs_id, m.type],
      );
      const oldYear = cur.rows[0]?.year != null ? Number(cur.rows[0].year) : null;
      if (oldYear && oldYear > 0) continue;   // raced — skip
      await client.query(
        ...
        [batchId, m.discogs_id, m.type, oldYear, m.new_year, m.donor_ref, m.donor_source, m.label_name, m.catno],
      );
      await client.query(
        `UPDATE release_cache
          ...
          WHERE discogs_id = $1 AND type = $2`,
        [m.discogs_id, m.type, m.new_year, m.donor_ref],
      );
      phase1Updated += 1;
    }

    // Phase 2: masters whose versions now (or already) have years.
    const masterMatches = await client.query(
      `SELECT m.discogs_id,
    ...
    for (const m of masterMatches.rows) {
      ...
      if (!Number.isFinite(newYear) || newYear <= 0) continue;
      await client.query(
        `INSERT INTO year_backfill_log
      ...
      await client.query(
        `UPDATE release_cache
          ...
          WHERE discogs_id = $1 AND type = 'master'`,
        [m.discogs_id, newYear],
      );
      phase2Updated += 1;
    }
...
export async function rollbackYearBackfillBatch(batchId: string): Promise<YearBackfillRollbackResult> {
  ...
  try {
    ...
    const logRows = await client.query(
      `SELECT id, discogs_id, type, old_year, new_year
         FROM year_backfill_log
        WHERE batch_id = $1
    ...
    for (const r of logRows.rows) {
      const cur = await client.query(
        `SELECT NULLIF(data->>'year','')::int AS year
           FROM release_cache WHERE discogs_id = $1 AND type = $2`,
        [r.discogs_id, r.type],
      );
      ...
      if (r.old_year == null) {
        ...
        await client.query(
          `UPDATE release_cache
            ...
            WHERE discogs_id = $1 AND type = $2`,
          [r.discogs_id, r.type, Number(r.old_year)],
        );
      }
      reverted += 1;
    }
