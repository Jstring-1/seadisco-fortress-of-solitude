...
app.get("/api/blues-archive/timeline", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try {
    const fromYear = Math.max(1900, Math.min(2100,
      parseInt(String(req.query.from ?? "1925"), 10) || 1925));
    const toYear = Math.max(fromYear, Math.min(2100,
      parseInt(String(req.query.to ?? "1930"), 10) || 1930));
    const limit = Math.max(1, Math.min(2000,
