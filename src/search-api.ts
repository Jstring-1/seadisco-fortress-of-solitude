...
app.post("/api/admin/query/nl", express.json({ limit: "8kb" }), async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  if (!anthropicKey) { res.status(503).json({ error: "AI not configured (ANTHROPIC_API_KEY unset)." }); return; }
  const prompt = String(req.body?.prompt ?? "").trim().slice(0, 2000);
  if (!prompt) { res.status(400).json({ error: "Empty request." }); return; }
  try {
...
- If the question implies a lot of rows, add LIMIT 1000.`;
    ...
    if (!sql) { res.status(502).json({ error: "AI returned no SQL." }); return; }
    res.json({ sql });
  } catch (err: any) {
    res.status(500).json({ error: err?.message ?? String(err) });
  }
