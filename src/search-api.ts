...
import {
  ...
  getExternalDiscographyStatus,       // used by /api/admin/workers/status aggregate
...
import {
  ...
  getBulkLabelSweepStatus,            // used by /api/admin/workers/status aggregate
  isBulkLabelSweepRunning,
} from "./label-bulk-sweep-worker.js";
import {
  initCacheProjectionBackfillModule,
  getCacheProjectionBackfillStatus,   // used by /api/admin/workers/status aggregate
  isCacheProjectionBackfillRunning,
} from "./cache-projection-backfill-worker.js";
import {
  initLabelUpstreamStatsModule,
  getLabelUpstreamStatsStatus,        // used by /api/admin/workers/status aggregate
...
let _clerkCountMemo: { n: number | null; at: number } = { n: null, at: 0 };
async function getClerkAccountCount(maxAgeMs = 60_000): Promise<number | null> {
  if (_clerkCountMemo.n != null && Date.now() - _clerkCountMemo.at < maxAgeMs) return _clerkCountMemo.n;
  ...
  try {
    ...
    if (!cr.ok) return _clerkCountMemo.n;
    const n = Number(((await cr.json()) as any)?.total_count);
    if (!Number.isFinite(n)) return _clerkCountMemo.n;
    _clerkCountMemo = { n, at: Date.now() };
    return n;
  } catch { return _clerkCountMemo.n; }
}
...
async function loggedFetch(service: string, url: string, init?: RequestInit & { context?: string; discogsPriority?: DiscogsPriority }): Promise<Response> {
  ...
  const { context: _ctx, discogsPriority: _pri, ...fetchInit } = (init ?? {}) as any;
  ...
  if (service === "discogs") {
    const auth = (fetchInit.headers as any)?.Authorization ?? (fetchInit.headers as any)?.authorization;
...
const _downloadTickets = new Map<string, { userId: string; exp: number }>();
function _mintDownloadTicket(userId: string): string {
  const now = Date.now();
  for (const [k, v] of _downloadTickets) if (v.exp <= now) _downloadTickets.delete(k);
...
async function authorizeAdminDownload(req: express.Request, res: express.Response): Promise<string | null> {
  const ticket = String(req.query.ticket ?? "");
  if (ticket) {
    const rec = _downloadTickets.get(ticket);
    _downloadTickets.delete(ticket); // single-use regardless of validity
    if (rec && rec.exp > Date.now() && rec.userId === ADMIN_CLERK_ID) return rec.userId;
...
function _peekDownloadTicket(req: express.Request): boolean {
  const ticket = String(req.query.ticket ?? "");
...
function _isTicketedDownloadReq(req: express.Request): boolean {
...
const _VIEW_META: Record<string, { title: string; desc: string; path: string }> = {
...
function _headTagsForView(view: string): string {
...
function _sendHtml(res: express.Response, relPath: string, req?: express.Request) {
...
app.get("/index.html", (req, res, next) => { if (!_sendHtml(res, "index.html", req)) next(); });
...
app.use((req, res, next) => {
  ...
  if (/\/admin-panel(\.html)?\/?$/i.test(p)) { res.status(404).end(); return; }
  next();
});
  ...
  if (req.method !== "GET" && req.method !== "HEAD" && req.method !== "OPTIONS") {
    res.on("finish", () => {
      ...
      try {
        ...
        if (raw) detail = { body: raw.length > 2000 ? raw.slice(0, 2000) + "…" : raw };
...
app.post("/api/track-yt/suggest", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  if (!ytSuggestUserLimiter.check(userId)) { res.status(429).json({ error: "Too many suggestions — try again later." }); return; }
  try {
...
app.post("/api/track-yt/suggest-batch", express.json({ limit: "64kb" }), async (req, res) => {
  ...
  for (const a of raw.slice(0, 64)) {
    ...
    if (!_TRACK_YT_RELEASE_ID_RE.test(releaseId) || !_TRACK_YT_POSITION_RE.test(trackPosition) || !/^[A-Za-z0-9_-]{11}$/.test(videoId)) continue;
    items.push({
...
app.post("/api/admin/track-yt/set", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  const albumIds      = (Array.isArray(b.albumIds) ? b.albumIds : [])
    .slice(0, 8).map((x: any) => String(x ?? "").trim()).filter((s: string) => /^\d{1,12}$/.test(s));
  if (!releaseId || !trackPosition) { res.status(400).json({ error: "Bad request" }); return; }
  ...
  try {
    ...
    let cascade: { playlistRows: number; queueRows: number } | null = null;
...
app.post("/api/youtube/report-unavailable", express.json({ limit: "1kb" }), async (req, res) => {
  ...
  if (!ytReportUserLimiter.check(userId)) { res.status(429).json({ error: "Too many reports — try again later." }); return; }
  try {
...
function _pruneScratchCaches(): void {
  pruneYoutubeSearchCache().then(n => { if (n) console.log(`[prune] youtube_search_cache: ${n} rows`); }).catch(() => {});
  pruneDiscogsSearchCache().then(n => { if (n) console.log(`[prune] discogs_search_cache: ${n} rows`); }).catch(() => {});
  pruneArchiveSearchCache().then(n => { if (n) console.log(`[prune] archive_search_cache: ${n} rows`); }).catch(() => {});
...
async function runBackgroundSync(userId: string, client: DiscogsClient, username: string, syncCollection: boolean, syncWantlist: boolean) {
  ...
  const collectionGapPageNums: number[] = [];
  const wantlistGapPageNums: number[] = [];
  let lastGapError = "";
  ...
  try {
    ...
    if (hadGaps) {
      const fmt = (nums: number[]) => nums.length <= 6 ? nums.join(", ") : `${nums.slice(0, 6).join(", ")}, +${nums.length - 6} more`;
      const parts: string[] = [];
...
app.get("/api/user/collection", async (req, res) => {
  ...
  if (req.query.genreStrict === "1" && filters.genre) { filters.genreStrict = filters.genre; delete filters.genre; }
  try {
...
let _wantedMemo: { at: number; items: object[] } | null = null;
...
app.get("/api/user/collection/export", async (req, res) => {
  ...
  try {
    ...
    if (!res.headersSent) res.status(500).json({ error: "Export failed" }); else res.end();
  }
});
...
async function _streamLibraryCsv(res: express.Response, kind: "collection" | "wantlist", userId: string, folderMap: Map<number, string> | null): Promise<void> {
  const PAGE = 2000;
...
function _csvLines(rows: any[], folderMap: Map<number, string> | null, headerOnly: boolean): string {
  const headers = ["Artist", "Title", "Label", "Cat#", "Year", "Format", "Genre", "Style", "Country"];
  if (folderMap) headers.push("Folder");
  if (headerOnly) return headers.join(",");
...
app.post("/api/user/playlists", express.json({ limit: "256kb" }), async (req, res) => {
  ...
  const cleaned = items
    ...
    .filter((it: any) => (it.source === "yt" || it.source === "loc") && it.externalId);
  if (!cleaned.length) { res.status(400).json({ error: "no valid items" }); return; }
  try {
    const id = await createPlaylist(userId, String(name ?? ""), cleaned as any);
    res.json({ id });
  } catch (e: any) {
...
app.get("/api/playlists/shared/:token", async (req, res) => {
  ...
  if (!/^[A-Za-z0-9_-]{16,64}$/.test(token)) { res.status(400).json({ error: "bad token" }); return; }
  try {
...
app.post("/api/user/playlists/:id/share", async (req, res) => {
  ...
  try {
    ...
    if (!token) { res.status(404).json({ error: "Not found or not yours" }); return; }
    res.json({ token });
  } catch (e: any) {
    res.status(500).json({ error: String(e?.message ?? e) });
  }
...
app.patch("/api/user/playlists/:id", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  try {
    ...
    if (!ok) { res.status(404).json({ error: "Not found or not yours" }); return; }
    res.json({ ok: true });
  } catch (e: any) {
    res.status(500).json({ error: String(e?.message ?? e) });
  }
...
app.put("/api/user/playlists/:id/replace", express.json({ limit: "256kb" }), async (req, res) => {
  ...
  if (!cleaned.length) { res.status(400).json({ error: "no valid items" }); return; }
  try {
    const ok = await replacePlaylistItems(id, userId, cleaned as any, typeof name === "string" ? name : undefined);
    if (!ok) { res.status(404).json({ error: "Not found or not yours" }); return; }
    res.json({ ok: true, id });
  } catch (e: any) {
    res.status(500).json({ error: String(e?.message ?? e) });
...
app.post("/api/feedback", express.json(), async (req, res) => {
  ...
  if (!feedbackUserLimiter.check(userId)) { res.status(429).json({ error: "Too much feedback — try again later." }); return; }
...
// `preferred` is NOT part of the query (any word in a YouTube query is
// required, so it can't express "favor"). It's a list of distributor /
// label terms matched against each result's channel, title and description
// AFTER the search: matches sort to the top of the review queue, and with
// autoApprovePreferred they're eligible for auto-approve like a trusted
// channel (same exact-title / duration / embeddable gate).
type YtQueryConfig = { albumTemplate: string; trackTemplate: string; noise: string; embeddableOnly: boolean; preferred: string; autoApprovePreferred: boolean };
...
function _ytParsePreferred(raw: any): string[] {
  const out: string[] = [];
...
function _ytPreferredFirst(rows: any[], terms: string[]): any[] {
  for (const r of rows) r.preferred_match = _ytPreferredMatch(terms, r.candidate_channel_title, r.candidate_title, r.candidate_description);
  const out: any[] = [];
  for (let i = 0; i < rows.length; ) {
    const key = `${rows[i].master_id}||${rows[i].track_position ?? ""}`;
    const group: any[] = [];
    while (i < rows.length && `${rows[i].master_id}||${rows[i].track_position ?? ""}` === key) group.push(rows[i++]);
...
function _ytValidateQueryConfig(raw: any): string[] {
  ...
  if (raw?.preferred != null && typeof raw.preferred !== "string") errs.push("preferred sources must be text");
  if (typeof raw?.preferred === "string" && raw.preferred.length > _YT_PREFERRED_MAX_LEN) errs.push(`preferred sources is longer than ${_YT_PREFERRED_MAX_LEN} characters`);
  if (raw?.autoApprovePreferred != null && typeof raw.autoApprovePreferred !== "boolean") errs.push("autoApprovePreferred must be true or false");
  return errs;
}
...
function _ytQuotaDayLA(d: Date = new Date()): string {
...
function _ytQuotaResetAtMs(now: Date = new Date()): number {
...
function _bumpYtQuotaPersisted(workerSearches: number, projectUnits: number): void {
  _ytQuotaUnitsToday += projectUnits;
  _ytReviewSearchesToday += workerSearches;
  if (workerSearches || projectUnits) {
    bumpReviewQuota(workerSearches, projectUnits).catch((e) =>
      console.warn("[yt-quota] persist failed:", e?.message ?? e));
  }
}
...
let _bannedChannelSet: Set<string> = new Set();
let _bannedChannelAt = 0;
const _BANNED_CACHE_TTL_MS = 60_000;
async function _getBannedChannelIds(): Promise<Set<string>> {
  if (Date.now() - _bannedChannelAt < _BANNED_CACHE_TTL_MS) return _bannedChannelSet;
  try {
...
function _ytFilterBannedBody(body: any, banned: Set<string>): any {
  if (!banned.size || !body || !Array.isArray(body.items) || !body.items.length) return body;
  const kept = body.items.filter((it: any) => !banned.has(String(it?.channelId || "")));
  if (kept.length === body.items.length) return body;
...
app.get("/api/youtube/video-info", async (req, res) => {
  ...
  try {
    ...
    if (r.status !== 400 && r.status !== 401) _bumpYtQuotaPersisted(0, 1);
    if (!r.ok) {
      const errText = await r.text().catch(() => "");
...
app.post("/api/cards/enrich", express.json({ limit: "32kb" }), async (req, res) => {
  ...
  try {
    const raw = Array.isArray(req.body?.items) ? req.body.items : [];
    const pairs = raw
      .map((it: any) => ({ id: Number(it?.id), type: String(it?.type ?? "") }))
      .filter((p: any) => Number.isFinite(p.id) && (p.type === "master" || p.type === "release"))
      .slice(0, 200);
    if (!pairs.length) { res.json({ items: [] }); return; }
    ...
    const overrideIdx = new Map<string, Map<string, string>>();
    for (const o of overrideRows) {
      const k = `${o.release_type}:${o.release_id}`;
      if (!overrideIdx.has(k)) overrideIdx.set(k, new Map());
      overrideIdx.get(k)!.set(String(o.track_position), String(o.video_id));
    }
    const items = rows.map((row: any) => {
      const d = row.data ?? {};
      const images = (Array.isArray(d.images) ? d.images.map((im: any) => im?.uri).filter(Boolean) : []).slice(0, 12);
      const tracklist = (Array.isArray(d.tracklist) ? d.tracklist : [])
        .map((t: any) => ({
          position: String(t?.position ?? ""),
          title:    String(t?.title    ?? ""),
      ...
      const videos = (Array.isArray(d.videos) ? d.videos : [])
        .map((v: any) => ({ uri: String(v?.uri ?? ""), title: String(v?.title ?? "") }))
    ...
    if (mode === "rare") {
      ...
      if (!rows.length) {
        ...
        try {
          ...
          if (uid) tasteProfile = await getOrComputeUserTasteProfile(uid);
...
async function _runPersonalSuggestionsForUser(
  ...
  if (!opts?.force) {
    ...
    if (eng.hibernated) return { saved: 0, reason: "hibernated" };
  ...
  for (const k of dismissed) excludeKeys.add(k);
  for (const k of recentlyClicked) excludeKeys.add(k);
...
async function _runSuggestionsCacheWarm(opts?: { ignoreWindow?: boolean }): Promise<void> {
  if (_suggCacheWarmState.running) return;
  ...
  try {
    ...
    while (totalDrained < _SUGG_WARM_DRAIN_MAX) {
      ...
      for (const ref of batch) {
        if (!opts?.ignoreWindow && !_inSuggWarmWindow()) break;
        totalDrained++;
        const path = ref.entity_type === "master" ? `/masters/${ref.discogs_id}` : `/releases/${ref.discogs_id}`;
        try {
          const data: any = await (client as any).get(path);
          if (data && typeof data === "object") {
            await cacheRelease(ref.discogs_id, ref.entity_type as any, data, { warmOnly: true });
...
function _maybeStartSuggCacheWarm(): void {
  if (_suggCacheWarmState.running) return;
  if (!_inSuggWarmWindow()) return;
...
app.post("/api/user/events/play", express.json({ limit: "4kb" }), async (req, res) => {
  ...
  if (!userId) { res.json({ ok: true, logged: false }); return; }
  const body = req.body ?? {};
...
app.get("/api/admin/cache-rate", async (req, res) => {
  ...
  try {
    const [totals, windows, hourly] = await Promise.all([
      getPool().query(
        `SELECT type, COUNT(*)::bigint AS n FROM release_cache GROUP BY type`,
      ),
      getPool().query(
        `SELECT
    ...
    for (const r of totals.rows) byType[r.type] = Number(r.n);
...
app.post("/api/admin/cache-fetch-queue/bootstrap", express.json({ limit: "1kb" }), async (req, res) => {
  ...
  try {
    ...
    const queued = await enqueueCacheFetches(
      refs.map(r => ({ entity_type: r.entity_type, discogs_id: r.discogs_id })),
      "bootstrap", 0,
    );
...
app.get("/api/admin/audit", async (req, res) => {
  ...
  try {
    res.json({ rows: await listAdminActions(limit, Number.isFinite(before as number) ? before : null) });
...
function _buildReleaseCacheWhere(q: any): { sql: string; args: any[] } {
  ...
  const labelsRaw = Array.isArray(q.labels)
    ? q.labels.map((s: any) => String(s ?? "").trim()).filter(Boolean)
...
function _buildSplitCacheWhere(q: any): { sql: string; args: any[] } {
  const where: string[] = [];
  const args: any[] = [];
  const type = String(q.type || "").trim().toLowerCase();
  if (type === "release" || type === "master") {
...
function _splitCacheOrderBy(q: any): string {
...
//                     before the catno is used as a sort key.
//   sortByCatnoOnly:  true → ORDER BY (normalised catno) only, no
//                     year sort first. Use for labels where the
//                     catno itself encodes chronology — without it
//                     the sort "restarts" every year boundary as the
//                     unknown-year cluster comes first then 1953
...
function _labelOrderBy(label: string): string {
  const rule = LABEL_CATNO_RULES[label];
...
function _stripLabelPrefixesJS(label: string, catno: string): string {
  const rule = LABEL_CATNO_RULES[label];
  if (!rule?.stripPrefixes?.length) return catno || "";
  for (const p of rule.stripPrefixes) {
    if ((catno || "").startsWith(p)) return catno.slice(p.length);
  }
  return catno || "";
}
function _catnoSortNum(label: string, catno: string | null | undefined): number | null {
  const stripped = _stripLabelPrefixesJS(label, String(catno || ""));
  const m = stripped.match(/^(\d+)/);
  return m ? Number(m[1]) : null;
}

// In-memory cache for the merged carousel — keyed on label + filters,
// 60s TTL. Pagination is in-memory (slice) so repeat page fetches with
// the same filters reuse this without re-running SQL.
interface LabelsCarouselCacheEntry {
  ts: number;
  items: any[];
  yearAnchors: Array<{ year: number; count: number }>;
  total: number;
  cacheCount: number;
  externalCount: number;
  externalSources: string[];
}
const _LABELS_CAROUSEL_CACHE = new Map<string, LabelsCarouselCacheEntry>();
...
function _invalidateLabelsCarouselCache(label?: string): void {
  if (!label) { _LABELS_CAROUSEL_CACHE.clear(); return; }
  for (const k of _LABELS_CAROUSEL_CACHE.keys()) {
    if (k.includes(`"label":"${label}"`)) _LABELS_CAROUSEL_CACHE.delete(k);
  }
}
...
app.post("/api/admin/external-discography/import", express.json({ limit: "8mb" }), async (req, res) => {
  ...
  if (!rows.length) { res.status(400).json({ error: "rows[] empty" }); return; }
  if (rows.length > 50_000) { res.status(400).json({ error: "rows[] too large (max 50k per request)" }); return; }
  try {
    const payload: ExternalDiscographyRow[] = rows.map((r: any) => ({
      label_name: label,
      label_id:   Number.isFinite(labelId) ? (labelId as number) : null,
      ...
      source,
      data:       r.data ?? null,
    })).filter((r: ExternalDiscographyRow) => r.catno);
...
app.post("/api/admin/label-directory/merge", express.json({ limit: "4kb" }), async (req, res) => {
  ...
  if (!fromLabel || !toLabel) { res.status(400).json({ error: "fromLabel and toLabel required" }); return; }
  try {
...
app.post("/api/admin/label-directory/set-alias", express.json({ limit: "4kb" }), async (req, res) => {
  ...
  if (!Number.isFinite(aliasId) || aliasId <= 0)         { res.status(400).json({ error: "aliasLabelId required" }); return; }
  if (!Number.isFinite(canonicalId) || canonicalId <= 0) { res.status(400).json({ error: "canonicalLabelId required" }); return; }
  try {
...
app.get("/api/admin/label-directory/discogs-search", async (req, res) => {
  ...
  const q = String(req.query.q ?? "").trim().slice(0, 200);
  if (!q) { res.status(400).json({ error: "q required" }); return; }
  if (!ADMIN_CLERK_ID) { res.status(503).json({ error: "Admin Clerk id not configured" }); return; }
  try {
    const oauth = await getOAuthCredentials(ADMIN_CLERK_ID);
    if (!oauth) { res.status(503).json({ error: "Admin Discogs OAuth not connected" }); return; }
    if (!discogsConsumerKey || !discogsConsumerSecret) {
    ...
    const sr: any = await client.search(q, { type: "label", perPage: 10 });
...
app.post("/api/admin/cache-analytics", express.json({ limit: "8kb" }), async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  const b = req.body || {};
  const trim = (v: any): string | undefined => {
  ...
  const filters: CacheAnalyticsFilters = {
    label:    trim(b.label),
...
app.get("/api/admin/release-cache/preview", async (req, res) => {
  ...
  try {
    ...
    if (useV2) {
      ...
      const r = await getPool().query(
        `${CACHE_ALL_CTE} SELECT COUNT(*)::bigint AS n FROM cache_all ca ${sql}`,
        args,
      );
  ...
  try {
    ...
    if (useV2) {
      ...
      try {
        ...
        const keys = await clientDel.query(
          `${CACHE_ALL_CTE}
           SELECT discogs_id, type, bucket FROM cache_all ca ${whereSql}`,
          args,
        );
        if (keys.rows.length > 0) {
          ...
          const dr1 = await clientDel.query(
            `DELETE FROM discogs_cache_masters_plus mp
              USING unnest($1::int[], $2::text[]) AS t(id, ty)
              WHERE mp.discogs_id = t.id AND mp.type = t.ty`,
            [ids, types],
          );
          // pressings: keyed by discogs_id (type is always 'release')
          const dr2 = await clientDel.query(
            `DELETE FROM discogs_cache_pressings p
              USING unnest($1::int[], $2::text[]) AS t(id, ty)
              WHERE p.discogs_id = t.id AND t.ty = 'release'`,
            [ids, types],
          );
          const dr3 = await clientDel.query(
            `DELETE FROM release_cache rc
              USING unnest($1::int[], $2::text[]) AS t(id, ty)
              WHERE rc.discogs_id = t.id AND rc.type = t.ty`,
            [ids, types],
          );
          // Side tables cascade via matching (discogs_id, bucket).
          const buckets = keys.rows.map(r => Number(r.bucket));
          await clientDel.query(
            `DELETE FROM release_labels rl
              USING unnest($1::int[], $2::int[]) AS t(id, b)
              WHERE rl.discogs_id = t.id AND rl.bucket = t.b`,
            [ids, buckets],
          );
          await clientDel.query(
            `DELETE FROM release_artists ra
              USING unnest($1::int[], $2::int[]) AS t(id, b)
              WHERE ra.discogs_id = t.id AND ra.bucket = t.b`,
            [ids, buckets],
          );
          await clientDel.query(
            `DELETE FROM release_tags rt
              USING unnest($1::int[], $2::int[]) AS t(id, b)
              WHERE rt.discogs_id = t.id AND rt.bucket = t.b`,
            [ids, buckets],
          );
        ...
        try { await clientDel.query("ROLLBACK"); } catch {}
        throw e;
      } finally {
        clientDel.release();
      }
    ...
    const r = await getPool().query(
      `DELETE FROM release_cache rc ${whereSql}`,
      args,
    );
    ...
    for (let offset = 0; offset < limit; offset += CHUNK) {
      ...
      for (const row of r.rows) {
        if (format === "csv") {
          ...
          const labels  = Array.isArray(row.labels_j)  ? row.labels_j.map((l: any) => l?.name).filter(Boolean).join(" | ") : null;
          const catno   = Array.isArray(row.labels_j)  ? row.labels_j.map((l: any) => l?.catno).filter(Boolean).join(" | ") : null;
...
app.get("/api/admin/release-cache/dump-v1", async (req, res) => {
  if (!await authorizeAdminDownload(req, res)) return;
  const CHUNK = 5000;
  const date = new Date().toISOString().slice(0, 10);
  res.setHeader("Content-Type", "application/x-ndjson; charset=utf-8");
...
const _DUMP_REDACTED_COLUMNS: Record<string, Set<string>> = {
  ...
  const dumpTable = async (table: string): Promise<void> => {
    ...
    while (true) {
      ...
      for (const row of r.rows) {
        ...
        for (const [k, v] of Object.entries(row)) {
          if (k === "ctid") continue;
          if (redact?.has(k) && v != null && v !== "" && v !== "__oauth__") { clean[k] = "[redacted]"; continue; }
          clean[k] = v instanceof Date ? v.toISOString() : v;
        }
        res.write(JSON.stringify(clean) + "\n");
        written++;
...
app.get("/api/admin/lyrics/export.doc", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="seadisco-lyrics-${new Date().toISOString().slice(0, 10)}.doc"`,
    );
    // ── Cover ────────────────────────────────────────────────────────
    res.write(
      "Blues Archive\n" +
...
app.get("/api/admin/lyrics/export.pdf", async (req, res) => {
  ...
  try {
    ...
    res.setHeader(
      "Content-Disposition",
      `attachment; filename="seadisco-lyrics-${new Date().toISOString().slice(0, 10)}.pdf"`,
    );
...
app.post("/api/blues-archive/tunings/bulk-update-position", express.json({ limit: "256kb" }), async (req, res) => {
  ...
  try {
    ...
    if (!idsRaw.length) { res.status(400).json({ error: "ids required" }); return; }
    if (idsRaw.length > 10000) { res.status(413).json({ error: "too many ids (max 10000 per call)" }); return; }
...
app.post("/api/admin/lyrics/bans", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  try {
    ...
    if (kind === "body_hash") {
      ...
      if (!row) { res.status(404).json({ error: "lyric not found" }); return; }
...
function _invalidateCwStats(): void {
  _cwStatsCache = null;
...
const _CW_CANONICAL_STYLES: Record<string, Set<string>> = {
...
async function _computeCwStats(): Promise<{ rows: any[]; release_cache_total: number }> {
  ...
  const rows = gridR.rows.filter((r: any) =>
    r.has_run || _isCanonicalCombo(String(r.genre_key ?? ""), String(r.style_key ?? "")),
  );
...
async function _getCwStats(): Promise<{ rows: any[]; release_cache_total: number }> {
  ...
  if (_cwStatsInflight) return _cwStatsInflight;
  const startedAt = Date.now();
  _cwStatsInflight = (async () => {
    try {
      const out = await _computeCwStats();
...
app.post("/api/admin/cache-warm-runs/start", express.json({ limit: "4kb" }), async (req, res) => {
  ...
  if (!genreKey) { res.status(400).json({ error: "genreKey required" }); return; }
  try {
...
app.post("/api/admin/cache-warm-runs/start-batch", express.json({ limit: "16kb" }), async (req, res) => {
  ...
  const num = (v: any) => { const n = parseInt(String(v ?? ""), 10); return Number.isFinite(n) ? n : undefined; };
  const combos = raw
    .map((c: any) => ({
    ...
    .filter((c: any) => c.genreKey);
  if (!combos.length) { res.status(400).json({ error: "no combos" }); return; }
  try {
...
app.get("/api/admin/cache-warm-runs/queue", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try { res.json({ queue: await getCacheWarmQueue() }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
app.post("/api/admin/cache-warm-runs/queue/clear", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try { await clearCacheWarmQueue(); _invalidateCwStats(); res.json({ ok: true }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
...
app.get("/api/admin/cache-warm-catno/status", async (req, res) => {
  ...
  try {
    ...
    for (const r of rows) rowsByKey.set(r.series_key, r);
...
app.post("/api/admin/cache-warm-catno/stop", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try { requestCacheWarmCatnoStop(); res.json({ ok: true }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});

app.post("/api/admin/cache-warm-catno/force-clear", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try { forceClearCacheWarmCatnoRunning(); res.json({ ok: true }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
...
app.post("/api/admin/cache-warm-catno/reset", express.json({ limit: "1kb" }), async (req, res) => {
  ...
  const seriesKey = String(req.body?.seriesKey ?? "").trim();
  if (!seriesKey) { res.status(400).json({ error: "seriesKey required" }); return; }
  try { await resetCacheWarmCatnoRun(seriesKey); res.json({ ok: true }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
...
function _lyricsBodyHash(plaintext: string | null | undefined): string {
...
async function _lyricsRefreshRecentRun(opts: {
  ...
  try {
    ...
    const bannedBodyHashes = await getBannedLyricBodyHashSet().catch(() => new Set<string>());
...
function _ytAutoApproveVerdict(args: {
  ...
  // Result names a preferred source and auto-approve of those is on.
  // Held to the same checks as a trusted channel.
  ...
  try {
    while (!_ytReviewStopRequested) {
      ...
      const [existingOverrides, searchedPositions] = await Promise.all([
        ...
        getSearchedTrackPositions(masterId).catch(() => new Set<string>()),
      ]);
      ...
      for (const tr of tracklist) {
        ...
        if (!pos || !tTitle) continue;
        if (existingOverrides.has(pos)) continue;
        if (searchedPositions.has(pos)) continue;
      ...
      const matchesByTrack: Map<string, { videoId: string; sn: any; pref: string | null }[]> = new Map();
      for (const it of items) {
      ...
      if (!result.cached) await new Promise(r => setTimeout(r, _YT_REVIEW_THROTTLE_MS));
...
app.delete("/api/admin/yt-review/bans", express.json({ limit: "1kb" }), async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  const channelId = String(req.body?.channelId ?? "").trim();
  if (!channelId) { res.status(400).json({ error: "bad_request" }); return; }
  try {
...
app.post("/api/admin/yt-review/quota/reset", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  if (_ytReviewRunning) { res.status(409).json({ error: "stop the worker first" }); return; }
  try {
    _ytQuotaUnitsToday = 0;
    _ytReviewSearchesToday = 0;
...
app.post("/api/admin/yt-review/dismiss-pending", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  if (_ytReviewRunning) { res.status(409).json({ error: "cannot dismiss while worker is running — stop it first" }); return; }
  try {
...
app.post("/api/admin/yt-review/unreject", express.json({ limit: "1kb" }), async (req, res) => {
  ...
  try {
    if (!await reviewQueueUnreject(id)) { res.status(404).json({ error: "not_found_or_not_rejected" }); return; }
    res.json({ ok: true });
  } catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
...
app.post("/api/admin/yt-review/reject-track", express.json({ limit: "1kb" }), async (req, res) => {
  ...
  if (!Number.isFinite(masterId)) { res.status(400).json({ error: "bad_request" }); return; }
  try {
...
app.post("/api/admin/yt-review/delete-approval", express.json({ limit: "4kb" }), async (req, res) => {
  ...
  try {
    ...
    if (!out.ok) { res.status(404).json({ error: "not_found_or_not_approved" }); return; }
...
app.post("/api/admin/lyrics", express.json({ limit: "200kb" }), async (req, res) => {
  ...
  try {
    ...
    if (!pageTitle) { res.status(400).json({ error: "page_title required" }); return; }
...
function _validateReadonlySql(raw: string): { ok: true; sql: string } | { ok: false; error: string } {
  ...
  sql = sql.replace(/;\s*$/, "");
  if (sql.includes(";")) return { ok: false, error: "Only one statement is allowed (remove the ';')." };
...
function _csvCell(v: any): string {
...
app.get("/api/blues-archive/favorites", async (req, res) => {
  ...
  try { res.json({ rows: await listLyricFavoritesWithDetails(userId) }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
app.get("/api/blues-archive/favorites/ids", async (req, res) => {
  const userId = await requireAdmin(req, res); if (!userId) return;
  try { res.json({ ids: await listLyricFavoriteIds(userId) }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
app.post("/api/blues-archive/favorites", express.json({ limit: "1kb" }), async (req, res) => {
  ...
  try {
    ...
    if (!Number.isFinite(lyricId)) { res.status(400).json({ error: "lyricId required" }); return; }
...
app.get("/api/blues-archive/setlists", async (req, res) => {
  const userId = await requireAdmin(req, res); if (!userId) return;
  try { res.json({ rows: await listSetlists(userId) }); }
  catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
app.post("/api/blues-archive/setlists", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  try {
    ...
    if (err?.code === "23505") { res.status(409).json({ error: "A setlist with that name already exists." }); return; }
    res.status(500).json({ error: err?.message ?? String(err) });
  }
});
...
app.put("/api/blues-archive/setlists/:id(\\d+)/items", express.json({ limit: "32kb" }), async (req, res) => {
  const userId = await requireAdmin(req, res); if (!userId) return;
  try {
    const items: Array<{ lyricId: number; sort_order: number }> = Array.isArray(req.body?.items)
      ? req.body.items.map((it: any) => ({ lyricId: Number(it.lyricId), sort_order: Number(it.sort_order) }))
                      .filter((it: any) => Number.isFinite(it.lyricId))
      : [];
...
app.post("/api/blues-archive/check", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  try {
    ...
    void artistIds; void releaseId; void masterId;
      ...
      if (artistNamesNorm.length) {
        const r = await getPool().query(
          `SELECT id, page_title, artist, artist_id, first_release_year
             FROM blues_lyrics
            WHERE LOWER(TRIM(page_title)) = ANY($1::text[])
              AND regexp_replace(LOWER(COALESCE(artist, '')), '[^a-z0-9]', '', 'g') = ANY($2::text[])`,
          [titlesLc, artistNamesNorm],
        );
        for (const row of r.rows) {
          const k = String(row.page_title).trim().toLowerCase();
          if (!out.tracks[k]) out.tracks[k] = { id: row.id, page_title: row.page_title, artist: row.artist, artist_id: row.artist_id, first_release_year: row.first_release_year };
...
app.get("/search", async (req, res) => {
  ...
  const rawBarcode = (req.query.barcode as string) ?? "";
  const searchBarcode: string | undefined = rawBarcode.trim() || undefined;
...
app.get("/artist-releases", async (req, res) => {
  ...
  const sortOrderRaw = (req.query.sort_order as string) || "";
  ...
  const roleParam = (req.query.role as string) || "main";
  ...
  try {
    ...
    const releases: any[] = data?.releases ?? [];
    let items = releases;
    if (onlyMain) items = items.filter(r => (r?.role ?? "Main") === "Main");
    if (typeParam === "master")  items = items.filter(r => r?.type === "master");
    if (typeParam === "release") items = items.filter(r => r?.type === "release");
    ...
    const reshaped = items.map(r => {
      ...
      const formatArr = Array.isArray(fmt)
        ? fmt
        : (typeof fmt === "string" && fmt ? fmt.split(",").map((s: string) => s.trim()).filter(Boolean) : []);
      const lbl = r?.label ?? "";
      const labelArr = Array.isArray(lbl)
        ? lbl
        : (typeof lbl === "string" && lbl ? lbl.split(",").map((s: string) => s.trim()).filter(Boolean) : []);
...
app.get("/api/admin/api-health", async (req, res) => {
  ...
  try {
    ...
    const limiters: any = {};
    try { limiters.loc = locLimiter.getStats(); } catch {}
...
async function _ylGate(req: express.Request, res: express.Response): Promise<boolean> {
  const userId = await getClerkUserId(req);
  const isAdmin = !!userId && !!ADMIN_CLERK_ID && userId === ADMIN_CLERK_ID;
  if (isAdmin) return true;
  if (!_yearLabelOpen || !userId) { res.status(userId ? 403 : 401).json({ error: "Not available" }); return false; }
  if (!_yearLabelUserLimiter.check(userId)) { res.status(429).json({ error: "Too many requests; try again in a few minutes" }); return false; }
  return true;
}
function _ylFilter(q: any): YearLabelFilter {
...
const _ylKey = (f: YearLabelFilter) => `${f.genre ?? "*"}|${f.strict && f.genre ? 1 : 0}`;
const _ylYear = (v: any): number | null => (/^(1[0-9]|20)[0-9]{2}$/.test(String(v ?? "")) ? Number(v) : null);
...
const _ylPairsMemo = new Map<string, { at: number; pairs: Array<[number, string, number]> }>();
const _ylPairsInflight = new Map<string, Promise<Array<[number, string, number]>>>();
async function _ylPairs(f: YearLabelFilter, refresh: boolean): Promise<Array<[number, string, number]>> {
  const key = _ylKey(f);
  const hit = _ylPairsMemo.get(key);
  if (hit && !refresh && Date.now() - hit.at < 30 * 60_000) return hit.pairs;
  ...
  while (_ylPairsMemo.size > 4) _ylPairsMemo.delete(_ylPairsMemo.keys().next().value as string);
  return pairs;
}
const _ylAlbumsMemo = new Map<string, { at: number; v: any }>();

app.get("/api/year-label/years", async (req, res) => {
  if (!await _ylGate(req, res)) return;
...
app.get("/api/year-label/labels", async (req, res) => {
  ...
  if (req.query.year != null && year == null) { res.status(400).json({ error: "bad year" }); return; }
  try {
...
app.get("/api/year-label/albums", async (req, res) => {
  ...
  if (year == null) { res.status(400).json({ error: "year required" }); return; }
  try {
    ...
    if (!v) {
      ...
      while (_ylAlbumsMemo.size > 200) _ylAlbumsMemo.delete(_ylAlbumsMemo.keys().next().value as string);
    }
    res.set("Cache-Control", "no-store");
...
app.get("/api/admin/media-stats", async (req, res) => {
  ...
  try {
    ...
    const feature7d = want
      .map(s => {
        const row = (svc as any[]).find(r => r.service === s);
  ...
  async function deleteIdleUnconnectedUsers(): Promise<number> {
    ...
    if (complete) {
      try {
        ...
        for (const row of ghostQ.rows as Array<{ clerk_user_id: string }>) {
          const id = row.clerk_user_id;
          if (!id || connected.has(id) || exempt.has(id) || clerkIds.has(id)) continue;
          await deleteUserData(id).catch(() => {});
          deleted++;
        }
      } catch (e) { console.error("[sync-schedule] orphan sweep error:", e); }
    }
    if (deleted) _clerkCountMemo.at = 0; // spots freed — refresh the cap check
    return deleted;
  }
  ...
  if (process.env.APP_DB_URL) {
    ...
    setInterval(async () => {
      try {
        ...
        if (freed.length) console.log(`Sync watchdog: reset ${freed.length} orphaned syncing row(s): ${freed.join(", ")}`);
      } catch (e) { console.error("Sync watchdog error:", e); }
    }, 5 * 60 * 1000).unref();
    ...
    // few seconds of boot.
    try { await _dbReady; } catch { /* logged in _dbReady */ }
    ...
    const FEED_POOL_MODES: Array<"feed"|"rare"|"dig"|"active"|"played"> = ["feed","rare","dig","active","played"];
    const tick = async () => {
      for (const mode of FEED_POOL_MODES) {
        try { await refreshFeedPool(mode); }
        catch (e) { console.error(`[feed-pool refresh ${mode}]`, e); }
      }
    };
