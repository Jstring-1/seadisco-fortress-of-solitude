...
import {
  ...
  getArtistSweepStatus,               // used by /api/admin/workers/status aggregate
...
app.get("/api/admin/labels/releases", async (req, res) => {
  ...
  const where: string[] = [`rc.data->'labels' @> $${args.length}::jsonb`];
  // Type filter modes:
  ...
  try {
    ...
    if (!cached || Date.now() - cached.ts > _LABELS_CAROUSEL_TTL_MS) {
      ...
      const cacheCatnoSorts = new Set<number>();
      for (const it of cacheItems) {
        const n = _catnoSortNum(label, it.catno);
        if (n != null) cacheCatnoSorts.add(n);
      }
      ...
      for (const r of externalRows) {
        ...
        if (sortKey != null && cacheCatnoSorts.has(sortKey)) continue; // covered by cache
        ...
        if (!g) {
          g = {
            ...
            _externalArtists: new Set<string>(),
            _catnoSort: sortKey,
          };
          grouped.set(groupKey, g);
        }
        if (r.artist) (g._externalArtists as Set<string>).add(String(r.artist));
        g.tracklist.push({
        ...
        if (r.year != null && (g.year == null || Number(r.year) < g.year)) g.year = Number(r.year);
      }

      const externalItems = Array.from(grouped.values()).map(g => {
        const artists = Array.from(g._externalArtists as Set<string>);
        ...
        if (!g.artist) g.artist = artists.length === 1 ? artists[0] : artists.length > 1 ? "Various" : "";
        // Title: prefer the first non-empty side title; many xlsx rows
        // already give us a sensible per-side title.
        if (!g.title) g.title = (g.tracklist[0]?.title || "");
        return g;
      });
      ...
      const externalSources = Array.from(new Set(
        externalItems.map(it => it._externalSource).filter(Boolean),
      )) as string[];
      ...
      if (_LABELS_CAROUSEL_CACHE.size >= _LABELS_CAROUSEL_MAX) {
        ...
        if (oldest !== undefined) _LABELS_CAROUSEL_CACHE.delete(oldest);
      }
...
app.post("/api/admin/blues/bulk-delete", express.json({ limit: "256kb" }), async (req, res) => {
  ...
  try {
    ...
    if (idsRaw.length > 10_000) { res.status(400).json({ error: "too many ids (max 10000)" }); return; }
    const ids = idsRaw.map((n: any) => Number(n)).filter((n: number) => Number.isFinite(n) && n > 0);
...
app.post("/api/admin/blues/:id(\\d+)/links", express.json({ limit: "8kb" }), async (req, res) => {
  ...
  if (!Number.isFinite(other) || other === id) { res.status(400).json({ error: "bad otherId" }); return; }
  if (!BLUES_ARTIST_LINK_KINDS.includes(kind as any)) {
    res.status(400).json({ error: "bad kind" });
    return;
  }
  try {
    await addBluesArtistLink(id, other, kind as any);
    res.json({ ok: true });
  } catch (err: any) { res.status(500).json({ error: err?.message ?? String(err) }); }
});
...
app.post("/api/admin/blues/discogs-preview-by-id", async (req, res) => {
  ...
  if (!Number.isFinite(did) || did <= 0) { res.status(400).json({ error: "bad id" }); return; }
  try {
...
app.put("/api/admin/blues-words/:headword", express.json({ limit: "500kb" }), async (req, res) => {
  ...
  if (!original) { res.status(400).json({ error: "headword required" }); return; }
  const headword = String(req.body?.headword || "").trim().toLowerCase();
  if (!headword) { res.status(400).json({ error: "new headword required" }); return; }
  ...
  const source_pages = Array.isArray(req.body?.source_pages)
    ? req.body.source_pages.map((n: any) => parseInt(String(n), 10)).filter((n: number) => Number.isFinite(n))
    : null;
  const citations: BluesWordCitationInput[] = Array.isArray(req.body?.citations)
    ? req.body.citations.map((c: any, i: number) => ({
        position:   Number.isFinite(c?.position) ? c.position : i + 1,
        quote:      typeof c?.quote === "string" ? c.quote : null,
        artist:     typeof c?.artist === "string" ? c.artist : null,
        song_title: typeof c?.song_title === "string" ? c.song_title : (typeof c?.song === "string" ? c.song : null),
  ...
  try {
    ...
    if (/already exists/.test(msg)) { res.status(409).json({ error: msg }); return; }
