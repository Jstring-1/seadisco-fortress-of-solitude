...
// POST /api/blues-archive/prune-post-1970 — deletes blues_artists rows
// whose earliest strict-Blues master in release_cache is after 1970.
// Only touches rows with no manually-curated data, so hand-edited
// entries (and artists with unknown years) are preserved.
app.post("/api/blues-archive/prune-post-1970", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try {
    const r = await pruneBluesArtistsFirstStrictAfter1970();
    console.log(`[blues-archive] prune-post-1970: removed=${r.removed}`);
    res.json({ ok: true, ...r });
  } catch (err: any) {
    console.error("[blues-archive prune-post-1970]", err);
    res.status(500).json({ error: err?.message ?? String(err) });
  }
});
