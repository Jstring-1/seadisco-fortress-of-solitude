...
// POST /api/blues-archive/pad/strict — populate blues_artists with
// every artist who has at least one strictly-Blues master in
// release_cache (genres = ['Blues'] exactly). Refreshes
// seed_strict_count on every match so the filter chip stays accurate.
app.post("/api/blues-archive/pad/strict", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try {
    const r = await padBluesArtistsStrict();
    console.log(`[blues-archive] pad/strict: scanned=${r.scanned}, inserted=${r.inserted}, refreshed=${r.refreshed}`);
    res.json({ ok: true, ...r });
  } catch (err: any) {
    console.error("[blues-archive pad/strict]", err);
    res.status(500).json({ error: err?.message ?? String(err) });
  }
});

// POST /api/all-blues/prune-non-strict — drop every Constellations
// queue artist (and their links) who isn't a primary credit on at
// least one strict-Blues master. Busts the graph cache afterwards.
app.post("/api/all-blues/prune-non-strict", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try {
    const r = await pruneAllBluesQueueToStrict();
    console.log(`[all-blues] prune-non-strict: artistsRemoved=${r.artistsRemoved}, linksRemoved=${r.linksRemoved}`);
    try {
    ...
    console.error("[all-blues prune-non-strict]", err);
    res.status(500).json({ error: err?.message ?? String(err) });
  }
});
