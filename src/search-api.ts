...
// POST /api/blues-archive/pad/strict-pre-1970 — re-pad with strict-Blues
// artists whose earliest master is in or before 1970. Inserts are tagged
// enrichment_status.source = "strict_pad_pre1970" so future cleanup
// can target them precisely.
app.post("/api/blues-archive/pad/strict-pre-1970", async (req, res) => {
  if (!await requireAdmin(req, res)) return;
  try {
    const r = await padBluesArtistsStrictPre1970();
    console.log(`[blues-archive] pad/strict-pre-1970: scanned=${r.scanned}, inserted=${r.inserted}, refreshed=${r.refreshed}`);
    res.json({ ok: true, ...r });
  } catch (err: any) {
    console.error("[blues-archive pad/strict-pre-1970]", err);
    res.status(500).json({ error: err?.message ?? String(err) });
  }
});
