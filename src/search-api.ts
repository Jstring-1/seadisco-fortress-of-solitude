...
app.get("/api/admin/labels/releases", async (req, res) => {
  ...
  try {
    ...
    const [pageRes, anchorsRes, countRes] = await Promise.all([
      ...
      getPool().query(
        `SELECT COALESCE(NULLIF(rc.data->>'year','')::int, 0) AS year, COUNT(*)::int AS n
           FROM release_cache rc
           ${whereSql}
          GROUP BY year
          ORDER BY year ASC`,
      ...
      getPool().query(
        `SELECT COUNT(*)::bigint AS n FROM release_cache rc ${whereSql}`,
        args,
      ),
    ]);
