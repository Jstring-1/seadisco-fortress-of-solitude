...
export function isApiKilled(): boolean {
  return _killed;
}

export function setApiKilled(v: boolean): void {
  _killed = !!v;
}

// Throw before an outgoing request when the switch is on.
export function assertApiAllowed(service: string): void {
  if (_killed) throw new Error(`API kill switch is active — ${service} request blocked`);
}
