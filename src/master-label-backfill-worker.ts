...
import {
  getAppSetting,
  setAppSetting,
  countMastersNeedingLabel,
  listMastersNeedingLabel,
  markMasterLabelTried,
  stampMasterLabelsFromCache,
...
interface State {
  startedAt:  string;
  cursor:     number;   // last master id processed
  total:      number;   // masters still needing a label when the fetch pass began
  fromCache:  number;   // stamped by the free pass
  fetched:    number;   // stamped from a fetched main release
  noLabel:    number;   // main release had no label / 404 / no main release
  errors:     number;
  processed:  number;
  lastError?: string | null;
  done?:      boolean;
  stopped?:   boolean;  // admin pressed Stop — don't auto-resume on boot
}
...
async function _persist() {
  ...
  catch (err) { console.error("[master-labels] persist failed:", err); }
}
async function _load(): Promise<State | null> {
  try {
    ...
    return p && typeof p.cursor === "number" ? p as State : null;
  } catch { return null; }
}
...
export function requestMasterLabelBackfillStop(): void {
  if (_running) _stopRequested = true;
  if (_state) { _state.stopped = true; _persist().catch(() => {}); }
}

export async function startMasterLabelBackfill(): Promise<{ ok: boolean; error?: string }> {
  ...
  if (isApiKilled()) return { ok: false, error: "API kill switch is on" };
...
async function _run(client: DiscogsClient, freshStart: boolean): Promise<void> {
  try {
    if (freshStart) {
    ...
    while (!_stopRequested) {
      ...
      if (!batch.length) { _state!.done = true; break; }
      for (const m of batch) {
        if (_stopRequested) break;
        if (isApiKilled()) { _state!.lastError = "Paused: API kill switch is on"; _stopRequested = true; break; }
        try {
          if (m.mainRelease == null) {
            await markMasterLabelTried(m.id);
            _state!.noLabel++;
          } else {
            const rel: any = await retryTransient(
              () => client.getRelease(m.mainRelease!),
              { label: `master-labels release=${m.mainRelease}` },
            );
            const lbl = Array.isArray(rel?.labels) ? rel.labels.find((l: any) => String(l?.name ?? "").trim()) : null;
            if (lbl) {
        ...
        _state!.cursor = m.id;
        _state!.processed++;
        if (_state!.processed % 10 === 0) await _persist();
    ...
    _running = false;
    _stopRequested = false;
    if (_state?.done) console.log(`[master-labels] done; cache=${_state.fromCache} fetched=${_state.fetched} noLabel=${_state.noLabel} errors=${_state.errors}`);
    await _persist();
  }
}

export function initMasterLabelBackfillModule(adminClerkId: string): void {
  _adminClerkId = adminClerkId || null;
  setTimeout(async () => {
    if (_running) return;
    const persisted = await _load();
    if (!persisted || persisted.done || persisted.stopped) { _state = persisted; return; }
    const client = await _adminClient();
    if (!client || isApiKilled()) { _state = persisted; return; }
    _state = persisted;
    _running = true;
    _stopRequested = false;
