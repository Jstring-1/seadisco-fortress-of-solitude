...
class SimpleClientsStore implements OAuthRegisteredClientsStore {
  ...
  async registerClient(metadata: Omit<OAuthClientInformationFull, "client_id" | "client_id_issued_at">) {
...
class SimpleOAuthProvider implements OAuthServerProvider {
  readonly clientsStore = new SimpleClientsStore();
  private codes = new Map<string, { client: OAuthClientInformationFull; params: AuthorizationParams }>();
  private tokens = new Map<string, AuthInfo & { expiresAt: number }>();
  ...
  async exchangeAuthorizationCode(client: OAuthClientInformationFull, code: string) {
    const data = this.codes.get(code);
    if (!data) throw new Error("Invalid authorization code");
    if (data.client.client_id !== client.client_id) throw new Error("Code not issued to this client");
  ...
  async revokeToken(_client: OAuthClientInformationFull, req: OAuthTokenRevocationRequest) {
...
function buildMcpServer() {
  ...
  server.tool(
    ...
    { release_id: z.union([z.number().int(), z.string()]).describe("Discogs release ID (numeric)") },
    async ({ release_id }) => {
