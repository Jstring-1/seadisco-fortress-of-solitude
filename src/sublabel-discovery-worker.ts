...
interface QueueItem { labelId: number; labelName: string; }
interface State {
  queue:       QueueItem[];
  cursor:      number;
  minCount:    number;
...
async function _persist() {
  ...
  catch (err) { console.error("[sublabel-discovery] persist failed:", err); }
}
...
export function isSublabelDiscoveryRunning(): boolean { return _running; }
...
export function requestSublabelDiscoveryStop(): void {
  if (_running) _stopRequested = true;
}
export function forceClearSublabelDiscovery(): void {
...
export async function startSublabelDiscovery(opts: {
  ...
  if (_running) return { ok: false, error: "Sublabel discovery already running" };
  ...
  const queue: QueueItem[] = rows
    .filter(r => Number.isFinite(r.label_id as number) && (r.label_id ?? 0) > 0 && (r.external_count ?? 0) >= minCount)
    .map(r => ({ labelId: r.label_id as number, labelName: r.label_name }));
  if (queue.length === 0) {
...
async function _run(client: DiscogsClient): Promise<void> {
  try {
    ...
    _running       = false;
    _stopRequested = false;
    if (drained) console.log(`[sublabel-discovery] drained; discovered=${_state?.discovered} inserted=${_state?.inserted} errors=${_state?.errors}`);
    await _persist();
  }
}
...
export function initSublabelDiscoveryModule(adminClerkId: string): void {
  ...
  setTimeout(async () => {
    ...
    const client = await _adminClient();
    if (!client) { console.warn(`[sublabel-discovery] boot-resume: no admin client, skipping`); return; }
    _state         = persisted;
    _running       = true;
    _stopRequested = false;
