...
async function _persist() {
  ...
  catch (err) { console.error("[master-versions-walk] persist failed:", err); }
}
async function _load(): Promise<State | null> {
  try {
    ...
    return p && Array.isArray(p.masterIds) ? p as State : null;
  } catch { return null; }
}
...
export function isMasterVersionsWalkRunning(): boolean { return _running; }
...
export function requestMasterVersionsWalkStop(): void {
  if (_running) _stopRequested = true;
}
export function forceClearMasterVersionsWalk(): void {
...
// outside the window.
async function _loadMasterIds(yearMax: number): Promise<number[]> {
  const pool = getPool();
  try {
    ...
    if (r.rows.length > 0) return r.rows.map(row => Number(row.discogs_id));
  } catch (err) { console.warn("[master-versions-walk] split load failed, falling back to release_cache:", err); }
  const r2 = await pool.query(
    `SELECT discogs_id FROM release_cache
      WHERE type = 'master'
...
export async function startMasterVersionsWalk(opts: {
  ...
  if (_running) return { ok: false, error: "Master-versions walk already running" };
  ...
  if (ids.length === 0) return { ok: false, error: "No cached masters within the yearMax window" };
...
async function _run(client: DiscogsClient): Promise<void> {
  try {
    ...
    _running       = false;
    _stopRequested = false;
    if (drained) console.log(`[master-versions-walk] queue drained; hits=${_state?.hits} skipped=${_state?.skipped} errors=${_state?.errors}`);
    await _persist();
  }
}

async function _walkOne(client: DiscogsClient, masterId: number, yearMax: number): Promise<void> {
  let page = 1;
  const perPage = 100;
  const maxPages = 100;
  while (page <= maxPages) {
...
export function initMasterVersionsWalkModule(adminClerkId: string): void {
  ...
  setTimeout(async () => {
    ...
    const client = await _adminClient();
    if (!client) { console.warn(`[master-versions-walk] boot-resume: no admin client, skipping`); return; }
    _state         = persisted;
    _running       = true;
    _stopRequested = false;
