...
let _currentLabel:   QueueItem | null = null;
...
async function _load(): Promise<BulkState | null> {
  try {
    ...
    return parsed as BulkState;
  } catch { return null; }
}

function _sleep(ms: number): Promise<void> { return new Promise(r => setTimeout(r, ms)); }

export function isBulkLabelSweepRunning(): boolean { return _running; }
...
// to unwind.
export function requestBulkLabelSweepStop(): void {
  if (!_running) return;
  _stopRequested = true;
  try { requestCacheWarmCatnoStop(); } catch {}
}

// Nuke in-memory + persisted state. Only for stuck-runner recovery.
export function forceClearBulkLabelSweep(): void {
...
export function initBulkLabelSweepModule(): void {
  setTimeout(async () => {
    if (_running) return;
