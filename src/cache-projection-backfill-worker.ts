...
import {
  getAppSetting,
  setAppSetting,
  writeProjectedCacheBatch,
  readReleaseCacheBatchForProjection,
...
interface BulkState {
  ...
  batchSize:     number;
  releaseCacheProjectable: number | null;
}

let _state:         BulkState | null = null;
...
async function _persist(): Promise<void> {
  try { await setAppSetting(STATE_KEY, _state ? JSON.stringify(_state) : null); } catch {}
}
async function _load(): Promise<BulkState | null> {
  try {
    const raw = await getAppSetting(STATE_KEY);
    if (!raw) return null;
    return JSON.parse(raw) as BulkState;
  } catch { return null; }
}

export function isCacheProjectionBackfillRunning(): boolean { return _running; }
...
export async function startCacheProjectionBackfill(opts: {
  ...
  if (_running) return { ok: false, error: "Backfill already running" };
...
async function _run(): Promise<void> {
  try {
    while (_state && !_stopRequested) {
      ...
      if (result.lastError) _state.lastError = result.lastError;
      ...
      for (const row of batch) {
        if (row.discogs_id > maxIdInBatch) maxIdInBatch = row.discogs_id;
      }
...
export function initCacheProjectionBackfillModule(): void {
  setTimeout(async () => {
    ...
    if (persisted.cursorId <= 0 && persisted.processed <= 0) return;
